...
#define BVERIFY_ATTRIBUTE_INT(_ATTR_, _MIN_SIZE_)                              \
  do {                                                                         \
    ...
    if (!request[_ATTR_].empty() &&                                            \
        request[_ATTR_] != SToStr(SToInt(request[_ATTR_])))                    \
...
#define BVERIFY_ATTRIBUTE_INT64(_ATTR_, _MIN_SIZE_)                            \
  do {                                                                         \
    if (request[_ATTR_].size() < _MIN_SIZE_)                                   \
      throw "402 Missing " _ATTR_;                                             \
    if (!request[_ATTR_].empty() &&                                            \
        request[_ATTR_] != SToStr(SToInt64(request[_ATTR_])))                  \
      throw "402 Malformed " _ATTR_;                                           \
  } while (false)
...
#define BVERIFY_ATTRIBUTE_DATE(_ATTR_, _MIN_SIZE_)                             \
  do {                                                                         \
    if (request[_ATTR_].size() < _MIN_SIZE_)                                   \
      throw "402 Missing " _ATTR_;                                             \
    if (!request[_ATTR_].empty() &&                                            \
        !SREMatch("^\\d{4}-\\d{2}-\\d{2}( \\d{2}:\\d{2}:\\d{2})?$",            \
                  request[_ATTR_]))                                            \
      throw "402 Malformed " _ATTR_;                                           \
  } while (false)
...
// Common max sizes
#define BMAX_SIZE_NONCOLUMN                                                    \
  1024 * 1024 * 1024                // No need to restrict size on these inputs.
...
// BREGISTER_PLUGIN is a macro to auto-instantiate a global instance of a plugin
// (_CT_). This lets plugin implementors
// just write `BREGISTER_PLUGIN(MyPluginName)` at the bottom of an
// implementation cpp file and get the plugin
// automatically registered with Bedrock.
//
// Why the weird `EXPAND{N}` macros? Because the pre-processor doesn't do
// recursive macro expansion with concatenation.
// See: http://stackoverflow.com/questions/1597007/
#define BREGISTER_PLUGIN_EXPAND2(_CT_, _NUM_)                                  \
  _CT_ __BREGISTER_PLUGIN_##_CT_##_NUM_
#define BREGISTER_PLUGIN_EXPAND1(_CT_, _NUM_)                                  \
  BREGISTER_PLUGIN_EXPAND2(_CT_, _NUM_)
