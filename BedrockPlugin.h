...
#define BVERIFY_ATTRIBUTE_INT(       _ATTR_, _MIN_SIZE_ )             do { if( request[_ATTR_].size() < _MIN_SIZE_ ) throw "402 Missing " _ATTR_; if( !request[_ATTR_].empty() && request[_ATTR_]!=SToStr(SToInt(  request[_ATTR_])) ) throw "402 Malformed " _ATTR_; } while(false)
#define BVERIFY_ATTRIBUTE_INT64(     _ATTR_, _MIN_SIZE_ )             do { if( request[_ATTR_].size() < _MIN_SIZE_ ) throw "402 Missing " _ATTR_; if( !request[_ATTR_].empty() && request[_ATTR_]!=SToStr(SToInt64(request[_ATTR_])) ) throw "402 Malformed " _ATTR_; } while(false)
#define BVERIFY_ATTRIBUTE_SIZE(      _ATTR_, _MIN_SIZE_, _MAX_SIZE_ ) do { if( request[_ATTR_].size() < _MIN_SIZE_ ) throw "402 Missing " _ATTR_; if( request[_ATTR_].size()>_MAX_SIZE_ ) throw "402 Malformed " _ATTR_; } while(false)
#define BVERIFY_ATTRIBUTE_DATE(      _ATTR_, _MIN_SIZE_ )             do { if( request[_ATTR_].size() < _MIN_SIZE_ ) throw "402 Missing " _ATTR_; if( !request[_ATTR_].empty() && !SREMatch( "^\\d{4}-\\d{2}-\\d{2}( \\d{2}:\\d{2}:\\d{2})?$", request[_ATTR_] ) ) throw "402 Malformed " _ATTR_; } while(false)
#define BTOLOWER_ATTRIBUTE( _ATTR_ ) do { if( request.isSet(_ATTR_) ) request[_ATTR_]=SToLower(request[_ATTR_]); } while(false)
#define BTOUPPER_ATTRIBUTE( _ATTR_ ) do { if( request.isSet(_ATTR_) ) request[_ATTR_]=SToUpper(request[_ATTR_]); } while(false)
