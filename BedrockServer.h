  ...
  public: // External Bedrock
    ...
    struct Thread {
        Thread(const string& name_,                              // Thraed name
               ...
               SSynchronized<SQLCState>& replicationState_,      // Shared var for
                                                                 // communicating
                                                                 // replication thread's
                                                                 // status.
               SSynchronized<uint64_t>& replicationCommitCount_, // Shared var for
                                                                 // communicating
                                                                 // replication
                                                                 // thread's
               // commit count (for sticky connections)
               SSynchronized<bool>& gracefulShutdown_, // Shared var for
                                                       // communicating shutdown
                                                       // status between threads.
               SSynchronized<string>& masterVersion_,  // Shared var for
                                                       // communicating the master
                                                       // version (for knowing if
               // we should skip the slave peek).
               MessageQueue& queuedRequests_,          // Shared external queue between
                                                       // threads. Queued for read-only
                                                       // thread(s)
               MessageQueue& queuedEscalatedRequests_, // Shared external queue
                                                       // between threads. Queued
                                                       // for replication thread
               MessageQueue& processedResponses_,      // Shared external queue between
                                                       // threads. Finished commands
                                                       // ready to
                                                       // return to client.
    ...
    // Control the command port. The server will toggle this as necessary, unless
    ...
    // Each plugin can register as many httpsManagers as it likes. They'll all get
    // checked for activity in the
