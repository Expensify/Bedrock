...
void BedrockTester::onResponse( const string& host, const SData& request, const SData& response )
{
    ...
    if( request.isSet("BedrockTester.correctResponse") )
    {
        ...
        for_each(correctResponse.nameValueMap.begin(), correctResponse.nameValueMap.end(), [&correctResponse](pair<string, string> item) {
...
void BedrockTest( SData& trueArgs )
{
    ...
    if( trueArgs.isSet("-all") )
    {
        ...
        STestTimer test( "Benchmarking " +SToStr(NUM_COMMANDS)+ " small read commands on " +SToStr(numReadThreads)+ " read threads" );
            tester->startServer( );
        ...
        STestTimer test( "Benchmarking " +SToStr(NUM_COMMANDS)+ " small write commands on " +SToStr(NUM_THREADS)+ " read threads" );
            tester->startServer( );
            SStopwatch benchmark;
        ...
        STestTimer test( "Benchmarking " +SToStr(NUM_COMMANDS)+ " large read commands on " +SToStr(numReadThreads)+ " read threads" );
            SData args;
