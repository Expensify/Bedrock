...
void BedrockTester::onResponse(const string &host, const SData &request,
  ...
  if (request.isSet("BedrockTester.correctResponse")) {
    ...
    SASSERTEQUALS(SToInt(response.methodLine),
                  SToInt(correctResponse.methodLine));
    for_each(correctResponse.nameValueMap.begin(),
             correctResponse.nameValueMap.end(),
             [&correctResponse](pair<string, string> item) {
...
void BedrockTester::sendQuery(const string &query, int numToSend,
...
void BedrockTester::testRequest(const SData &request,
                                const SData &correctResponse) {
...
void BedrockTest(SData &trueArgs) {
  ...
  if (trueArgs.isSet("-all")) {
    ...
    for (int numReadThreads = 1; numReadThreads <= 4; ++numReadThreads) {
      ...
      STestTimer test("Benchmarking " + SToStr(NUM_COMMANDS) +
                      " small read commands on " + SToStr(numReadThreads) +
                      " read threads");
      tester->startServer();
      SStopwatch benchmark;
      tester->sendQuery("SELECT 1;", NUM_COMMANDS);
      uint64_t commandsPerSec =
          NUM_COMMANDS * 1000 / (benchmark.elapsed() / STIME_US_PER_MS);
...

    {
      ...
      STestTimer test("Benchmarking " + SToStr(NUM_COMMANDS) +
                      " small write commands on " + SToStr(NUM_THREADS) +
                      " read threads");
      tester->startServer();
      SStopwatch benchmark;
    ...
    for (int numReadThreads = 1; numReadThreads <= 4; numReadThreads++) {
      ...
      STestTimer test("Benchmarking " + SToStr(NUM_COMMANDS) +
                      " large read commands on " + SToStr(numReadThreads) +
                      " read threads");
      SData args;
      ...
      tester->sendQuery("INSERT INTO foo SELECT bar FROM foo;",
                        18); // 2^18 rows
      SStopwatch benchmark;
      tester->sendQuery("SELECT SUM(bar) FROM foo;", NUM_COMMANDS);
      float commandsPerSec = NUM_COMMANDS * 1000.0 /
                             (float)(benchmark.elapsed() / STIME_US_PER_MS);
      STESTLOG(commandsPerSec << " commands/s");
      tester->stopServer();
    }
  }
  ...
  for_each(Plugin::g_registeredPluginList->begin(),
           Plugin::g_registeredPluginList->end(), [&](Plugin *plugin) {
             // Do we need to clean up?
             if (tester->server) {
               // Stop the server to provide a clean test environment for the
               // next plugin
               tester->stopServer();
             }
             ...
             list<string> pluginList =
                 SParseList(SToLower(trueArgs["-plugins"]));
             if (trueArgs.isSet("-all") ||
                 SContains(pluginList, SToLower(plugin->getName()))) {
               // Yep, test this one
