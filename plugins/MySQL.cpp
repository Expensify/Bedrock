...
struct MySQLPacket
{
    ...
    static string serializeQueryResponse(int sequenceID, const SQResult& result)
    {
        ...
        SFOREACHCONST (vector< vector<string> >, result.rows, rowIt) {
            ...
            SFOREACHCONST (vector<string>, *rowIt, cellIt) {
                rowPacket.payload += lenEncStr(*cellIt);
            }
...
bool BedrockPlugin_MySQL::onPortRequestComplete(const SData& response, STCPManager::Socket* s)
{
    ...
    if (SToInt(response.methodLine) == 200) {
        ...
        s->send(MySQLPacket::serializeERR(response.calc("request.sequenceID"), SToInt(response.methodLine), response["error"]));
    }
    return true;
}
