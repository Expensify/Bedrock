...
private:
  // Helper functions
  string _constructNextRunDATETIME(const string &lastScheduled,
                                   const string &lastRun, const string &repeat);
...
void BedrockPlugin_Jobs::upgradeDatabase(BedrockNode *node, SQLite &db) {
  ...
  // These indexes are not used by the Bedrock::Jobs plugin, but provided for
  // easy analysis
  // using the Bedrock::DB plugin.
  SASSERT(db.write(
      "CREATE INDEX IF NOT EXISTS jobsState    ON jobs ( state    );"));
  SASSERT(db.write(
      "CREATE INDEX IF NOT EXISTS jobsName     ON jobs ( name     );"));
  SASSERT(db.write(
      "CREATE INDEX IF NOT EXISTS jobsNextRun  ON jobs ( nextRun  );"));
  SASSERT(db.write(
      "CREATE INDEX IF NOT EXISTS jobsLastRun  ON jobs ( lastRun  );"));
  SASSERT(db.write(
      "CREATE INDEX IF NOT EXISTS jobsPriority ON jobs ( priority );"));

  // This index is used to optimize the Bedrock::Jobs::GetJob call.
  SASSERT(db.write("CREATE INDEX IF NOT EXISTS jobsStatePriorityNextRunName ON "
                   "jobs ( state, priority, nextRun, name );"));
}

// ==========================================================================
bool BedrockPlugin_Jobs::peekCommand(BedrockNode *node, SQLite &db,
                                     BedrockNode::Command *command) {
  ...
  if (SIEquals(request.methodLine, "GetJob")) {
    ...
    if (!db.read("SELECT 1 "
                 ...
                 "WHERE state='QUEUED' "
                 "  AND " +
                     SCURRENT_TIMESTAMP() + ">=nextRun " + "  AND name GLOB " +
                     SQ(name) + " " + "LIMIT 1;",
                 result)) {
    ...
    if (result.empty() || SToInt(result[0][0]) == 0) {
      ...
      if (SIEquals(request["Connection"], "wait")) {
        ...
        // state.
        SINFO("No results found and 'Connection: wait'; placing request on "
              "hold until we get a new job "
              "matching name '"
              << request["name"] << "'");
        request["HeldBy"] = "Jobs:" + name;
        response.clear(); // Clear default response so we don't accidentally
                          // think we're done
  ...
  // ----------------------------------------------------------------------
    ...
    SQResult result;
    if (!db.read("SELECT created, jobID, state, name, nextRun, lastRun, "
                 "repeat, data "
                 "FROM jobs "
                 "WHERE jobID=" +
    ...
    // If unique flag was passed and the job exist in the DB, then we can finish
    // the command without escalating to
    // master.
    if (!request.test("unique")) {
      return false;
    }
    SQResult result;
    SINFO("Unique flag was passed, checking existing job with name "
          << request["name"]);
    ...
    SINFO(
        "Job already existed and unique flag was passed, reusing existing job "
        << result[0][0]);
...
// ==========================================================================
bool BedrockPlugin_Jobs::processCommand(BedrockNode *node, SQLite &db,
                                        BedrockNode::Command *command) {
  // Pull out some helpful variables
  SData &request = command->request;
  ...
  if (SIEquals(request.methodLine, "CreateJob")) {
    ...
    //     - unique - if true, it will check that no other job with this name
    //     already exists, if it does it will
    /// return that jobID
    //
    //     Returns:
    ...
    if (request.test("unique")) {
      ...
      if (!result.empty()) {
        SINFO("Job already existed and unique flag was passed, reusing "
              "existing job "
              << result[0][0]);
        content["jobID"] = result[0][0];
        return true;
    ...
    const string &safeFirstRun = request["firstRun"].empty()
                                     ? SCURRENT_TIMESTAMP()
                                     : SQ(request["firstRun"]);

    // If no data was provided, use an empty object
    const string &safeData =
        request["data"].empty() ? SQ("{}") : SQ(request["data"]);
    ...
    int priority = request.isSet("priority") ? request.calc("priority")
                                             : JOBS_DEFAULT_PRIORITY;

    // We'd initially intended for any value to be allowable here, but for
    // performance reasons, we currently
    // will only allow specific values to try and keep queries fast. If you pass
    // an invalid value, we'll throw
    // here so that the caller can know that he did something wrong rather than
    // having his job sit unprocessed
    // in the queue forever. Hopefully we can remove this restriction in the
    // future.
    ...
    // Create this new job
    db.write("INSERT INTO jobs ( created, state, name, nextRun, repeat, data, "
             "priority ) "
             "VALUES( " +
             SCURRENT_TIMESTAMP() + ", " + SQ("QUEUED") + ", " +
             SQ(request["name"]) + ", " + safeFirstRun + ", " +
             SQ(SToUpper(request["repeat"])) + ", " + safeData + ", " +
             SQ(priority) + " );");
    ...
    if (lastInsertRowID != maxJobID) {
      SALERT("We might be returning the wrong jobID maxJobID="
             << maxJobID << " lastInsertRowID=" << lastInsertRowID);
    }
  ...
  // ----------------------------------------------------------------------
    ...
    string query =
        ...
        "WHERE state='QUEUED' "
        "  AND " +
        SCURRENT_TIMESTAMP() + ">=nextRun " + "  AND name GLOB " + SQ(name) +
        " " + "  AND priority=" + SQ(1000) + " " + "ORDER BY nextRun ASC " +
        "LIMIT 1) " + "UNION ALL " + "SELECT * FROM "
                                     "(SELECT jobID, name, data, priority "
                                     "FROM jobs "
                                     "WHERE state='QUEUED' "
                                     "  AND " +
        SCURRENT_TIMESTAMP() + ">=nextRun " + "  AND name GLOB " + SQ(name) +
        " " + "  AND priority=" + SQ(500) + " " + "ORDER BY nextRun ASC " +
        "LIMIT 1) " + "UNION ALL " + "SELECT * FROM "
                                     "(SELECT jobID, name, data, priority "
                                     "FROM jobs "
                                     "WHERE state='QUEUED' "
                                     "  AND " +
        SCURRENT_TIMESTAMP() + ">=nextRun " + "  AND name GLOB " + SQ(name) +
        " " + "  AND priority=" + SQ(0) + " " + "ORDER BY nextRun ASC " +
        "LIMIT 1) " + ") " + "ORDER BY priority DESC "
                             "LIMIT 1;";
    ...
    if (result.empty()) {
      ...
      // **FIXME: If "Connection: wait" should re-apply the hold.  However, this
      // is super edge
      //          as we could only get here if the job somehow got consumed
      //          between the peek
      //          and process -- which could happen during heavy load.  But it'd
      //          just return
      //          no results (which is correct) faster than it would otherwise
      //          time out.  Either
      //          way the worker will likely just loop, so it doesn't really
      //          matter.
      throw "404 No job found";
    }
  ...
  // ----------------------------------------------------------------------
    ...
    if (state != "RUNNING") {
      SWARN("Trying to finish job#" << request["jobID"]
                                    << ", but isn't RUNNING (" << state << ")");
      throw "405 Can only retry/finish RUNNING jobs";
    }

    // If we're doing RetryJob and there isn't a repeat, construct one with the
    // delay
    ...
    // Are we rescheduling?
    if (!repeat.empty()) {
      // Configured to repeat.  The "nextRun" at this point is still storing the
      // last time this job was
      // *scheduled* to
      // be run; lastRun contains when it was *actually* run.
      const string &lastScheduled = nextRun;
      const string &newNextRun =
          _constructNextRunDATETIME(lastScheduled, lastRun, repeat);
      if (newNextRun.empty()) {
        throw "402 Malformed repeat";
      }
      ...
      // Are we updating the data too? (This can be used to pass state from
      // worker to worker
      ...
      // Update this job
      if (!db.write("UPDATE jobs SET " + SComposeList(updateList) +
      ...
      if (!db.write("DELETE FROM jobs WHERE jobID=" +
                    SQ(request.calc64("jobID")) + ";")) {
  ...
  // ----------------------------------------------------------------------
    ...
    if (state != "RUNNING") {
      SWARN("Trying to fail job#" << request["jobID"] << ", but isn't RUNNING ("
                                  << state << ")");
...
void BedrockPlugin_Jobs::test(BedrockTester *tester) {
  ...
  STESTASSERT(
      _validateRepeat("STARTED, +1 MONTH, -1 DAY, START OF DAY, +4 HOURS"));
  STESTASSERT(!_validateRepeat("FINISHED, WEEKDAY 7")); // only 0-6 weekdays
}

// ==========================================================================
string BedrockPlugin_Jobs::_constructNextRunDATETIME(
    const string &lastScheduled, const string &lastRun, const string &repeat) {
  // Some "canned" times for convenience
  if (SIEquals(repeat, "HOURLY"))
    return "STRFTIME( '%Y-%m-%d %H:00:00', DATETIME( " + SCURRENT_TIMESTAMP() +
           ", '+1 HOUR' ) )";
  if (SIEquals(repeat, "DAILY"))
    return "DATETIME( " + SCURRENT_TIMESTAMP() +
           ", '+1 DAY', 'START OF DAY'  )";
  if (SIEquals(repeat, "WEEKLY"))
    return "DATETIME( " + SCURRENT_TIMESTAMP() +
           ", '+1 DAY', 'WEEKDAY 0', 'START OF DAY' )";
  ...
  SFOREACH(list<string>, parts, partIt) {
    ...
    if (SREMatch("^(\\+|-)\\d{1,3} (YEAR|MONTH|DAY|HOUR|MINUTE|SECOND)S?$",
                 part)) {
      ...
      // Malformed part
      SWARN("Syntax error, failed parsing repeat '" << repeat << "' on part '"
                                                    << part << "'");
      return "";
    }
  }
