...
void BedrockPlugin_Cache::initialize( const SData& args )
{
    ...
    if( SEndsWith( maxCache, "KB" ) ) maxCacheSize *= 1024;
    if( SEndsWith( maxCache, "MB" ) ) maxCacheSize *= 1024*1024;
    if( SEndsWith( maxCache, "GB" ) ) maxCacheSize *= 1024*1024*1024;
    if( !maxCacheSize )
    {
        // Provide a default
