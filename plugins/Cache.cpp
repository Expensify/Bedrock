...
void BedrockPlugin_Cache::LRUMap::pushMRU(const string &name) {
  ...
  if (mapIt == _lruMap.end()) {
    ...
    entry->mapIt =
        _lruMap.insert(std::pair<string, Entry *>(name, entry)).first;
...
void BedrockPlugin_Cache::upgradeDatabase(BedrockNode *node, SQLite &db) {
  ...
  // Add a one row, one column table to keep track of the current size of the
  // cache
  SASSERT(db.verifyTable("cacheSize", "CREATE TABLE cacheSize ( size INTEGER )",
                         ignore));
  SQResult result;
  ...
  SASSERT(db.write(
      "CREATE TRIGGER IF NOT EXISTS cacheOnInsert AFTER INSERT ON cache "
      "BEGIN "
      "UPDATE cacheSize SET size = size + LENGTH( NEW.value ); "
      "END;"));
  SASSERT(db.write(
      "CREATE TRIGGER IF NOT EXISTS cacheOnUpdate AFTER UPDATE ON cache "
      "BEGIN "
      "UPDATE cacheSize SET size = size - LENGTH( OLD.value ) + LENGTH( "
      "NEW.value ); "
      "END;"));
  SASSERT(db.write(
      "CREATE TRIGGER IF NOT EXISTS cacheOnDelete AFTER DELETE ON cache "
      "BEGIN "
...
// ==========================================================================
bool BedrockPlugin_Cache::peekCommand(BedrockNode *node, SQLite &db,
                                      BedrockNode::Command *command) {
  ...
  if (SIEquals(request.methodLine, "ReadCache")) {
    ...
    //         . value - raw value associated with that name (in the body of the
    //         response)
    //     - 404 - No cache found
    //
...
// ==========================================================================
bool BedrockPlugin_Cache::processCommand(BedrockNode *node, SQLite &db,
                                         BedrockNode::Command *command) {
  // Pull out some helpful variables
  SData &request = command->request;
  ...
  if (SIEquals(request.methodLine, "WriteCache")) {
    ...
    //     - value          - Raw data to associate with this value, as a
    //     request header (1MB max) or content
    /// body (64MB max)
    ...
    // Make sure we're not trying to cache something larger than the cache
    // itself
    int64_t contentSize =
        valueHeader.empty() ? request.content.size() : valueHeader.size();
    ...
    if (!request["invalidateName"].empty()) {
      if (!db.write("DELETE FROM cache WHERE name GLOB " +
                    SQ(request["invalidateName"]) + ";"))
        throw "502 Query failed (invalidating)";
    }

    // Clear out room for the new object
    while (SToInt64(db.read("SELECT size FROM cacheSize;")) + contentSize >
           _maxCacheSize) {
      // Find the least recently used (LRU) item if there is one.  (If the
      // server was recently restarted,
      // its LRU might not be fully populated.)
      const string &name =
          (_lruMap.empty() ? db.read("SELECT name FROM cache LIMIT 1")
                           : _lruMap.popLRU());
      SASSERT(!name.empty());
    ...
    const string &name = request["name"];
    const string &safeValue =
        SQ(valueHeader.empty() ? request.content : valueHeader);
