...
// ==========================================================================
bool BedrockPlugin_Status::peekCommand(BedrockNode *node, SQLite &db,
                                       BedrockNode::Command *command) {
  ...
  if (SIEquals(request.methodLine, "GET /status/isSlave HTTP/1.1")) {
    ...
    if (node->getState() == SQLC_SLAVING)
      ...
      response.methodLine = "HTTP/1.1 500 Not slaving. State=" +
                            (string)SQLCStateNames[node->getState()];
    return true; // Successfully peeked
  }

  // --------------------------------------------------------------------------
  else if (SIEquals(request.methodLine,
    ...
    if (node->getState() != SQLC_SLAVING) {
      ...
      response.methodLine =
          "HTTP/1.1 500 Mismatched version. Version=" + node->getVersion();
    } else {
      response.methodLine = "HTTP/1.1 200 Slaving";
    }
    return true; // Successfully peeked
  ...
  // ----------------------------------------------------------------------
    ...
    content["queuedCommandList"] =
        SComposeJSONArray(node->getQueuedCommandList());
    content["escalatedCommandList"] =
        SComposeJSONArray(node->getEscalatedCommandList());
    content["processedCommandList"] =
        SComposeJSONArray(node->getProcessedCommandList());
