...
# If the TRAVIS_BRANCH env variable is not set to main, fetch the main branch so we can compare against it
if [[ "$TRAVIS_BRANCH" != "main" ]]; then
    git fetch origin main:main
fi

# Run 'git fetch origin' so that we have a local branch called $TRAVIS_BRANCH that we can compare against
# Only if it starts with pull/
if [[ "$TRAVIS_BRANCH" == pull/* ]]; then
...
done < <(git diff --name-status --diff-filter=AM main..."${TRAVIS_BRANCH}" -- '*.cpp' '*.h')
