...
void RetrySystem(const string &command) {
  ...
  int numRetries = 3;
  SINFO("Trying to run '" << command << "' up to " << numRetries
                          << " times...");
  while (numRetries--) {
    ...
    if (returnCode) {
      // Didn't work
      SWARN("'" << command << "' failed with return code " << returnCode
                << ", waiting 5s and retrying " << numRetries << " more times");
...
void BackupDB(const string &dbPath) {
  ...
  if (SFileExists(dbWalPath)) {
    ...
    SASSERT(SFileCopy(
        dbWalPath, BACKUP_DIR + string(basename((char *)dbWalPath.c_str()))));
  ...
  if (SFileExists(dbShmPath)) {
    ...
    SASSERT(SFileCopy(
        dbShmPath, BACKUP_DIR + string(basename((char *)dbShmPath.c_str()))));
...
int main(int argc, char *argv[]) {
  ...
  if (args.empty()) {
    ...
    cout << "Protip: check syslog for details, or run 'bedrock -?' for help"
         << endl;
  }
  ...
  if (args.isSet("-h") || args.isSet("-?") || args.isSet("-help")) {
    ...
    cout << "bedrock [-clean] [-v] [-db <filename>] [-serverHost <host:port>] "
            "[-nodeHost <host:port>] [-nodeName "
            "<name>] [-peerList <list>] [-priority <value>] [-plugins <list>] "
            "[-cacheSize <kb>] [-readThreads <#>] "
            "[-versionOverride <version>]"
         << endl;
    cout << endl;
    cout << "Common Commands:" << endl;
    cout << "----------------" << endl;
    cout << "-?, -h, -help               Outputs instructions and exits"
         << endl;
    cout << "-version                    Outputs version and exits" << endl;
    cout << "-v                          Enables verbose logging" << endl;
    cout << "-clean                      Recreate a new database from scratch"
         << endl;
    cout << "-versionOverride <version>  Pretends to be a different version "
            "when talking to peers"
         << endl;
    cout << "-db             <filename>  Use a database with the given name "
            "(default 'bedrock.db')"
         << endl;
    cout << "-serverHost     <host:port> Listen on this host:port for cluster "
            "connections (default 'localhost:8888')"
         << endl;
    cout << "-nodeName       <name>      Name this specfic node in the cluster "
            "as indicated (defaults to '"
         << SGetHostName() << "')" << endl;
    cout << "-nodeHost       <host:port> Listen on this host:port for "
            "connections from other nodes"
         << endl;
    cout << "-peerList       <list>      See below" << endl;
    cout << "-priority       <value>     See '-peerList Details' below "
            "(defaults to 100)"
         << endl;
    cout << "-plugins        <list>      Enable these plugins (defaults to "
            "'status,db,jobs,cache')"
         << endl;
    cout << "-cacheSize      <kb>        number of KB to allocate for a page "
            "cache (defaults to 1GB)"
         << endl;
    cout << "-readThreads    <#>         Number of read threads to start (min "
            "1, defaults to 1)"
         << endl;
    cout << "-queryLog       <filename>  Set the query log filename (default "
            "'queryLog.csv', SIGUSR2/SIGQUIT to "
            "enable/disable)"
         << endl;
    cout << "-maxJournalSize <#commits>  Number of commits to retainin the "
            "historical journal (default 1000000)"
         << endl;
    cout << endl;
    cout << "Quick Start Tips:" << endl;
    cout << "-----------------" << endl;
    cout << "In a hurry?  Just run 'bedrock -clean' the first time, and it'll "
            "create a new database called "
            "'bedrock.db', then use all the defaults listed above.  (After the "
            "first time, leave out the '-clean' "
            "to reuse the same database.)  Once running, you can verify it's "
            "working using NetCat to manualy send "
    ...
    cout << "------------------" << endl;
    cout << "The -peerList parameter enables you to configure multiple Bedrock "
            "nodes into a redundant cluster.  "
            "Bedrock supports any number of nodes: simply start each node with "
            "a comma-separated list of the "
            "'-nodeHost' of all other nodes.  You can safely send any command "
            "to any node.  Some best practices:"
         << endl;
    cout << endl;
    cout << "- Put each Bedrock node on a different server." << endl;
    cout << endl;
    cout << "- Assign each node a different priority (greater than 0).  The "
            "highest priority node will be the "
...
#define SETDEFAULT(_NAME_, _VAL_)                                              \
  ...
  while (!(SCatchSignal(SIGTERM) || SCatchSignal(SIGINT))) {
    ...
    if (SGetSignals()) {
      // Log and clear
      SALERT("Uncaught exceptions (" << SGetSignalNames(SGetSignals())
                                     << "), ignoring.");
      SClearSignals();
    }

    // Make sure the BedrockServer is destroyed before VACUUM so it lets go of
    // the db files.
    {
      // Run the server
    ...
    if (SCatchSignal(SIGUSR2)) {
      ...
      const string &cmdCheckpointMainWal =
          "sqlite3 " + args["-db"] + " 'SELECT * FROM accounts LIMIT 1;'";
      SINFO("Starting main wal checkpoint. WAL filesize="
            << SToStr(SFileSize(args["-db"] + "-wal") / (float)(1024 * 1024))
            << " MB");
