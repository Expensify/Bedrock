...
void RetrySystem( const string& command )
{
    ...
    while( numRetries-- )
    {
        ...
        {
            // Didn't work
            SWARN( "'" << command << "' failed with return code " << returnCode << ", waiting 5s and retrying " << numRetries << " more times" );
...
int main( int argc, char* argv[] )
{
    ...
    if( args.isSet("-h") || args.isSet("-?") || args.isSet("-help") )
    {
        ...
        cout << "bedrock [-clean] [-v] [-db <filename>] [-serverHost <host:port>] [-nodeHost <host:port>] [-nodeName <name>] [-peerList <list>] [-priority <value>] [-plugins <list>] [-cacheSize <kb>] [-readThreads <#>] [-versionOverride <version>]" << endl;
        cout << endl;
        ...
        cout << "-serverHost     <host:port> Listen on this host:port for cluster connections (default 'localhost:8888')" << endl;
        cout << "-nodeName       <name>      Name this specfic node in the cluster as indicated (defaults to '" << SGetHostName() << "')" << endl;
        ...
        cout << "-queryLog       <filename>  Set the query log filename (default 'queryLog.csv', SIGUSR2/SIGQUIT to enable/disable)" << endl;
        cout << "-maxJournalSize <#commits>  Number of commits to retainin the historical journal (default 1000000)" << endl;
        cout << endl;
        cout << "Quick Start Tips:" << endl;
        cout << "-----------------" << endl;
        cout << "In a hurry?  Just run 'bedrock -clean' the first time, and it'll create a new database called 'bedrock.db', then use all the defaults listed above.  (After the first time, leave out the '-clean' to reuse the same database.)  Once running, you can verify it's working using NetCat to manualy send a Ping request as follows:" << endl;
        cout << endl;
        ...
        cout << "------------------" << endl;
        cout << "The -peerList parameter enables you to configure multiple Bedrock nodes into a redundant cluster.  Bedrock supports any number of nodes: simply start each node with a comma-separated list of the '-nodeHost' of all other nodes.  You can safely send any command to any node.  Some best practices:" << endl;
        cout << endl;
        cout << "- Put each Bedrock node on a different server." << endl;
        cout << endl;
        cout << "- Assign each node a different priority (greater than 0).  The highest priority node will be the 'master', which will coordinate distributed transactions." << endl;
        cout << endl;
        return 1;
    }
    if( args.isSet("-v") )
    ...
    #define SETDEFAULT( _NAME_, _VAL_ ) \
        do { if( !args.isSet(_NAME_) ) args[_NAME_] = _VAL_; } while(false)
    ...
    while (!(SCatchSignal(SIGTERM) || SCatchSignal(SIGINT)))
    {
        ...
        if (SCatchSignal(SIGUSR2))
        {
            ...
            SINFO( "Starting main wal checkpoint. WAL filesize=" << SToStr( SFileSize( args["-db"] + "-wal" ) / (float) (1024 * 1024) ) << " MB" );
