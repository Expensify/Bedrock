...
* Pass the *expectedData* string returned by `GetJob` or `GetJobs` unchanged to `FinishJob`, `RetryJob`, or `FailJob`. This string contains the job's original JSON data; do not decode and re-encode it.
