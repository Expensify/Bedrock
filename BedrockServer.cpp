...
void BedrockServer::waitForHTTPS(BedrockCommand&& command) {
    ...
    SINFO("Moving BedrockCommand " << command.request.methodLine << " to _outstandingHTTPSCommands queue.");
    ...
    for (auto transaction : completedHTTPSRequests) {
        ...
        if (commandPtrIt != _outstandingHTTPSCommands.end()) {
            ...
            if (commandPtr->areHttpsRequestsComplete()) {
                ...
                SINFO("Moving BedrockCommand " << commandPtr->request.methodLine << " to _outstandingHTTPSCommands queue.");
