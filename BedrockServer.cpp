...
void BedrockServer_WorkerThread_ProcessDirectMessages(
    BedrockNode &node, BedrockServer::MessageQueue &directMessages) {
  ...
  while (true) {
    ...
    if (SIEquals(message.methodLine, "CANCEL_REQUEST")) {
      ...
      BedrockNode::Command *command =
          node.findCommand("requestCount", message["requestCount"]);
      if (command) {
        if (command->response.empty()) {
          SINFO("Canceling unprocessed request #"
                << message["requestCount"] << " ("
                << command->request.methodLine << ")");
        } else {
          SWARN("Canceling processed request #"
                << message["requestCount"] << " ("
                << command->request.methodLine
        ...
        SINFO("No need to cancel request #"
              << message["requestCount"] << " because not queued, ignoring.");
      }
    } else
...
void BedrockServer_WorkerThread(void *_data) {
  ...
  BedrockServer::MessageQueue &queuedEscalatedRequests =
      data->queuedEscalatedRequests;
  ...
  SINFO("Starting " << (readOnly ? "read-only" : "read/write")
                    << " worker thread for '" << data->name << "'");
  ...
  if (readOnly) {
    for (;;) {
      // Set the read-only node's state/master status coming from the
      // replication thread.
      // Only read-only nodes will allow an external party to set these
      // properties.
      ...
      // If we've been instructed to shutdown and there are no more requests
      // waiting
      // to be processed, then exit the loop. Main thread will join us and
      // continue
      ...
      SINFO("Dispatching request '"
            << request.methodLine << "' (Connection: " << request["Connection"]
            << ", creationTimestamp: " << creationTimestamp << ")");

      node.openCommand(request, priority, false, creationTimestamp);

      // Now pull that same command off the internal queue and put it on the
      // appropriate external (threaded) queue
      BedrockNode::Command *command = 0;
      if ((command = node.getProcessedCommand())) {
        // If it was fully processed in openCommand(), that means it was peeked
        // successfully.
        SINFO("Peek successful. Putting command '" << command->id
                                                   << "' on processed list.");
        ...
        // Otherwise, it must be unpeekable -- make sure it didn't open any
        // secondary request, and send to
        // the write thread.
        SASSERT(!command->httpsRequest);
        SINFO("Peek unsuccessful. Signaling replication thread to process "
              "command '"
              << command->id << "'.");
        queuedEscalatedRequests.push(request);
      } else
        SERROR("[dmb] Lost command after read-only peek. This should never "
               "happen");
    ...
    while (!node.shutdownComplete()) {
      // Update shared var so all threads have awareness of the current
      // replication state
      ...
      for_each(httpsManagers.begin(), httpsManagers.end(),
               [&](list<SHTTPSManager *> managerList) {
                 for_each(
                     managerList.begin(), managerList.end(),
      ...
      while ((command = node.getProcessedCommand())) {
        ...
        SINFO("Putting escalated command '" << command->id
                                            << "' on processed list.");
        BedrockServer_PrepareResponse(command);
        processedResponses.push(command->response);
...
BedrockServer::BedrockServer(const SData &args)
    : STCPServer(""), _replicationState(SQLC_SEARCHING),
      _replicationCommitCount(0), _nodeGracefulShutdown(false),
      _masterVersion("") {
  // Initialize
  _args = args;
  _requestCount = 0;
  ...
  _version =
      args.isSet("-versionOverride") ? args["-versionOverride"] : SVERSION;

  // Output the list of plugins compiled in
  map<string, BedrockNode::Plugin *> registeredPluginMap;
  SFOREACH(list<BedrockNode::Plugin *>,
           *BedrockNode::Plugin::g_registeredPluginList, pluginIt) {
    // Add one more plugin
  ...
  SFOREACH(list<string>, pluginNameList, pluginNameIt) {
    ...
    // As this is a list of lists, push_back will push a *copy* of the list onto
    // our local list, meaning that the
    // plugin's list must be complete and final when `initialize` finishes.
    // There is no facility to add more
  ...
  _writeThread =
      new Thread("write0", _args, _replicationState, _replicationCommitCount,
                 _nodeGracefulShutdown, _masterVersion, _queuedRequests,
                 _queuedEscalatedRequests, _processedResponses, this);
  _writeThread->args["-readOnly"] = "false";
  SINFO("Lauching write thread '" << _writeThread->name << "'");
  _writeThread->thread =
      SThreadOpen(BedrockServer_WorkerThread, _writeThread, _writeThread->name);
  while (!_writeThread->ready.get()) {
    // Wait a bit longer
    SINFO("Waiting for '" << _writeThread->name
  ...
  for (int c = 0; c < readThreads; ++c) {
    ...
    Thread *readThread = new Thread(
        "read" + SToStr(c), _args, _replicationState, _replicationCommitCount,
        _nodeGracefulShutdown, _masterVersion, _queuedRequests,
        _queuedEscalatedRequests, _processedResponses, this);
    ...
    readThread->thread =
        SThreadOpen(BedrockServer_WorkerThread, readThread, readThread->name);
...
BedrockServer::~BedrockServer() {
  ...
  for (list<Socket *>::iterator socketIt = socketList.begin();
       socketIt != socketList.end();) {
...
bool BedrockServer::shutdownComplete() {
  ...
  return _nodeGracefulShutdown.get() &&
         _replicationState.get() <= SQLC_WAITING && _queuedRequests.empty() &&
...
void BedrockServer::postSelect(fd_map &fdm, uint64_t &nextActivity) {
  ...
  _processedResponses.postSelect(
      fdm, 100); // Can 'consume' up 100 processed responses.

  // Open the port the first time we enter a command-processing state
  SQLCState state = _replicationState.get();

  // If we're a slave, and the master's on a different version than us, we don't
  // open the command port.
  // If we do, we'll escalate all of our commands to the master, which causes
  // undue load on master during upgrades.
  // Instead, we'll simply not respond and let this request get re-directed to
  // another slave.
  string masterVersion = _masterVersion.get();
  if (!_suppressCommandPort && state == SQLC_SLAVING &&
      (masterVersion != _version)) {
    SINFO("Node " << _args["-nodeName"] << " slaving on version " << _version
                  << ", master is version: " << masterVersion
                  << ", not opening command port.");
    suppressCommandPort(true);

    // If we become master, or if master's version resumes matching ours, open
    // the command port again.
  } else if (_suppressCommandPort &&
             (state == SQLC_MASTERING || (masterVersion == _version))) {
    SINFO("Node " << _args["-nodeName"] << " disabling previously suppressed "
                                           "command port after version check.");
    suppressCommandPort(false);
  }

  if (!_suppressCommandPort && port < 0 &&
      (state == SQLC_MASTERING || state == SQLC_SLAVING) &&
      !_nodeGracefulShutdown.get()) {
    // Open the port
    SINFO("Ready to process commands, opening command port on '"
          << _args["-serverHost"] << "'");
    openPort(_args["-serverHost"]);
  }

  // **NOTE: We leave the port open between startup and shutdown, even if we
  // enter a state where
  //         we can't process commands -- such as a non master/slave state.  The
  //         reason is we
  //         expect any state transitions between startup/shutdown to be due to
  //         temporary conditions
  //         that will resolve themselves automatically in a short time.  During
  //         this periond we
  //         prefer to receive commands and queue them up, even if we can't
  //         proesss them immediately,
  //         on the assumption that we'll be able to process them before the
  //         browser times out.
  ...
  if (sigmask) {
    ...
    if (SCatchSignal(SIGTTIN)) {
      ...
      if (_args.isSet("-queryLog")) {
        ...
        SWARN("Can't begin logging queries because -queryLog isn't set, "
              "ignoring.");
      }
      SClearSignals();
      ...
      // For anything else, just shutdown -- but only if we're not already
      // shutting down
      if (!_nodeGracefulShutdown.get()) {
        // Begin a graceful shutdown; close our port
        SINFO("Beginning graceful shutdown due to '"
              << SGetSignalNames(sigmask) << "', closing command port on '"
              << _args["-serverHost"] << "'");
  ...
  while (socketIt != socketList.end()) {
    ...
    if (s->state == STCP_CLOSED) {
        ...
        if (mapIt->second == s) {
          ...
          if (_queuedRequests.cancel("requestCount", SToStr(requestCount))) {
            SINFO("Cancelling abandoned request #"
                  << requestCount
                  << " in queuedRequests; was never processed by read thread.");
          } else if (_queuedEscalatedRequests.cancel("requestCount",
                                                     SToStr(requestCount))) {
            SINFO("Cancelling abandoned request #"
                  << requestCount << " in queuedEscalatedRequests; was never "
                                     "processed by write thread.");
          } else if (_processedResponses.cancel("request.requestCount",
                                                SToStr(requestCount))) {
            SWARN("Can't cancel abandoned request #"
                  << requestCount
                  << " in processedResponses; this *was* processed by the "
                     "write thread, but too late now.");
          } else {
            // Doesn't seem to be in any of the queues, meaning it's actively
            // being processed by one of the
            // threads.
            // Send a cancel command to all threads.  This will *probably* work,
            // but it's possible that the
            // thread will
            // finish processing this command before it gets to processing our
            // cancel request.  But that's
            // fine -- this
            // doesn't need to be airtight.  There will always be scenarios
            // where the server processes a
            // command that
            // the client has abandoned (eg, if the socket dies while sending
            // the response), so the client
            // already needs
            // to handle this scenario.  We just want to minimize it wherever
            // possible.
            SHMMM("Attempting to cancel abandoned request #"
                  << requestCount << " being processed by some thread; it "
      ...
      while ((requestSize = request.deserialize(s->recvBuffer))) {
        ...
        // Either shut down the socket or store it so we can eventually write
        // out the response.
        if (SIEquals(request["Connection"], "forget")) {
          ...
          SINFO("Recording '" << request.methodLine << "' as requestCount #"
                              << requestCount << " for later response.");
  ...
  while (!_processedResponses.empty()) {
    ...
    uint64_t totalTime =
        STimeNow() - response.calc64("request.creationTimestamp");
    ...
    map<uint64_t, Socket *>::iterator socketIt =
        _requestCountSocketMap.find(requestCount);
    Socket *s =
        (socketIt != _requestCountSocketMap.end() ? socketIt->second : 0);

    // **FIXME: Abandon requests are mistaken for being internal; somehow detect
    // this and give plugins
    //          a chance to repair the problem.  Specifically, the Jobs plugin
    //          doesn't want to send a
    ...
    const string &commandStatus = "'" + response["request.methodLine"] + "' "
                                  ...
                                  (s ? SToStr(s->addr) : "internal") + "' "
                                                                       "in " +
                                  SToStr(totalTime / STIME_US_PER_MS) + "=" +
                                  SToStr(waitTime / STIME_US_PER_MS) + "+" +
                                  SToStr(processingTime / STIME_US_PER_MS) +
                                  " ms";
    ...
    // Warn on high latency commands.  Lets not include ones that needed to send
    // out other requests though.
    ...
    if (s) {
      ...
      if (!SIEquals(response["request.Connection"], "forget"))
        SWARN("Cannot deliver response for request '"
              << response["request.methodLine"] << "' #" << requestCount);
    }
  }

  // If any plugin timers are firing, let the plugins know.
  for_each(BedrockNode::Plugin::g_registeredPluginList->begin(),
           BedrockNode::Plugin::g_registeredPluginList->end(),
           [&](BedrockNode::Plugin *plugin) {
             for_each(plugin->timers.begin(), plugin->timers.end(),
                      [&](SStopwatch *timer) {
                        if (timer->ding()) {
...
void BedrockServer::suppressCommandPort(bool suppress, bool manualOverride) {
  // If we've set the manual override flag, then we'll only actually make this
  // change if we've specified it again.
  if (_suppressCommandPortManualOverride && !manualOverride) {
    return;
  }

  // Save the state of manual override. Note that it's set to *suppress* on
  // purpose.
  if (manualOverride) {
...
void BedrockServer::queueRequest(const SData &request) {
  // This adds a request to the queue, but it doesn't affect our `select` loop,
  // so any messages queued here may not
  // trigger until the next time `select` finishes (which should be within 1
  // second).
  // We could potentially interrupt the select loop here (perhaps by writing to
  // our own incoming server socket) if
