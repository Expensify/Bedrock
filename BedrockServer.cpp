...
void BedrockServer::sync()
{
    ...
    _dbPool = make_shared<SQLitePool>(_replicationState, fdLimit, args["-db"], args.calc("-cacheSize"), args.calc("-maxJournalSize"), workerThreads, args["-synchronous"], mmapSizeGB);
    ...
    while (true) {
        ...
        }
        {
            ...
            if (!calledPeek || peekResult == BedrockCore::RESULT::SHOULD_PROCESS) {
                ...
                // called to check it.
                if (!command->processOnFollowers() || command->onlyProcessOnSyncThread() || !canWriteParallel) {
