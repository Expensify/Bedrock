...
void BedrockServer::worker(int threadId)
{
    ...
    while (true) {
        try {
            ...
            cout << args["-nodeName"] << " " << "Dequeued command " << command->request.methodLine << " in worker, "
                  << commandQueue.size() << " commands in " << (threadId ? "" : "blocking") << " queue." << endl;
            ...
            if (command->complete) {
                ...
                cout << args["-nodeName"] << " " << "Command " << command->request.methodLine << " not complete." << endl;
            }
            ...
            while (retry) {
                ...
                if (!calledPeek || peekResult == BedrockCore::RESULT::SHOULD_PROCESS) {
                    ...
                    if (command->onlyProcessOnSyncThread() || !canWriteParallel) {
                        ...
                        cout << args["-nodeName"] << " " << "Sending non-parallel command " << command->request.methodLine << "(commandID: " << command->id << ") to leader." << endl;
...
void BedrockServer::_reply(unique_ptr<BedrockCommand>& command) {
    ...
    cout << args["-nodeName"] << " replying to " << command->request.methodLine << endl;
    cout << command->response.serialize() << endl;

    // Don't reply to commands with pseudo-clients (i.e., commands that we generated by other commands).
    if (command->initiatingClientID < 0) {
        cout << args["-nodeName"] << " e1" << endl;
        return;
    }
...
void BedrockServer::_postPollCommands(fd_map& fdm, uint64_t nextActivity) {
    ...
    while (it != _outstandingHTTPSCommands.end()) {
        ...
        if (command->areHttpsRequestsComplete()) {
            ...
            cout << args["-nodeName"] << " " << "All HTTPS requests complete, returning to main queue." << endl;
...
void BedrockServer::waitForHTTPS(unique_ptr<BedrockCommand>&& command) {
    ...
    cout << args["-nodeName"] << " " << "_outstandingHTTPSCommands.size(): " << _outstandingHTTPSCommands.size() << endl;
}
