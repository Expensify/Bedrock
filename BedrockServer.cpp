...
void BedrockServer_WorkerThread_ProcessDirectMessages( BedrockNode& node, BedrockServer::MessageQueue& directMessages ) {
    ...
    while( true ) {
        ...
        if( message.methodLine.empty() ) break;
        ...
        if( SIEquals( message.methodLine, "CANCEL_REQUEST" ) ) {
            ...
            if( command ) {
                if( command->response.empty() ) {
                    SINFO( "Canceling unprocessed request #" << message["requestCount"] << " (" << command->request.methodLine << ")" );
                }
                else {
                    SWARN( "Canceling processed request #" << message["requestCount"] << " (" << command->request.methodLine << "), response=" << command->response.methodLine << ")" );
                }
        ...
        else SWARN( "Unrecognized message '" << message.methodLine << "', ignoring." );
    }
}
...
void BedrockServer_WorkerThread( void* _data )
{
    ...
    if( readOnly )
    {
        for( ;; )
        {
            ...
            SINFO( "Dispatching request '" << request.methodLine << "' (Connection: " << request["Connection"] << ", creationTimestamp: " << creationTimestamp << ")" );
    ...
    else
    {
        ...
        while( !node.shutdownComplete( ) )
        {
            ...
            for_each(httpsManagers.begin(), httpsManagers.end(), [&](list<SHTTPSManager*> managerList){
                for_each(managerList.begin(), managerList.end(), [&](SHTTPSManager* manager){
      ...
      _masterVersion( "" )
{
    ...
    SFOREACH( list<string>, pluginNameList, pluginNameIt )
    {
        ...
        if( !plugin ) SERROR( "Cannot find plugin '" << pluginName << "', aborting." );
    ...
    _writeThread->thread = SThreadOpen(
        BedrockServer_WorkerThread,
        _writeThread,
        _writeThread->name
    );
...
bool BedrockServer::shutdownComplete( )
{
    ...
    return _nodeGracefulShutdown.get( )             &&
           _replicationState.get( ) <= SQLC_WAITING &&
           _queuedRequests.empty( )                 &&
           _queuedEscalatedRequests.empty( )        &&
           _processedResponses.empty( );
}
...
void BedrockServer::postSelect( fd_map& fdm, uint64_t& nextActivity )
{
    ...
    if( !_suppressCommandPort && port<0 && (state == SQLC_MASTERING || state == SQLC_SLAVING) && !_nodeGracefulShutdown.get( ) )
    {
        // Open the port
    ...
    if (sigmask)
    {
        ...
        else
        {
            ...
            if(!_nodeGracefulShutdown.get())
            {
                ...
                SINFO( "Beginning graceful shutdown due to '" << SGetSignalNames(sigmask) << "', closing command port on '" << _args["-serverHost"] << "'" );
            ...
            while( nextIt != _requestCountSocketMap.end() )
            {
                ...
                if( mapIt->second == s )
                {
                    ...
                    if( _queuedRequests.cancel( "requestCount", SToStr(requestCount) ) ) {
                        SINFO( "Cancelling abandoned request #" << requestCount << " in queuedRequests; was never processed by read thread." );
                    }
                    else if( _queuedEscalatedRequests.cancel( "requestCount", SToStr(requestCount) ) ) {
                        SINFO( "Cancelling abandoned request #" << requestCount << " in queuedEscalatedRequests; was never processed by write thread." );
                    }
                    else if( _processedResponses.cancel( "request.requestCount", SToStr(requestCount) ) ) {
                        SWARN( "Can't cancel abandoned request #" << requestCount << " in processedResponses; this *was* processed by the write thread, but too late now." );
                    }
                    else {
                        // Doesn't seem to be in any of the queues, meaning it's actively being processed by one of the threads.
                        // Send a cancel command to all threads.  This will *probably* work, but it's possible that the thread will
                        // finish processing this command before it gets to processing our cancel request.  But that's fine -- this
                        // doesn't need to be airtight.  There will always be scenarios where the server processes a command that
                        // the client has abandoned (eg, if the socket dies while sending the response), so the client already needs
                        // to handle this scenario.  We just want to minimize it wherever possible.
                        SHMMM( "Attempting to cancel abandoned request #" << requestCount << " being processed by some thread; it might slip through the cracks." );
        ...
        else if( s->state == STCP_CONNECTED )
        {
            ...
            while( (requestSize = request.deserialize( s->recvBuffer )) )
            {
                ...
                else
                {
                    ...
                    SINFO( "Recording '" << request.methodLine << "' as requestCount #" << requestCount << " for later response." );
    ...
    while( !_processedResponses.empty( ) )
    {
        ...
        const string& commandStatus =
            ...
            "#" + SToStr(requestCount) + " "
            "(result '" + response.methodLine + "') "
            "from '" + ( s ? SToStr(s->addr) : "internal" ) + "' "
            "in " + SToStr(totalTime/STIME_US_PER_MS) + "=" + SToStr(waitTime/STIME_US_PER_MS) + "+" + SToStr(processingTime/STIME_US_PER_MS) + " ms";
        ...
        if( s )
        {
            ...
            for( map<string,string>::iterator it = response.nameValueMap.begin( ); it != response.nameValueMap.end( ); /* no inc. handled in loop body*/ )
        ...
        else
        {
            ...
            if( !SIEquals( response["request.Connection"], "forget" ) )
                SWARN( "Cannot deliver response for request '" << response["request.methodLine"] << "' #" << requestCount );
        }
    }
