...
int tpunit::_TestFixture::tpunit_detail_do_run(int threads, function<void()> threadInitFunction, function<bool(_TestFixture*, _TestFixture*)> sortFunction)
{
    const set<string> include, exclude;
    const list<string> before, after;
...
int tpunit::_TestFixture::tpunit_detail_do_run(const set<string>& include, const set<string>& exclude,
                                               ...
                                               function<void()> threadInitFunction, function<bool(_TestFixture*, _TestFixture*)> sortFunction)
{
    threadInitFunction();
     ...
     */
    list<_TestFixture*> testFixtureList = *tpunit_detail_fixture_list();
            ...
            try {
                ...
                while (1) {
                    ...
                    if (currentTestName.size() && currentTestName != "UNSPECIFIED") {
                        ...
                        testTimes.emplace(make_pair(chrono::duration_cast<chrono::milliseconds>(end - start), currentTestName));
                    }
                }
...
void tpunit::_TestFixture::tpunit_detail_do_tests(_TestFixture* f)
{
    ...
    while (t) {
        testThreads.push_back(thread([t, f]() {
            ...
            timeStream << "(" << chrono::duration_cast<chrono::milliseconds>(end - start);
            if (chrono::duration_cast<chrono::milliseconds>(end - start) > 5000ms) {
...
int tpunit::Tests::run(int threads, function<void()> threadInitFunction, function<bool(_TestFixture*, _TestFixture*)> sortFunction)
{
...
int tpunit::Tests::run(const set<string>& include, const set<string>& exclude,
                       const list<string>& before, const list<string>& after, int threads, function<void()> threadInitFunction, function<bool(_TestFixture*, _TestFixture*)> sortFunction)
{
