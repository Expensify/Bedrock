...
// Convenience macro for iterating over a priorty queue that is map of priority -> ordered list. Abstracts away map and list iteration into one loop.
#define SFOREACHPRIORITYQUEUE( _CT_, _C_, _I_ )       for(map< int, list< _CT_ > >::reverse_iterator i = (_C_).rbegin( ); i != (_C_).rend( ); ++i ) for( list< _CT_ >::iterator _I_ = (i)->second.begin( ); _I_ != (i)->second.end( ); ++_I_)
...
#define SQL_NODE_SYNCHRONIZING_RECV_TIMEOUT  STIME_US_PER_S*60  // Seperate timeout for receiving and applying synchronization commits
                                                                // Increase me during a rekey if you need larger commits to have more time

const char* SQLCStateNames[] =
{
    "SEARCHING",
    "SYNCHRONIZING",
    "WAITING",
...
const char* SQLCConsistencyLevelNames[] =
{
    "ASYNC",
    "ONE",
    "QUORUM"
...
SQLiteNode::SQLiteNode( const string& filename, 
                        ...
                        bool          readOnly,
                        int           maxJournalSize )
    : STCPNode( name, host, max( SQL_NODE_DEFAULT_RECV_TIMEOUT, SQL_NODE_SYNCHRONIZING_RECV_TIMEOUT ) ), _db( filename, cacheSize, autoCheckpoint, readOnly, maxJournalSize )
{
...
bool SQLiteNode::shutdownComplete( )
{
    ...
    if( _state > SQLC_WAITING )
    {
        ...
        SINFO( "Can't graceful shutdown yet because state=" << SQLCStateNames[_state] << ", queued=" << getQueuedCommandList().size() << ", escalated=" << _escalatedCommandMap.size() << ", processed=" << _processedCommandList.size() );
        return false;
    }
    ...
    {
        // Not done yet
        SINFO( "Can't graceful shutdown yet because waiting on commands: queued=" << getQueuedCommandList().size() << ", escalated=" << _escalatedCommandMap.size() << ", processed=" << _processedCommandList.size() );
        return false;
    }
}

// --------------------------------------------------------------------------
SQLiteNode::Command* SQLiteNode::openCommand( const SData& request, int priority, bool unique, int64_t commandExecuteTime )
{
...
SASSERT( priority <= SPRIORITY_MAX ); // Else will trump UpgradeDatabase
    ...
    if( commandExecuteTime > now )
    {
        ...
        SINFO( "Scheduling command " << command->id << " for " << (((int64_t)command->creationTimestamp-now)/STIME_US_PER_S) << "s in the future." );
    }
    ...
    bool peekIt = (_state==SQLC_MASTERING || _state==SQLC_SLAVING) && (int64_t)command->creationTimestamp <= now && nodeUpToDate;
    ...
    if( SIEquals( request.methodLine, "Status" ) )
    {
        ...
        peekIt      = !_readOnly;
             ...
             SIEquals( request.methodLine, "GET /status/isSlave HTTP/1.1" ) ||
    ...
    if( peekIt )
    {
        ...
        {
            // Skip
            SHMMM( "Skipping slave peek because our version (" << _version << ") doesn't match master (" << getMasterVersion( ) << ")." );
            peekIt = false;
        }
        ...
        if( _state == SQLC_MASTERING )
        {
            ...
            {
                // Skip
                SHMMM( "Skipping master peek because only " << numFullSlaves << " of " << numFullPeers << " full peers (" << peerList.size() << " with permaslaves) subscribed so remote commit isn't possible." );
                peekIt = false;
            }
        }
    }
    ...
    {
        // Queue it.
        SINFO( "Queuing new non-peekable command '" << request.methodLine << "' (" << id << "), priority=" << priority << (!nodeUpToDate ? " because node out of date." : "") );
...
SQLiteNode::Command* SQLiteNode::getProcessedCommand( )
{
    ...
    if( _processedCommandList.empty() ) return 0;
...
void SQLiteNode::clearCommandHolds( const string& heldBy )
{
    ...
    SFOREACHPRIORITYQUEUE( Command*, _queuedCommandMap, commandIt )
    {
        ...
        if( !commandHeldBy.empty() )
        {
            ...
            {
                // Release this one
                SINFO( "Releasing command '" << command->request.methodLine << "' (#" << command->id << ") held by '" << commandHeldBy << "'" );
                command->request["HeldBy"].clear();
            }
            else
            {
                // Continue holding
                SINFO( "Continuing to hold '" << command->request.methodLine << "' (#" << command->id << ") held by '" << commandHeldBy << "'" );
            }
        }
    }
}
...
SASSERT( command );
    ...
    list<Command*>::iterator commandIt = ::find( _queuedCommandMap[command->priority].begin( ), _queuedCommandMap[command->priority].end( ), command );
    ...
    if( _currentCommand == command )
    {
        ...
        SWARN( "Closing outstanding command '" << command->request.methodLine << "' (" << command->id << ") after " << elapsed/STIME_US_PER_MS << "ms, aborting." );
        _abortCommand( _db, command );
        if( _db.insideTransaction() ) _db.rollback();
        _currentCommand = 0;
    }
...
void SQLiteNode::_queueCommand( Command* command )
{
    ...
    else if( command->request.isSet( "writeConsistency" ) )
    {
        ...
        int wc = SStateNameToInt( SQLCConsistencyLevelNames, command->request["writeConsistency"], SQLC_NUM_CONSISTENCY_LEVELS );
        if( wc >=0 ) command->writeConsistency = (SQLCConsistencyLevel)wc;
        else         SWARN( "Unknown write consistency supplied '" << command->request["writeConsistency"] << "'. Ignoring." );
        SINFO( "'" << command->request.methodLine << "' has overridden default consistency with '" << command->request["writeConsistency"] << "' (" << SQLCConsistencyLevelNames[command->writeConsistency] << ")" );
    }
    ...
    if( _queuedCommandMap[command->priority].empty( ) || command->creationTimestamp > _queuedCommandMap[command->priority].back( )->creationTimestamp )
...
void SQLiteNode::_escalateCommand( Command* command )
{
    ...
    SINFO( "Escalating '" << command->request.methodLine << "' (" << command->id << ") to MASTER '" << _masterPeer->name << "' after " << elapsed/STIME_US_PER_MS << " ms" ); 
...
SASSERT( command );
    ...
    //          started.
    uint64_t elapsed = STimeNow() - command->creationTimestamp;
    uint64_t httpElapsed = ( command->httpsRequest ? 
        command->httpsRequest->finished - command->httpsRequest->created :
        0
    );
    uint64_t replicationElapsed = ( command->replicationStartTimestamp ? 
        STimeNow( ) - command->replicationStartTimestamp :
        0
    );
    SINFO( "Finishing command '" << command->request.methodLine << "'(" << command->id << ")->'" << command->response.methodLine << "' after " << elapsed/STIME_US_PER_MS << "ms (" << httpElapsed/STIME_US_PER_MS << "ms in http) (" << replicationElapsed / STIME_US_PER_MS << " ms in replication)" );
    ...
    if( command->initiator )
    {
        ...
        {
            SINFO( "Responding to escalation for transaction '" << command->request.methodLine << "' (" << command->id << ") after " << elapsed/STIME_US_PER_MS << " ms" );
        }
        else
        {
            SWARN( "Responding to escalation for transaction '" << command->request.methodLine << "' (" << command->id << ") after " << elapsed/STIME_US_PER_MS << " ms (slow)" );
        }
...
bool SQLiteNode::update( uint64_t& nextActivity )
{
    ...
    switch( _state )
    {
        ...
        SASSERTWARN(  _db.getUncommittedHash().empty() );
            ...
            if( shutdownComplete() ) return false; // Don't re-update
            ...
            SFOREACH( list<Peer*>, peerList, peerIt )
            {
                ...
                if (loggedIn) {
                    ...
                    if (peerIsSlaving && peerLatencyKnown && peerIsFresherThanUs && (!nearestSlave || peer->latency < nearestSlave->latency)) {
            ...
            SINFO( "Signed in to " << numLoggedInFullPeers << " of " << numFullPeers << " full peers (" << peerList.size() << " with permaslaves), timeout in " << (_stateTimeout - STimeNow())/STIME_US_PER_MS << "ms" );
            if( ((float)numLoggedInFullPeers < numFullPeers/2.0) && (STimeNow() < _stateTimeout) )
                return false;
            
            // We've given up searching; did we time out?
            if( STimeNow() >= _stateTimeout ) SHMMM( "Timeout SEARCHING for peers, continuing." );
            ...
            if (nearestSlave) {
                ...
                SINFO( "SYNCHRONIZING from nearest slave '" << nearestSlave->name << " (" << nearestSlave->latency/STIME_US_PER_MS << "ms latency)" );
                ...
                SINFO( "SYNCHRONIZING from freshest peer '" << freshestPeer->name << " (" << freshestPeer->latency/STIME_US_PER_MS << "ms latency)" );
        ...
        SASSERTWARN(  _escalatedCommandMap.empty() );
            ...
            SFOREACH( list<Peer*>, peerList, peerIt )
            {
                ...
                if( peer->params["Permaslave"] != "true" ) {
                    ...
                    if( SIEquals( (*peer)["LoggedIn"], "true" ) )
                    {
                        ...
                        if( SIEquals( peerState, "STANDINGUP" ) ||
                            SIEquals( peerState, "MASTERING"  ) ||
                            SIEquals( peerState, "STANDINGDOWN" ) )
                        {
                            // Found the current master
                            if( currentMaster ) PHMMM( "Multiple peers trying to stand up (also '" << currentMaster->name << "'), let's hope they sort it out." );
                            currentMaster = peer;
                        }
                    }
                }
            ...
            // master.
            if( !currentMaster && numLoggedInFullPeers*2 >= numFullPeers && _priority > highestPriorityPeer->calc("Priority") )
            {
            ...
            // Keep waiting
            SDEBUG( "Connected to " << numLoggedInFullPeers << " of " << numFullPeers << " full peers (" << peerList.size() << " with permaslaves), priority=" << _priority );
            break;
        }
        ...
        SASSERTWARN( !_masterPeer );
            ...
            if( _currentCommand && !_currentCommand->transaction.empty() )
            {
                ...
                // checkpoint limit.
                SQLCConsistencyLevel consistencyRequired = (_commitsSinceCheckpoint >= _quorumCheckpoint) ? SQLC_QUORUM : _currentCommand->writeConsistency;
                ...
                SDEBUG( "'" << _currentCommand->request.methodLine << "' (#" << _currentCommand->id << "): "
                    ...
                    "writeConsistency=" << SQLCConsistencyLevelNames[_currentCommand->writeConsistency] << ", "
                    ...
                    "initiatorSubscribed=" << initiatorSubscribed << ", "
                ...
                if( !initiatorSubscribed || (everybodyResponded && !consistentEnough) )
                {
                    ...
                    SWARN( "Rolling back transaction for '" << _currentCommand->request.methodLine << "' because initiatorSubscribed=" << initiatorSubscribed << ", everybodyResponded=" << everybodyResponded << ", consistentEnough=" << consistentEnough );
                    _db.rollback( );
                ...
                else if( consistentEnough )
                {
                    ...
                    uint64_t totalElapsed = _db.getLastTransactionTiming(
                        beginElapsed,
                        readElapsed,
                        writeElapsed,
                        prepareElapsed,
                    ...
                    SINFO("Committed master transaction for '" << _currentCommand->request.methodLine << 
                        "' (" << _currentCommand->id << ") "
                        "#" << _db.getCommitCount()+1 << 
                        " (" << _db.getUncommittedHash() << "). " << 
                        _commitsSinceCheckpoint << " commits since quorum "
                        "(consistencyRequired=" << SQLCConsistencyLevelNames[consistencyRequired] << "), " << 
                        numFullApproved << " of " << 
                        numFullPeers << " approved "
                        "(" << peerList.size() << " total) in " << 
                        totalElapsed   /STIME_US_PER_MS << " ms (" << 
                        beginElapsed   /STIME_US_PER_MS << "+" << 
                        readElapsed    /STIME_US_PER_MS << "+" << 
                        writeElapsed   /STIME_US_PER_MS << "+" << 
                        prepareElapsed /STIME_US_PER_MS << "+" << 
                        commitElapsed  /STIME_US_PER_MS << "+" << 
                ...
                {
                    // Still waiting
                    SINFO( "Waiting to commit: '" << _currentCommand->request.methodLine << "' (#" << _currentCommand->id << "): consistencyRequired=" << SQLCConsistencyLevelNames[consistencyRequired] << ", commitsSinceCheckpoint=" << _commitsSinceCheckpoint );
                }
            }
                    ...
                    SFOREACH( list<Peer*>, peerList, peerIt )
                    {
                        ...
                        else if( SIEquals( (*peer)["State"], "WAITING" ) )
                        {
                            ...
                            if( peer->calc("Priority") > _priority )
                            {
                                ...
                                standDownReason = "Found higher priority WAITING peer (" + peer->name + ") while MASTERING, STANDINGDOWN";
                            }
                            else if( peer->calcU64("CommitCount") > _db.getCommitCount() )
                            {
                                // It's got data that we don't, stand down so we can get it.
                                standDownReason = "Found WAITING peer (" + peer->name + ") with more data than us (we have " + SToStr( _db.getCommitCount() ) + "/" + _db.getCommittedHash() + ", it has " + (*peer)["CommitCount"] + "/" + (*peer)["Hash"] + ") while MASTERING, STANDINGDOWN";
                            }
                        }
                    }
                }
                        ...
                        if( !commandList->empty() )
                        {
                            ...
                            while( nextIt != commandList->end() )
                            {
                                ...
                                if( !command->request["HeldBy"].empty() )
                                {
                                    ...
                                    uint64_t elapsed = STimeNow() - command->creationTimestamp;
                                    if( command->request.isSet("Timeout") && elapsed > command->request.calc64("Timeout")*STIME_US_PER_MS )
                                    {
                                        // Command timed out, return the result
                                        SINFO( "Command '" << command->id << "' timed out after " << elapsed/STIME_US_PER_MS << "ms (" << command->request["Timeout"] << "ms configured): " << command->request.methodLine );
                                ...
                                if( !command->httpsRequest )
                                {
                                    ...
                                    if( _peekCommandWrapper( _db, command ) )
                                    {
                                        ...
                                        SINFO( "Finished processing peekable command '" << command->request.methodLine << "' (" << command->id << "), nothing to commit." );
                                    ...
                                    if( !command->request["HeldBy"].empty() )
                                    {
                                        ...
                                        SINFO( "Hold re-placed on command by '" << command->request["HeldBy"] << "', skipping." );
                                        continue;
                                    }
                                ...
                                SINFO( "Starting processing command '" << _currentCommand->request.methodLine << "' (" << _currentCommand->id << ")" );
                                ...
                                if( _db.insideTransaction() ) 
                                {
                                    ...
                                    SINFO( "Finished processing command '" << _currentCommand->request.methodLine << "' (" << _currentCommand->id << "), beginning distributed transaction for commit #" << _db.getCommitCount() + 1 << " (" << _db.getUncommittedHash() << ")" );
                                ...
                                else
                                {
                                    ...
                                    SINFO( "Finished processing command '" << _currentCommand->request.methodLine << "' (" << _currentCommand->id << "), nothing to commit." );
                                    SASSERT( !_db.insideTransaction() );
                                    _finishCommand( _currentCommand );
                                    _currentCommand = 0;
                                }
            ...
            SFOREACHMAPREVERSE( int, list<Command*>, _queuedCommandMap, it )
            {
                ...
                while( !commandList->empty() )
                {
                    ...
                    if( command->httpsRequest )
                    {
                        ...
                        SHMMM( "Escalated command '" << command->request.methodLine << "' (" << command->id << ") has secondary request; cancelling and new MASTER will re-submit." );
                        _cleanCommand( command );
                    }
                    commandList->pop_front();
                    _escalateCommand( command );
        ...
        default: SERROR( "Invalid state #" << _state );
    }
...
SASSERTWARN( !message.empty() );
    ...
    if( !message.isSet("CommitCount") ) throw "missing CommitCount";
    if( !message.isSet("Hash") )        throw "missing Hash";
    ...
    if( SIEquals( message.methodLine, "LOGIN" ) )
    {
        ...
        /// 
        if( (*peer)["LoggedIn"] == "true" ) throw "already logged in";
        if( !message.isSet("Priority") )    throw "missing Priority";
        if( !message.isSet("State"   ) )    throw "missing State";
        if( !message.isSet("Version" ) )    throw "missing Version";
        ...
        SASSERT( !_priority || message.calc("Priority") != _priority ); // to prevent misconfig: only one at each priority, except 0
        PINFO( "Peer logged in at '" << message["State"] << "', priority #" << message["Priority"] << " commit #" << message["CommitCount"] << " (" << message["Hash"] << ")" );
        (*peer)["Priority"]    = message["Priority"];
        (*peer)["State"]       = message["State"];
        (*peer)["LoggedIn"]    = "true";
        (*peer)["Version"]     = message["Version"];
    }
    else if( !SIEquals( (*peer)["LoggedIn"], "true" ) ) throw "not logged in";
    else if( SIEquals( message.methodLine, "STATE" ) )
    {
        ...
        if( oldState == newState )
        {
            ...
            PINFO( "Peer received new commit in state '" << oldState << "', commit #" << message["CommitCount"] << " (" << message["Hash"] << ")" );
        }
        else
        {
            // State changed -- first see if it's doing anything unusual
            PINFO( "Peer switched from '" << oldState << "' to '" << newState << "' commit #" << message["CommitCount"] << " (" << message["Hash"] << ")" );
            int from=0, to=0;
            for( from=SQLC_SEARCHING; from<=SQLC_SLAVING; from++ ) if( SIEquals( oldState, SQLCStateNames[from] ) ) break;
            for( to  =SQLC_SEARCHING;   to<=SQLC_SLAVING;   to++ ) if( SIEquals( newState, SQLCStateNames[to]   ) ) break;
            if( from > SQLC_SLAVING ) PWARN( "Peer coming from unrecognized state '" << oldState << "'" );
            if( to   > SQLC_SLAVING ) PWARN( "Peer going to unrecognized state '"    << newState << "'" );
            bool okTransition = false;
            switch( from )
            {
                case SQLC_SEARCHING:      okTransition = ( to==SQLC_SYNCHRONIZING || to==SQLC_WAITING || to==SQLC_MASTERING ); break;
                case SQLC_SYNCHRONIZING : okTransition = ( to==SQLC_SEARCHING || to==SQLC_WAITING ); break;
                case SQLC_WAITING:        okTransition = ( to==SQLC_SEARCHING || to==SQLC_STANDINGUP || to==SQLC_SUBSCRIBING ); break;
                case SQLC_STANDINGUP:     okTransition = ( to==SQLC_SEARCHING || to==SQLC_MASTERING ); break;
                case SQLC_MASTERING:      okTransition = ( to==SQLC_SEARCHING || to==SQLC_STANDINGDOWN ); break;
                case SQLC_STANDINGDOWN:   okTransition = ( to==SQLC_SEARCHING ); break;
                case SQLC_SUBSCRIBING:    okTransition = ( to==SQLC_SEARCHING || to==SQLC_SLAVING ); break;
                case SQLC_SLAVING:        okTransition = ( to==SQLC_SEARCHING ); break;
            }
            if( !okTransition ) PWARN( "Peer making invalid transition from '" << SQLCStateNames[from] << "' to '" << SQLCStateNames[to] << "'" );
            ...
            else if( to == SQLC_STANDINGUP )
            {
                ...
                if( SWITHIN( SQLC_STANDINGUP, _state, SQLC_STANDINGDOWN ) )
                {
                    ...
                    if( peer->calc("Priority") > _priority )
                    {
                        ...
                        PWARN( "Higher-priority peer is trying to stand up while we are " << SQLCStateNames[_state] << ", reconnecting and SEARCHING." );
                        _reconnectAll( );
                        _changeState( SQLC_SEARCHING );
                    }
                    else
                    {
                        ...
                        // Hmm, why is a lower priority peer trying to stand up?  Is it possble we're not longer in control
                        ...
                        if( _majoritySubscribed( ) )
                        {
                            ...
                            PHMMM( "Lower-priority peer is trying to stand up while we are " << SQLCStateNames[_state] << " with a majority of the cluster; denying and ignoring." );
                        }
                        else
                        {
                            ...
                            PWARN( "Lower-priority peer is trying to stand up while we are " << SQLCStateNames[_state] << ", but we don't have a majority of the cluster so reconnecting and SEARCHING." );
                            _reconnectAll( );
                            _changeState( SQLC_SEARCHING );
                        }
                    }
                ...
                else
                {
                    ...
                    SFOREACH( list<Peer*>, peerList, otherPeerIt )
                        if( *otherPeerIt != peer )
                        {
                            ...
                            if( SIEquals( state, "STANDINGUP" ) ||
                                SIEquals( state, "MASTERING" )  ||
                ...
                // Send the response
                if( SIEquals( response["Response"], "approve" ) ) PINFO( "Approving standup request" ); 
                else                                              PHMMM( "Denying standup request because " << response["Reason"] );
            ...
            else if( from == SQLC_STANDINGDOWN )
            {
                ...
                if( !_db.getUncommittedHash().empty() )
                {
                    ...
                    PWARN( "Was expecting a response for transaction #" << _db.getCommitCount()+1 << " (" << _db.getUncommittedHash() << ") but stood down prematurely, rolling back and hoping for the best." );
                    _db.rollback();
                }
            }
        }
    ...
    else if( SIEquals( message.methodLine, "STANDUP_RESPONSE" ) )
    {
        ...
        /// 
        if( _state != SQLC_STANDINGUP ) throw "not standing up";
        if( !message.isSet("Response") ) throw "missing Response";
        if( peer->isSet("StandupResponse") ) PWARN( "Already received standup response '" << (*peer)["StandupResponse"] << "', now receiving '" << message["Response"] << "', odd -- multiple masters competing?" );
    ...
    else if( SIEquals( message.methodLine, "SYNCHRONIZE_RESPONSE" ) )
    {
        ...
        /// 
        if(  _state != SQLC_SYNCHRONIZING ) throw "not synchronizing";
        if( !_syncPeer )                    throw "too late, gave up on you";
        if(   peer != _syncPeer )           throw "sync peer mismatch";
        ...
        try
        {
            ...
            {
                // All done
                SINFO( "Synchronization complete, at commitCount #" << _db.getCommitCount() << " (" << _db.getCommittedHash() << "), WAITING" );
                _syncPeer = 0;
                _changeState( SQLC_WAITING );
            }
            else if( _db.getCommitCount() > peerCommitCount )
            {
                ...
                SWARN( "We have more data (" << _db.getCommitCount() << ") than our sync peer '" << _syncPeer->name << "' (" << peerCommitCount << "), reconnecting and SEARCHING." );
                _reconnectPeer( _syncPeer );
                _syncPeer = 0;
                _changeState( SQLC_SEARCHING );
            }
            else
            {
                ...
                SINFO( "Synchronization underway, at commitCount #" << _db.getCommitCount() << " (" << _db.getCommittedHash() << "), " << peerCommitCount-_db.getCommitCount() << " to go." );
                _sendToPeer( _syncPeer, SData("SYNCHRONIZE") );

                // Also, extend our timeout so long as we're still alive
               _stateTimeout = STimeNow() + 
                               SQL_NODE_SYNCHRONIZING_RECV_TIMEOUT + 
                               SRand64()%STIME_US_PER_M*5;
            }
        }
        catch( const char* e )
        {
    ...
    else if( SIEquals( message.methodLine, "SUBSCRIBE" ) )
    {
        ...
        ///     to this new slave.
        /// 
        if( _state != SQLC_MASTERING ) throw "not mastering";
        ...
        if( _currentCommand )
        {
            ...
            SINFO( "Inviting peer into distributed transaction already underway '" << _currentCommand->request.methodLine << "' #" << _db.getCommitCount() + 1 << " (" << _db.getUncommittedHash() << ")" );
    ...
    else if( SIEquals( message.methodLine, "SUBSCRIPTION_APPROVED" ) )
    {
        ...
        /// 
        if( _state != SQLC_SUBSCRIBING ) throw "not subscribing";
        if( _masterPeer != peer )        throw "not subscribing to you";
        ...
        try
        {
            ...
            SINFO( "Subscription complete, at commitCount #" << _db.getCommitCount() << " (" << _db.getCommittedHash() << "), SLAVING" );
    ...
    else if( SIEquals( message.methodLine, "BEGIN_TRANSACTION" ) )
    {
        ...
        /// 
        if( !message.isSet("ID") )                               throw "missing ID";
        if( !message.isSet("NewCount") )                         throw "missing NewCount";
        if( !message.isSet("NewHash") )                          throw "missing NewHash";
        if( _state != SQLC_SLAVING )                             throw "not slaving";
        if( !_masterPeer )                                       throw "no master?";
        if( !_db.getUncommittedHash().empty() )                  throw "already in a transaction";
        if( _db.getCommitCount()+1 != message.calcU64("NewCount") ) throw "commit count mismatch";
        if( !_db.beginTransaction( ) )                           throw "failed to begin transaction";
        try
        {   
            // Inside transaction; get ready to back out on error
            if( !_db.write( message.content ) ) throw "failed to write transaction";
            if( !_db.prepare( ) )               throw "failed to prepare transaction";
        }
        catch( const char* e )
        {
        ...
        if( _db.getUncommittedHash() != message["NewHash"] )
        {
            ...
            PWARN( "New hash mismatch: command='" << _currentTransactionCommand << "', commitCount=#" << _db.getCommitCount() << "', committedHash='" << _db.getCommittedHash() << "', uncommittedHash='" << _db.getUncommittedHash() << "', uncommittedQuery='" << _db.getUncommittedQuery() << "'" );
        ...
        if( _priority ) {
            ...
            PINFO( "Approving transaction #" << _db.getCommitCount()+1 << " (" << _db.getUncommittedHash() << ") for command '" << _currentTransactionCommand << "'" );
            ...
            PINFO( "Would approve transaction #" << _db.getCommitCount()+1 << " (" << _db.getUncommittedHash() << ") for command '" << _currentTransactionCommand << "', but a permaslave -- keeping quiet." ); 
        }
    ...
    else if( SIEquals( message.methodLine, "APPROVE_TRANSACTION" ) )
    {
        ...
        if( !message.isSet("NewCount") )                              throw "missing NewCount";
        if( !message.isSet("NewHash") )                               throw "missing NewHash";
        if( _state != SQLC_MASTERING && _state != SQLC_STANDINGDOWN ) throw "not mastering";
        try
        {
            ...
            if( _currentCommand && _currentCommand->id == message["ID"] )
            {
                ...
                PINFO( "Peer approved transaction #" << message["NewCount"] << " (" << message["NewHash"] << ") after " << replicationElapsed / STIME_US_PER_MS << "ms" );
                (*peer)["TransactionResponse"] = "approve";
            }
            else
                // Old command.  Nothing to do.  We already sent a commit or rollback.
                PINFO( "Peer approved transaction #" << message["NewCount"] << " (" << message["NewHash"] << ") after commit." );
        }
        catch( const char* e )
        {
            ...
            SWARN( "Received APPROVE_TRANSACTION for transaction #" << message.calc("NewCount") << " (" << message["NewHash"] << ", " << message["ID"] << ") but '" << e << "', ignoring." );
        }
    }
    else if( SIEquals( message.methodLine, "COMMIT_TRANSACTION" ) )
    {
        ...
        /// 
        if( _state != SQLC_SLAVING )                                throw "not slaving";
        if( _db.getUncommittedHash().empty() )                      throw "no outstanding transaction";
        if( message.calcU64("CommitCount") != _db.getCommitCount()+1 ) throw "commit count mismatch";
        if( message["Hash"] != _db.getUncommittedHash() )           throw "hash mismatch";
        _db.commit();
        uint64_t beginElapsed, readElapsed, writeElapsed, prepareElapsed, commitElapsed, rollbackElapsed;
        uint64_t totalElapsed = _db.getLastTransactionTiming(
            beginElapsed,
        ...
        SINFO("Committed slave transaction #" << message["CommitCount"] << 
            " (" << message["Hash"] << ") for " 
            "'" << _currentTransactionCommand << "' in " << 
            totalElapsed   /STIME_US_PER_MS << " ms (" << 
            beginElapsed   /STIME_US_PER_MS << "+" << 
            readElapsed    /STIME_US_PER_MS << "+" << 
            writeElapsed   /STIME_US_PER_MS << "+" << 
    ...
    else if( SIEquals( message.methodLine, "ESCALATE" ) )
    {
        ...
        else
        {
            ...
            SData request;
            if( !request.deserialize(message.content) ) throw "malformed request";
            if( (*peer)["Subscribed"] != "true" )       throw "not subscribed";
            if( !message.isSet("ID") )                  throw "missing ID";
    ...
    else if( SIEquals( message.methodLine, "ESCALATE_RESPONSE" ) )
    {
        ...
        if( !message.isSet("ID") )                     throw "missing ID";
        SData response;
        if( !response.deserialize( message.content ) ) throw "malformed contet";
    ...
    else if( SIEquals( message.methodLine, "ESCALATE_ABORTED" ) )
    {
        ...
        if( commandIt != _escalatedCommandMap.end() )
        {
            // Re-queue this
            Command* command = commandIt->second;
            PINFO( "Re-queueing command '" << message["ID"] << "' (" << command->request.methodLine << ") (" << command->id << ")" );
            _queuedCommandMap[command->priority].push_front( command );
    ...
    else throw "unrecognized message";
}
...
SASSERT( peer );
    ...
    if( peer == _masterPeer )
    {
        ...
        if( !_db.getUncommittedHash().empty() )
        {
            ...
            PHMMM( "Was expecting a response for transaction #" << _db.getCommitCount()+1 << " (" << _db.getUncommittedHash() << ") but disconnected prematurely; rolling back." );
            _db.rollback();
        }
        ...
        while( !escalatedCommandList.empty() )
        {
            ...
            else
            {
                ...
                PWARN( "Aborting escalated command '" << command->request.methodLine << "' (" << command->id << ") in transaction state '" << command->transaction.methodLine << "'" );
                _abortCommand( _db, command );
                _processedCommandList.push_back( command );
            }
            escalatedCommandList.pop_back( );
    ...
    SFOREACHMAP( int, list<Command*>, _queuedCommandMap, it )
    {
        ...
        while( nextCommandIt != commandList->end() )
        {
            ...
            {
                // Drop this command
                PHMMM( "Queued command initiator disconnected, dropping '" << command->request.methodLine << "' (" << command->id << ")" );
    ...
    if( _currentCommand && _currentCommand->initiator == peer )
    {
        ...
        PHMMM( "Current command initiator disconnected, aborting '" << _currentCommand->request.methodLine << "' (" << _currentCommand->id << ") and rolling back." );
        SASSERTWARN( _state == SQLC_MASTERING || _state == SQLC_STANDINGDOWN );
        SASSERTWARN( SIEquals( (*peer)["State"], "SLAVING" ) );
...
SASSERT( peer );
    ...
    if( peerCommitCount > _db.getCommitCount() ) throw "you have more data than me";
    if( peerCommitCount )
    {
        // It has some data -- do we agree on what we share?
        string myHash, ignore;
        if( !_db.getCommit( peerCommitCount, ignore, myHash ) ) throw "error getting hash";
        if( myHash != (*peer)["Hash"] )                         throw "hash mismatch";
        PINFO( "Latest commit hash matches our records, beginning synchronization." );
    }
    else PINFO( "Peer has no commits, beginning synchronization." );
    ...
    else 
    {
        ...
        if( !sendAll ) toIndex = SMin( toIndex, fromIndex+100 ); // 100 transactions at a time
        if( !_db.getCommits( fromIndex, toIndex, result ) ) throw "error getting commits";
        if( (uint64_t)result.size() != toIndex-fromIndex+1 )     throw "mismatched commit count";
...
SASSERT( peer );
    ...
    if(  !message.isSet("NumCommits") ) throw "missing NumCommits";
    ...
    while( (messageSize = commit.deserialize( content, remaining )) )
    {
        ...
        if( !SIEquals( commit.methodLine, "COMMIT" ) )             throw "expecting COMMIT";
        if( !commit.isSet("CommitIndex") )                         throw "missing CommitIndex";
        if( commit.calc64("CommitIndex") < 0 )                     throw "invalid CommitIndex";
        if( !commit.isSet("Hash") )                                throw "missing Hash";
        if( commit.content.empty() )                               throw "missing content";
        if( commit.calcU64("CommitIndex") != _db.getCommitCount()+1 ) throw "commit index mismatch";
        if( !_db.beginTransaction( ) )                             throw "failed to begin transaction";
        try
        {
            // Inside a transaction; get ready to back out if an error 
            if( !_db.write( commit.content ) ) throw "failed to write transaction";
            if( !_db.prepare( ) )              throw "failed to prepare transaction";
        }
        catch( const char* e )
        {
        ...
        _db.commit( );
        if( _db.getCommittedHash() != commit["Hash"] ) throw "potential hash mismatch";
        --commitsRemaining;
    }

    // Did we get all our commits?
    if( commitsRemaining ) throw "commits remaining at end";
}
