...
#define SFOREACHPRIORITYQUEUE(_CT_, _C_, _I_)                                  \
  for (map<int, list<_CT_>>::reverse_iterator i = (_C_).rbegin();              \
       i != (_C_).rend(); ++i)                                                 \
    for (list<_CT_>::iterator _I_ = (i)->second.begin();                       \
         _I_ != (i)->second.end(); ++_I_)

// We've bumped these values back up to 5 minutes because some of the billing
// commands take over 1 minute to process.
#define SQL_NODE_DEFAULT_RECV_TIMEOUT                                          \
  STIME_US_PER_M * 5 // Receive timeout for 'normal' SQLiteNode messages
#define SQL_NODE_SYNCHRONIZING_RECV_TIMEOUT                                    \
  STIME_US_PER_S * 60 // Seperate timeout for receiving and applying
                      // synchronization commits
// Increase me during a rekey if you need larger commits to have more time

const char *SQLCStateNames[] = {"SEARCHING",   "SYNCHRONIZING", "WAITING",
                                "STANDINGUP",  "MASTERING",     "STANDINGDOWN",
                                "SUBSCRIBING", "SLAVING"};
const char *SQLCConsistencyLevelNames[] = {"ASYNC", "ONE", "QUORUM"};

// --------------------------------------------------------------------------
SQLiteNode::SQLiteNode(const string &filename, const string &name,
                       const string &host, int priority, int cacheSize,
                       int autoCheckpoint, uint64_t firstTimeout,
                       const string &version, int quorumCheckpoint,
                       const string &synchronousCommands, bool readOnly,
                       int maxJournalSize)
    : STCPNode(name, host, max(SQL_NODE_DEFAULT_RECV_TIMEOUT,
...
void SQLiteNode::beginShutdown() {
  ...
  if (!gracefulShutdown()) {
    ...
    _gracefulShutdownTimeout.alarmDuration =
        STIME_US_PER_S * 30; // 30s timeout before we give up
...
bool SQLiteNode::shutdownComplete() {
  ...
  if (_state > SQLC_WAITING) {
    ...
    SINFO("Can't graceful shutdown yet because state="
          ...
          << ", queued=" << getQueuedCommandList().size()
  ...
  if ((*peerIt)->s && !(*peerIt)->s->sendBuffer.empty()) {
    // Still sending data
    SINFO("Can't graceful shutdown yet because unsent data to peer '"
          << (*peerIt)->name << "'");
    return false;
  }
...
// --------------------------------------------------------------------------
SQLiteNode::Command *SQLiteNode::openCommand(const SData &request, int priority,
                                             bool unique,
  ...
  //
  bool nodeUpToDate = request["commitCount"].empty() ||
                      request.calcU64("commitCount") <= getCommitCount();
  bool peekIt = (_state == SQLC_MASTERING || _state == SQLC_SLAVING) &&
                (int64_t)command->creationTimestamp <= now && nodeUpToDate;
  ...
  if (peekIt) {
    ...
    if (_state == SQLC_SLAVING && _version != getMasterVersion()) {
      // Skip
      SHMMM("Skipping slave peek because our version ("
            << _version << ") doesn't match master (" << getMasterVersion()
            << ").");
      peekIt = false;
    }
    ...
    if (_state == SQLC_MASTERING) {
      ...
      if (!_majoritySubscribed(numFullPeers, numFullSlaves)) {
        ...
        SHMMM("Skipping master peek because only "
              << numFullSlaves << " of " << numFullPeers << " full peers ("
              << peerList.size() << " with permaslaves) subscribed so remote "
  ...
  if ((peekIt || forcePeekIt) && _peekCommandWrapper(_db, command)) {
    ...
    // Queue it.
    SINFO("Queuing new non-peekable command '"
          << request.methodLine << "' (" << id << "), priority=" << priority
...
SQLiteNode::Command *SQLiteNode::getQueuedCommand(int priority) {
  ...
  SINFO("Returning queued command '" << front->request.methodLine << "' ("
                                     << front->id << ")");
  return front;
}
...
SQLiteNode::Command *SQLiteNode::getProcessedCommand() {
  ...
  SINFO("Returning processed command '" << front->request.methodLine << "' ("
                                        << front->id << ")");
  return front;
}

// --------------------------------------------------------------------------
SQLiteNode::Command *SQLiteNode::findCommand(const string &name,
                                             const string &value) {
...
void SQLiteNode::clearCommandHolds(const string &heldBy) {
  ...
  SFOREACHPRIORITYQUEUE(Command *, _queuedCommandMap, commandIt) {
    ...
    if (!commandHeldBy.empty()) {
      ...
      if (SREMatch(SReplace(SToLower(commandHeldBy), "%", ".*"),
                   SToLower(heldBy))) {
        // Release this one
        SINFO("Releasing command '" << command->request.methodLine << "' (#"
                                    << command->id << ") held by '"
                                    << commandHeldBy << "'");
        command->request["HeldBy"].clear();
      } else {
        // Continue holding
        SINFO("Continuing to hold '" << command->request.methodLine << "' (#"
                                     << command->id << ") held by '"
                                     << commandHeldBy << "'");
      }
    }
...
void SQLiteNode::closeCommand(Command *command) {
  ...
  list<Command *>::iterator commandIt =
      ::find(_queuedCommandMap[command->priority].begin(),
             _queuedCommandMap[command->priority].end(), command);
  ...
  if (_currentCommand == command) {
    ...
    SWARN("Closing outstanding command '"
          << command->request.methodLine << "' (" << command->id << ") after "
  ...
  if (SContains(_escalatedCommandMap, command->id)) {
    ...
    if (_masterPeer && _masterPeer->s) {
      ...
      SWARN("Trying to cancel escalated command "
            << command->id << ", but no master. Just erasing it.");
    }
...
list<string> SQLiteNode::getQueuedCommandList() {
  ...
  SFOREACHPRIORITYQUEUE(Command *, _queuedCommandMap, commandIt)
  commandList.push_back((*commandIt)->request.methodLine + " (" +
                        (*commandIt)->id + ")");
  return commandList;
}
...
list<string> SQLiteNode::getEscalatedCommandList() {
  list<Command *> orderedEscalatedCommands =
      _getOrderedCommandListFromMap(_escalatedCommandMap);
  list<string> commandList;
...
void SQLiteNode::_queueCommand(Command *command) {
  ...
  if (SContains(_synchronousCommands, SToLower(command->request.methodLine))) {
    ...
    SINFO("'" << command->request.methodLine
              << "' in synchronous comand list; setting QUORUM consistency");
    ...
    int wc = SStateNameToInt(SQLCConsistencyLevelNames,
                             command->request["writeConsistency"],
                             SQLC_NUM_CONSISTENCY_LEVELS);
    if (wc >= 0)
      command->writeConsistency = (SQLCConsistencyLevel)wc;
    else
      SWARN("Unknown write consistency supplied '"
            << command->request["writeConsistency"] << "'. Ignoring.");
    SINFO("'" << command->request.methodLine
              << "' has overridden default consistency with '"
              << command->request["writeConsistency"] << "' ("
  ...
  if (_queuedCommandMap[command->priority].empty() ||
      command->creationTimestamp >
          _queuedCommandMap[command->priority].back()->creationTimestamp)
...
void SQLiteNode::_escalateCommand(Command *command) {
  ...
  SINFO("Escalating '" << command->request.methodLine << "' (" << command->id
                       << ") to MASTER '" << _masterPeer->name << "' after "
...
void SQLiteNode::_finishCommand(Command *command) {
  ...
  uint64_t replicationElapsed =
      (command->replicationStartTimestamp
           ? STimeNow() - command->replicationStartTimestamp
           : 0);
  SINFO("Finishing command '"
        << command->request.methodLine << "'(" << command->id << ")->'"
        << command->response.methodLine << "' after "
        << elapsed / STIME_US_PER_MS << "ms (" << httpElapsed / STIME_US_PER_MS
        << "ms in http) (" << replicationElapsed / STIME_US_PER_MS
        << " ms in replication)");
  ...
  if (command->initiator) {
    ...
    if (elapsed < STIME_US_PER_S * 4 || !command->request["HeldBy"].empty()) {
      SINFO("Responding to escalation for transaction '"
            << command->request.methodLine << "' (" << command->id << ") after "
            << elapsed / STIME_US_PER_MS << " ms");
    } else {
      SWARN("Responding to escalation for transaction '"
            << command->request.methodLine << "' (" << command->id << ") after "
            << elapsed / STIME_US_PER_MS << " ms (slow)");
    }
...
bool SQLiteNode::update(uint64_t &nextActivity) {
  ...
  case SQLC_SEARCHING: {
    ...
    SFOREACH(list<Peer *>, peerList, peerIt) {
      ...
      if (loggedIn) {
        ...
        if (!freshestPeer ||
            peer->calcU64("CommitCount") >
                freshestPeer->calcU64("CommitCount")) {
          freshestPeer = peer;
        }
        ...
        bool peerIsFresherThanUs =
            (peer->calcU64("CommitCount") > _db.getCommitCount());
    ...
    // OR timeout
    SINFO("Signed in to " << numLoggedInFullPeers << " of " << numFullPeers
                          << " full peers (" << peerList.size()
                          << " with permaslaves), timeout in "
                          << (_stateTimeout - STimeNow()) / STIME_US_PER_MS
                          << "ms");
    if (((float)numLoggedInFullPeers < numFullPeers / 2.0) &&
        (STimeNow() < _stateTimeout))
      return false;
    ...
    if (freshestPeerCommitCount == _db.getCommitCount()) {
      // We're up to date
      SINFO("Synchronized with the freshest peer '" << freshestPeer->name
                                                    << "', WAITING.");
      _changeState(SQLC_WAITING);
      return true; // Re-update
    }
    ...
    if (nearestSlave) {
      ...
      SINFO("SYNCHRONIZING from nearest slave '"
            << nearestSlave->name << " ("
      ...
      SINFO("SYNCHRONIZING from freshest peer '"
            << freshestPeer->name << " ("
  ...
  case SQLC_WAITING: {
    ...
    if (gracefulShutdown()) {
      ...
      if (_queuedCommandMap.empty()) {
        ...
        SHMMM(
            "Graceful shutdown underway but queued commands so continuing...");
      }
    }
      ...
      if (peer->params["Permaslave"] != "true") {
        ...
        if (SIEquals((*peer)["LoggedIn"], "true")) {
          ...
          if (!highestPriorityPeer ||
              peer->calc("Priority") > highestPriorityPeer->calc("Priority"))
          ...
          if (SIEquals(peerState, "STANDINGUP") ||
            ...
            if (currentMaster)
              PHMMM("Multiple peers trying to stand up (also '"
                    << currentMaster->name
    ...
    if (!currentMaster && numLoggedInFullPeers * 2 >= numFullPeers &&
      ...
      SINFO("No master and we're highest priority (over "
            << highestPriorityPeer->name << "), STANDINGUP");
      SFOREACH(list<Peer *>, peerList, peerIt)
    ...
    // Keep waiting
    SDEBUG("Connected to " << numLoggedInFullPeers << " of " << numFullPeers
                           << " full peers (" << peerList.size()
      ...
      if (peer->params["Permaslave"] != "true") {
        ...
        if (SIEquals((*peer)["LoggedIn"], "true")) {
          ...
          if (!peer->isSet("StandupResponse")) {
            ...
            PHMMM("Refused our STANDUP (" << (*peer)["Reason"]
                                          << "), cancel and RESEARCH");
            _changeState(SQLC_SEARCHING);
            return true; // Re-update
          }
  ...
  case SQLC_STANDINGDOWN: {
    ...
    if (_currentCommand && !_currentCommand->transaction.empty()) {
      ...
      int numFullResponded =
          0; // Num full peers that have responded approve/deny
      ...
      SQLCConsistencyLevel consistencyRequired =
          (_commitsSinceCheckpoint >= _quorumCheckpoint)
              ? SQLC_QUORUM
              : _currentCommand->writeConsistency;
      ...
      // resubmitted.
      bool initiatorSubscribed =
          (!_currentCommand->initiator ||
           _currentCommand->initiator->test("Subscribed"));

      // Record these for posterity
      SDEBUG("'" << _currentCommand->request.methodLine << "' (#"
                 << _currentCommand->id << "): "
                                           "numFullPeers="
                 << numFullPeers << ", "
                                    "numFullSlaves="
      ...
      if (!initiatorSubscribed || (everybodyResponded && !consistentEnough)) {
        ...
        SWARN("Rolling back transaction for '"
              << _currentCommand->request.methodLine
              << "' because initiatorSubscribed=" << initiatorSubscribed
        ...
        _currentCommand->response.methodLine =
            "500 Failed to get adequate consistency";
        ...
        uint64_t beginElapsed, readElapsed, writeElapsed, prepareElapsed,
            commitElapsed, rollbackElapsed;
        uint64_t totalElapsed = _db.getLastTransactionTiming(
            beginElapsed, readElapsed, writeElapsed, prepareElapsed,
            commitElapsed, rollbackElapsed);
        SINFO("Committed master transaction for '"
              ...
              << "). " << _commitsSinceCheckpoint << " commits since quorum "
                                                     "(consistencyRequired="
              << SQLCConsistencyLevelNames[consistencyRequired] << "), "
              << numFullApproved << " of " << numFullPeers << " approved "
                                                              "("
      ...
      if (commandFinished) {
        ...
        SINFO("Waiting to commit: '"
              << _currentCommand->request.methodLine << "' (#"
              << _currentCommand->id << "): consistencyRequired="
              << SQLCConsistencyLevelNames[consistencyRequired]
      ...
      if (gracefulShutdown()) {
        ...
        SFOREACH(list<Peer *>, peerList, peerIt) {
          ...
          if (SIEquals((*peer)["State"], "MASTERING")) {
            ...
            standDownReason = "Found another MASTER (" + peer->name +
                              "), STANDINGDOWN to clean it up.";
            ...
            if (peer->calc("Priority") > _priority) {
              ...
              standDownReason = "Found higher priority WAITING peer (" +
                                peer->name + ") while MASTERING, STANDINGDOWN";
              ...
              standDownReason = "Found WAITING peer (" + peer->name +
                                ") with more data than us (we have " +
                                SToStr(_db.getCommitCount()) + "/" +
                                _db.getCommittedHash() + ", it has " +
                                (*peer)["CommitCount"] + "/" + (*peer)["Hash"] +
            ...
            while (nextIt != commandList->end()) {
              ...
              if (!command->request["HeldBy"].empty()) {
                ...
                if (command->request.isSet("Timeout") &&
                    elapsed >
                        command->request.calc64("Timeout") * STIME_US_PER_MS) {
              ...
              if (!command->httpsRequest) {
                ...
                if (_peekCommandWrapper(_db, command)) {
                  ...
                  SINFO("Finished processing peekable command '"
                        << command->request.methodLine << "' (" << command->id
                        << "), nothing to commit.");
                ...
                if (!command->request["HeldBy"].empty()) {
                  ...
                  SINFO("Hold re-placed on command by '"
                        << command->request["HeldBy"] << "', skipping.");
                  continue;
                }
              ...
              SINFO("Starting processing command '"
                    << _currentCommand->request.methodLine << "' ("
                    << _currentCommand->id << ")");
              _processCommandWrapper(_db, _currentCommand);
              SASSERT(
                  !_currentCommand->response.empty()); // Must set a response
              ...
              if (_db.insideTransaction()) {
                ...
                _currentCommand->transaction["Command"] =
                    _currentCommand->request.methodLine;
                _currentCommand->transaction["NewCount"] =
                    SToStr(_db.getCommitCount() + 1);
                _currentCommand->transaction["NewHash"] =
                    _db.getUncommittedHash();
                _currentCommand->transaction["ID"] = _currentCommand->id;
                _currentCommand->transaction.content =
                    _db.getUncommittedQuery();
                _sendToAllPeers(_currentCommand->transaction,
                ...
                SINFO("Finished processing command '"
                      << _currentCommand->request.methodLine << "' ("
                      << _currentCommand->id << "), nothing to commit.");
                SASSERT(!_db.insideTransaction());
                _finishCommand(_currentCommand);
                _currentCommand = 0;
              }
  ...
  case SQLC_SLAVING:
    ...
    if (!SIEquals((*_masterPeer)["State"], "MASTERING")) {
      ...
      list<Command *> escalatedCommandList =
          _getOrderedCommandListFromMap(_escalatedCommandMap);
      while (!escalatedCommandList.empty()) {
        ...
        SHMMM("Re-queueing escalated command '" << command->request.methodLine
                                                << "' (" << command->id << ")");
    ...
    SFOREACHMAPREVERSE(int, list<Command *>, _queuedCommandMap, it) {
      ...
      while (!commandList->empty()) {
        ...
        if (command->httpsRequest) {
          ...
          SHMMM("Escalated command '" << command->request.methodLine << "' ("
                                      << command->id
...
void SQLiteNode::_onMESSAGE(Peer *peer, const SData &message) {
  ...
  if (SIEquals(message.methodLine, "LOGIN")) {
    ...
    SASSERT(!_priority ||
            message.calc("Priority") != _priority); // to prevent misconfig:
                                                    // only one at each
                                                    // priority, except 0
    PINFO("Peer logged in at '"
          << message["State"] << "', priority #" << message["Priority"]
          << " commit #" << message["CommitCount"] << " (" << message["Hash"]
          << ")");
    ...
    if (oldState == newState) {
      ...
      PINFO("Peer received new commit in state '"
            << oldState << "', commit #" << message["CommitCount"] << " ("
            << message["Hash"] << ")");
    } else {
      // State changed -- first see if it's doing anything unusual
      PINFO("Peer switched from '" << oldState << "' to '" << newState
                                   << "' commit #" << message["CommitCount"]
                                   << " (" << message["Hash"] << ")");
      int from = 0, to = 0;
      ...
      switch (from) {
      case SQLC_SEARCHING:
        okTransition = (to == SQLC_SYNCHRONIZING || to == SQLC_WAITING ||
                        to == SQLC_MASTERING);
        break;
      case SQLC_SYNCHRONIZING:
        ...
        break;
      case SQLC_WAITING:
        okTransition = (to == SQLC_SEARCHING || to == SQLC_STANDINGUP ||
                        to == SQLC_SUBSCRIBING);
        break;
      case SQLC_STANDINGUP:
      ...
      if (!okTransition)
        PWARN("Peer making invalid transition from '"
              << SQLCStateNames[from] << "' to '" << SQLCStateNames[to] << "'");
        ...
        if (SWITHIN(SQLC_STANDINGUP, _state, SQLC_STANDINGDOWN)) {
          ...
          if (peer->calc("Priority") > _priority) {
            ...
            if (_majoritySubscribed()) {
              ...
              PHMMM(
                  "Lower-priority peer is trying to stand up while we are "
                  << SQLCStateNames[_state]
              ...
              PWARN("Lower-priority peer is trying to stand up while we are "
                    << SQLCStateNames[_state]
                    << ", but we don't have a majority of the cluster so "
    ...
    if (peer->isSet("StandupResponse"))
      PWARN("Already received standup response '"
            << (*peer)["StandupResponse"] << "', now receiving '"
    ...
    try {
      ...
      if (_db.getCommitCount() == peerCommitCount) {
        // All done
        SINFO("Synchronization complete, at commitCount #"
        ...
        SWARN("We have more data ("
              << _db.getCommitCount() << ") than our sync peer '"
              << _syncPeer->name << "' (" << peerCommitCount
              << "), reconnecting and SEARCHING.");
        _reconnectPeer(_syncPeer);
        _syncPeer = 0;
        _changeState(SQLC_SEARCHING);
        ...
        _stateTimeout = STimeNow() + SQL_NODE_SYNCHRONIZING_RECV_TIMEOUT +
                        SRand64() % STIME_US_PER_M * 5;
      }
    } catch (const char *e) {
      // Transaction failed
      SWARN("Synchronization failed '" << e
                                       << "', reconnecting and re-SEARCHING.");
      _reconnectPeer(_syncPeer);
      _syncPeer = 0;
      _changeState(SQLC_SEARCHING);
      throw e;
    ...
    try {
      ...
      SINFO("Subscription complete, at commitCount #"
            << _db.getCommitCount() << " (" << _db.getCommittedHash()
            << "), SLAVING");
      _changeState(SQLC_SLAVING);
    } catch (const char *e) {
      // Transaction failed
      SWARN("Subscription failed '"
            << e << "', reconnecting to master and re-SEARCHING.");
      _reconnectPeer(_masterPeer);
      _changeState(SQLC_SEARCHING);
      throw e;
    }
    ...
    if (_db.getUncommittedHash() != message["NewHash"]) {
      ...
      PWARN("New hash mismatch: command='"
            << _currentTransactionCommand << "', commitCount=#"
            << _db.getCommitCount() << "', committedHash='"
            << _db.getCommittedHash() << "', uncommittedHash='"
            << _db.getUncommittedHash() << "', uncommittedQuery='"
            << _db.getUncommittedQuery() << "'");
    ...
    if (_priority) {
      ...
      PINFO("Would approve transaction #"
            << _db.getCommitCount() + 1 << " (" << _db.getUncommittedHash()
            << ") for command '" << _currentTransactionCommand
    ...
    if (commandIt != _escalatedCommandMap.end()) {
      ...
      // crashes.
      SINFO("Master is processing our command "
            << message["ID"] << " (" << _currentTransactionCommand << ")");
    ...
    try {
      ...
      if (_currentCommand && _currentCommand->id == message["ID"]) {
        ...
        uint64_t replicationElapsed =
            STimeNow() - _currentCommand->replicationStartTimestamp;
        PINFO("Peer approved transaction #"
              << message["NewCount"] << " (" << message["NewHash"] << ") after "
        ...
        PINFO("Peer approved transaction #" << message["NewCount"] << " ("
                                            << message["NewHash"]
                                            << ") after commit.");
    } catch (const char *e) {
      ...
      SWARN("Received APPROVE_TRANSACTION for transaction #"
            << message.calc("NewCount") << " (" << message["NewHash"] << ", "
            << message["ID"] << ") but '" << e << "', ignoring.");
    }
    ...
    uint64_t beginElapsed, readElapsed, writeElapsed, prepareElapsed,
        commitElapsed, rollbackElapsed;
    uint64_t totalElapsed = _db.getLastTransactionTiming(
        beginElapsed, readElapsed, writeElapsed, prepareElapsed, commitElapsed,
        rollbackElapsed);
    ...
    if (commandIt != _escalatedCommandMap.end()) {
      ...
      // so we know that this command might be corrupted if the master
      // crashes.
      SINFO("Master has committed in response to our command "
            << message["ID"]);
      commandIt->second->transaction = message;
    }
    ...
    if (commandIt != _escalatedCommandMap.end()) {
      ...
      // so we know that this command might be corrupted if the master
      // crashes.
      SINFO("Master has rolled back in response to our command "
            << message["ID"]);
      commandIt->second->transaction = message;
    }
    ...
    if (_state != SQLC_MASTERING) {
      ...
      if (!message.isSet("ID"))
        throw "missing ID";
      PINFO("Received ESCALATE command for '" << message["ID"] << "' ("
                                              << request.methodLine << ")");
      Command *command = new Command;
    ...
    if (_state != SQLC_MASTERING) {
      ...
      if (SIEquals((*commandIt)->id, commandID)) {
        // Cancel that command
        Command *command = *commandIt;
        SINFO("Cancelling escalated command "
              << command->id << " (" << command->request.methodLine << ")");
    ...
    if (commandIt != _escalatedCommandMap.end()) {
      ...
      SHMMM("Received ESCALATE_RESPONSE for unknown command ID '"
            << message["ID"] << "', ignoring.");
    ...
    if (!message.isSet("ID"))
      throw "missing ID";
    PINFO("Received ESCALATE_ABORTED for '" << message["ID"] << "' ("
                                            << message["Reason"] << ")");
    ...
    if (commandIt != _escalatedCommandMap.end()) {
      // Re-queue this
      Command *command = commandIt->second;
      PINFO("Re-queueing command '" << message["ID"] << "' ("
                                    << command->request.methodLine << ") ("
                                    << command->id << ")");
      _queuedCommandMap[command->priority].push_front(command);
      _escalatedCommandMap.erase(commandIt);
    } else
      SWARN("Received ESCALATE_ABORTED for unescalated command "
            << message["ID"] << ", ignoring.");
  } else
...
void SQLiteNode::_onDisconnect(Peer *peer) {
  ...
  if (peer->s && peer->s->sendBuffer.find("ESCALATE_RESPONSE") != string::npos)
    PWARN("Initiating slave died before receiving response to escalation: "
          << peer->s->sendBuffer);
  ...
  if (peer == _masterPeer) {
    ...
    if (!_db.getUncommittedHash().empty()) {
      ...
      PHMMM("Was expecting a response for transaction #"
            << _db.getCommitCount() + 1 << " (" << _db.getUncommittedHash()
    ...
    while (!escalatedCommandList.empty()) {
      ...
      if (command->transaction.methodLine.empty()) {
        ...
        // died, let's resubmit
        PHMMM("Re-queueing escalated command '" << command->request.methodLine
                                                << "' (" << command->id << ")");
        _queuedCommandMap[command->priority].push_front(command);
      } else {
        ...
        PWARN("Aborting escalated command '"
              << command->request.methodLine << "' (" << command->id
  ...
  SFOREACHMAP(int, list<Command *>, _queuedCommandMap, it) {
    ...
    while (nextCommandIt != commandList->end()) {
      ...
      if (command->initiator == peer) {
        // Drop this command
        PHMMM("Queued command initiator disconnected, dropping '"
              << command->request.methodLine << "' (" << command->id << ")");
  ...
  if (_currentCommand && _currentCommand->initiator == peer) {
    ...
    PHMMM("Current command initiator disconnected, aborting '"
...
void SQLiteNode::_sendToAllPeers(const SData &message, bool subscribedOnly) {
  ...
  SFOREACH(list<Peer *>, peerList, peerIt) {
    ...
    Peer *peer = *peerIt;
    if (peer->s &&
        (!subscribedOnly || SIEquals((*peer)["Subscribed"], "true"))) {
...
void SQLiteNode::_changeState(SQLCState newState) {
  ...
  if (newState != oldState) {
    ...
    SDEBUG("Switching from '" << SQLCStateNames[_state] << "' to '"
                              << SQLCStateNames[newState] << "'");
    uint64_t timeout = 0;
    if (newState == SQLC_SEARCHING || newState == SQLC_STANDINGUP ||
        newState == SQLC_SUBSCRIBING)
      timeout = SQL_NODE_DEFAULT_RECV_TIMEOUT + SRand64() % STIME_US_PER_S * 5;
    else if (newState == SQLC_SYNCHRONIZING)
      timeout =
          SQL_NODE_SYNCHRONIZING_RECV_TIMEOUT + SRand64() % STIME_US_PER_M * 5;
    else
      timeout = 0;
    ...
    if (newState == SQLC_MASTERING) {
      ...
      if (!_escalatedCommandMap.empty()) {
        ...
        _escalatedCommandMap.clear();
        SWARN("Switching from '" << SQLCStateNames[_state] << "' to '"
                                 << SQLCStateNames[newState]
  ...
  SASSERT(!_currentCommand ||
          (_state != SQLC_MASTERING && _state != SQLC_STANDINGDOWN));
}
...
void SQLiteNode::_queueSynchronize(Peer *peer, SData &response, bool sendAll) {
  ...
  if (peerCommitCount == _db.getCommitCount()) {
    ...
    PINFO("Synchronizing commits from " << peerCommitCount + 1 << "-"
                                        << _db.getCommitCount());
    ...
    SASSERTWARN(response.content.size() <
                10 * 1024 * 1024); // Let's watch if it gets over 10MB
  }
}
...
// --------------------------------------------------------------------------
list<SQLiteNode::Command *> SQLiteNode::_getOrderedCommandListFromMap(
...
// --------------------------------------------------------------------------
bool SQLiteNode::_majoritySubscribed(int &numFullPeersOut,
                                     int &numFullSlavesOut) {
