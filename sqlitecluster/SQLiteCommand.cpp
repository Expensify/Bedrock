...
SQLiteCommand::SQLiteCommand(SData&& _request) :
    ...
    escalated(false)
{
    // The command's unique cluster id is carried on the request's ID header (generated by the server for new commands,
    // or preserved from the originating node for escalated ones), so initialize id from it here.
    if (request.isSet("ID")) {
        id = request["ID"];
    }
