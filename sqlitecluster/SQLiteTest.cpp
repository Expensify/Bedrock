    ...
    void postSelect( fd_map& fdm, uint64_t& nextActivity )
    {
        ...
        while( nextIt != socketList.end() )
        {
            ...
            {   
                ...
                else nextActivity = SMin( nextActivity, timeout );
            }
...
struct SQLiteTestNode : public SQLiteNode
{
    ...
    SQLiteTestNode( const string& filename, const string& host, int priority )
        : SQLiteNode( filename, filename, host, priority, 1000, 0, STIME_US_PER_S*2 + SRand64()%STIME_US_PER_S*5, "testversion", 100 )
    {
    ...
    virtual void _processCommand( SQLite& db, Command* command )
    {
        ...
        else
        {
            ...
            else SERROR( "Unrecognized request '" << command->request.methodLine << "'" );
        }
    }
    ...
    bool update( uint64_t& nextActivity )
    {
        ...
        if( commandTimeout ) nextActivity = SMin( nextActivity, commandTimeout );
        return result;
    }
};
...
struct SQLiteTester
{
    ...
    // ---------------------------------------------------------------------
    SQLiteTester( int numDBs ) :
        _httpsServer( "localhost:12345" )
    {
        // Initialize
    ...
    ~SQLiteTester( )           { for( size_t c=0; c<_nodeArray.size(); ++c ) if( _nodeArray[c] ) SDELETE( _nodeArray[c] ); }
    ...
    string getActivitySummary( )
    {
        ...
        for( size_t c=0; c<_nodeArray.size(); ++c )
        {
            ...
            if( !node->getQueuedCommandList().empty()    ) activityList.push_back( node->name + ":queued" );
            if( !node->getEscalatedCommandList().empty() ) activityList.push_back( node->name + ":escalated" );
            if( !node->getProcessedCommandList().empty() ) activityList.push_back( node->name + ":processed" );
        }
    ...
    // ---------------------------------------------------------------------
    void unlink( size_t c ) { SASSERT( !_nodeArray[c] ); ::unlink( ("test" + SToStr(c) + ".db").c_str() ); }
    void unlinkAll( )       { for( size_t c=0; c<_nodeArray.size(); ++c ) unlink( c ); }
    ...
    void createCommand( int index, const SData& request )
    {
        ...
        Command command =
        {
            _nodeArray[index]->openCommand( requestCopy, 0 ),
            _nodeArray[index]
        };
    ...
    void createCommands(int index, uint64_t count)
    {
        ...
        while( count-- )
        {
            ...
            {
                case 0 : request.methodLine = "INCREMENT";          break;
                case 1 : request.methodLine = "NOOP";               break;
                case 2 : request.methodLine = "NOOP_EXTERNAL";      break;
                case 3 : request.methodLine = "INCREMENT_EXTERNAL"; break;
            }
    ...
    SASSERT( SWITHIN( 0, c, _nodeArray.size()-1 ) );
        ...
        if( _nodeArray[c] )
        {
            ...
            if( graceful ) _nodeArray[c]->beginShutdown( );
            ...
            while( nextIt != _commandList.end() )
            {
                ...
                if( !graceful || (command.command->request["Connection"] == "wait") )
                    ...
                    {
                        // Close it
                        SINFO( "Closing outstanding command '" << command.command->request.methodLine << " on node #" << c );
            ...
            if( !graceful ) SDELETE( _nodeArray[c] );
        }
        else
        {
            // Add a new peer
            ...
            _nodeArray[c] = new SQLiteTestNode( dbName, host, c < 3 ? 10 - (int)c : 0 ); // Nodes after the third are 0-priority permaslaves
            ...
            for( size_t d=0; d<_nodeArray.size(); ++d )
                if( d != c ) {
                    STable params;
                    if( d>=3 ) params["Permaslave"] = "true"; // Nodes after the third are 0-priority permaslaves
    ...
    void update( STestTimer& test )
    {
        ...
        SFOREACH( list<Command>, _commandList, commandIt )
        {
            ...
            numOustandingExternalCommands += (*commandIt).command->httpsRequest && !(*commandIt).command->httpsRequest->response;
        }
        if( activeSockets == 0 && !numHeldCommands && timeout>STIME_US_PER_S*9 )
        {
            // This select shouldn't happen: we didn't have any activity and aren't waiting on any held commands, and the state
            // machine didn't seem to be waiting for anything (because the timeout was large).
            STESTLOG( "Idle select: activeSockets=" << activeSockets << ", timeout=" << timeout/STIME_US_PER_MS << "ms, elapsed=" << elapsed/STIME_US_PER_MS << "ms, waiting on " << numNonHeldCommands << " non-held commands (of " << _commandList.size() << " total, " << numOustandingExternalCommands << " external outstanding)" << endl );
        }
        else
        {
            // This is a valid select -- waiting for socket activity or timeout
            SDEBUG( "Active select: activeSockets=" << activeSockets << ", timeout=" << timeout/STIME_US_PER_MS << "ms, elapsed=" << elapsed/STIME_US_PER_MS << "ms, waiting on " << numNonHeldCommands << " non-held commands (of " << _commandList.size() << " total, " << numOustandingExternalCommands << " external outstanding)" << endl );
        }
                    ...
                    SFOREACH( list<Command>, _commandList, commandIt )
                        if( commandIt->command == sqlCommand )
                        {
                            ...
                            if( aborted )
                            {
                                ...
                                if( !_abortsOk ) STESTEQUALS( sqlCommand->response.methodLine, "ABORTED" );
                                _commandsAborted++;
                            }
...
STestTimer test( message );
    ...
    while( STimeNow() < end )
    {
        ...
        if( tester._commandList.empty() ) commands++; // Always want something
    ...
    if( !tester._nodeArray[ blinker ] ) tester.toggle( blinker );
    while( true )
    {
        // Update
        tester.update( test );
        ...
        if( !tester.mastering(0) ) continue;
        ...
        for( size_t c=1; c<tester._nodeArray.size(); ++c )
            if( !tester.slaving(c) )
                allSlaving = false;
        if( !allSlaving ) continue;

        // Make sure all commands are done processing
        if( !tester._commandList.empty() ) continue;
    ...
    if( blinker == initiator )
    {
        ...
        {
            // Too many
            STESTLOG( "Too many dropped commands: expected up to " << PERCENT << "% but had " << 100*dropped/tester._commandsStarted << "% (" << dropped << "/" << tester._commandsStarted << ")" );
            test.success = false;
        }
    }
    else
    {
        ...
        {
            // Out of range
            STESTLOG( "Database value out of range: is " << tester.read(0) << ", should be within " << minDB << "-" << maxDB );
        }
        ...
        {
            // Note the failure
            STESTLOG( "Too many aborted commands: expected up to " << numBlinks << " but had " << tester._commandsAborted );
            test.success = false;
        }
...
void SQLiteTest( const SData& args )
{
    if( args.isSet("-all") || args.isSet("-1") )
    {
        ...
        STestTimer test("Standing up one node");
            ...
            while( !tester.mastering(0) ) tester.update( test ); // Wait for 0 to master
        }

        {
        STestTimer test("Testing standalone commands");
            tester.createCommands( 0, NUM_COMMANDS ); // Queue a bunch of commands
            while( !tester._commandList.empty() ) tester.update( test ); // Wait until they're done
    ...
    if( args.isSet("-all") || args.isSet("-2") )
    {
        ...
        STestTimer test("Standing up two nodes");
            ...
            while( !tester.mastering(0) || !tester.slaving(1) ) tester.update( test ); // Wait until stable
        }
        ...
        STestTimer test("Testing two-node master commands");
            ...
            while( !tester._commandList.empty() ) tester.update( test ); // Process the commands
        ...
        STestTimer test("Test a master hold/release command");
            ...
            while( STimeNow() < waitUntil ) tester.update( test );
            ...
            while( !tester._commandList.empty() ) tester.update( test ); // Verify both process
            STESTEQUALS( tester.getActivitySummary(), "" );
        }
        ...
        STestTimer test("Test a slave hold/timeout command");
            ...
            tester.createCommand( 1, holdRequest ); // Queue the held slave command
            while( !tester._commandList.empty() ) tester.update( test ); // Verify it timesout without being released
            STESTEQUALS( tester.getActivitySummary(), "" );
        }
        ...
        STestTimer test("Test slave hold/release commands are cancelled on dirty slave shutdown");
            ...
            while( !tester.mastering(0) || !tester.slaving(1) ) tester.update( test ); // Wait to stabilize
        ...
        STestTimer test("Testing graceful two-node recovery of the slave");
            ...
            while( tester._nodeArray[1] ) tester.update( test ); // Wait for 1 to shut down
        ...
        STestTimer test("Testing dirty two-node recovery of the slave");
            ...
            tester.unlink( 1 ); // Clean up 1
            tester.toggle( 1 ); // Restart 1
            while( !tester.mastering(0) || !tester.slaving(1) ) tester.update( test ); // Stabilize
            STESTEQUALS( tester.read(0), tester.read(1) ); // Verify they're in sync
            STESTEQUALS( tester.getActivitySummary(), "" );
        }
        ...
        STestTimer test("Testing graceful two-node recovery of the master");
            ...
            while( tester._nodeArray[0] ) tester.update( test ); // Wait for 0 to shut down
        ...
        STestTimer test("Testing graceful two-node master shutdown with outstanding commands");
            tester.createCommands( 0, NUM_COMMANDS ); // Queue a bunch of commands on the master
            while( tester._commandList.size() > NUM_COMMANDS/2 ) tester.update( test ); // Process half
            tester.toggle( 0, true ); // Shut down the master gracefully while 1/2 commands remain
            while( tester._nodeArray[0] ) tester.update( test ); // Wait for it to shut down
            ...
            while( !tester.mastering(0) || !tester.slaving(1) ) tester.update( test ); // Stabilize
            STESTEQUALS( tester.read(0), tester.read(1) ); // Verify they're in sync
            while( !tester._commandList.empty() ) tester.update( test ); // Wait for slave to re-escalate and process the rest
            STESTEQUALS( tester.getActivitySummary(), "" );
        }
        ...
        STestTimer test("Verify a HeldBy: command doesn't block a master from graceful shutdown");
            ...
            while( tester._nodeArray[0] ) tester.update( test ); // Wait for master to shut down
        ...
        STestTimer test("Verify a Connection:wait command doesn't block a slave from graceful shutdown");
            ...
            while( tester._nodeArray[1] ) tester.update( test ); // Wait for slave to shut down
    ...
    if( args.isSet("-all") || args.isSet("-3") )
    {
        ...
        STestTimer test("Standing up three nodes");
            ...
            while( !tester.mastering(0) ||
                   !tester.slaving(1) ||
                   !tester.slaving(2) ) tester.update( test );
        }
        ...
        STestTimer test("Testing three-node master commands");
            ...
            while( !tester._commandList.empty() ) tester.update( test);
        ...
        STestTimer test("Testing three-node failover/recovery of master");
            tester.toggle( 0 );
            while( !tester.mastering(1) ) tester.update( test );
            tester.unlink( 0 );
            tester.toggle( 0 );
            while( !tester.mastering(0) ||
                   !tester.slaving(1) ||
        ...
        STestTimer test("Testing three-node failure/recovery of primary slave");
            ...
            tester.unlink( 1 );
            tester.toggle( 1 );
            while( !tester.slaving(1) ) tester.update( test );
            STESTEQUALS( tester.read(0), tester.read(1) );
            STESTEQUALS( tester.read(0), tester.read(2) );
            STESTEQUALS( tester.getActivitySummary(), "" );
        }
        ...
        STestTimer test("Testing three-node failure/recovery of secondary slave");
            ...
            tester.unlink( 2 );
            tester.toggle( 2 );
            while( !tester.slaving(2) ) tester.update( test );
            STESTEQUALS( tester.read(0), tester.read(1) );
            STESTEQUALS( tester.read(0), tester.read(2) );
            STESTEQUALS( tester.getActivitySummary(), "" );
        }
