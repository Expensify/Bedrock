...
struct SQLiteTestWebClient : public SHTTPSManager {
  ...
  virtual bool _onRecv(Transaction *transaction) {
    ...
    string ignore;
    if (!SParseResponseMethodLine(transaction->fullResponse.methodLine, ignore,
                                  transaction->response, ignore))
...
struct SQLiteTestNode : public SQLiteNode {
  ...
  SQLiteTestNode(const string &filename, const string &host, int priority)
      : SQLiteNode(filename, filename, host, priority, 1000, 0,
                   STIME_US_PER_S * 2 + SRand64() % STIME_US_PER_S * 5,
                   "testversion", 100) {
  ...
  virtual bool _peekCommand(SQLite &db, Command *command) {
    ...
    if (command->request.methodLine == "NOOP") {
      ...
      return true; // Done
    } else if (getState() == SQLC_MASTERING &&
               SEndsWith(command->request.methodLine, "EXTERNAL")) {
  ...
  virtual void _processCommand(SQLite &db, Command *command) {
    ...
    if (SIEquals(command->request.methodLine, "UpgradeDatabase")) {
      // Verify the database
      bool created = false;
      SASSERT(db.verifyTable("test", "CREATE TABLE test ( value INTEGER )",
                             created));
      if (created) {
        // Seed the database
...
struct SQLiteTester {
  ...
  void createCommand(int index, const SData &request) {
    // Create the command, and add a bunch of weirdly encoded attributes to make
    ...
    requestCopy["t"] = "t\t t\t";
    requestCopy["z"] = "z";
    requestCopy["writeConsistency"] =
        SQLCConsistencyLevelNames[_commandsStarted %
                                  SQLC_NUM_CONSISTENCY_LEVELS];
    Command command = {_nodeArray[index]->openCommand(requestCopy, 0),
                       _nodeArray[index]};
  ...
  void toggle(size_t c, bool graceful = false) {
    ...
    if (_nodeArray[c]) {
      ...
      SHMMM("<<<<<<<<<<<<<<<<<<< " << (graceful ? "Stopping" : "Killing")
                                   << " server #" << c
                                   << "<<<<<<<<<<<<<<<<<<<<<");
      ...
      while (nextIt != _commandList.end()) {
        ...
        if (!graceful || (command.command->request["Connection"] == "wait"))
          if (command.cluster == _nodeArray[c]) {
            // Close it
            SINFO("Closing outstanding command '"
                  << command.command->request.methodLine << " on node #" << c);
      ...
      _nodeArray[c] = new SQLiteTestNode(
          dbName, host,
          c < 3 ? 10 - (int)c
                : 0); // Nodes after the third are 0-priority permaslaves

      // Configure this node to connect to all the others (whether or not they
      // exist yet).
      ...
      for (size_t d = 0; d < _nodeArray.size(); ++d)
        if (d != c) {
          ...
          if (d >= 3)
            params["Permaslave"] =
                "true"; // Nodes after the third are 0-priority permaslaves
          _nodeArray[c]->addPeer("test" + SToStr(d) + ".db",
                                 "127.0.0.1:" + SToStr(10000 + d), params);
        }
    }
  }
  ...
  void update(STestTimer &test) {
    ...
    for (size_t c = 0; c < _nodeArray.size(); ++c)
      if (_nodeArray[c]) {
        if (_nodeArray[c]->shutdownComplete()) {
          ...
          SINFO("<<<<<<<<<<<<<<<< Graceful shutdown complete #"
                << c << "<<<<<<<<<<<<<");
          SDELETE(_nodeArray[c]);
        } else {
          // Alive, process
    ...
    // If we waited out the full timeout, verify there are no non-held,
    // non-externally-blocked commands
    ...
    SFOREACH(list<Command>, _commandList, commandIt) {
      ...
      numOustandingExternalCommands +=
          (*commandIt).command->httpsRequest &&
          !(*commandIt).command->httpsRequest->response;
    }
    if (activeSockets == 0 && !numHeldCommands &&
        timeout > STIME_US_PER_S * 9) {
      // This select shouldn't happen: we didn't have any activity and aren't
      // waiting on any held commands, and
      // the state
      // machine didn't seem to be waiting for anything (because the timeout was
      // large).
      STESTLOG("Idle select: activeSockets="
               << activeSockets << ", timeout=" << timeout / STIME_US_PER_MS
               << "ms, elapsed=" << elapsed / STIME_US_PER_MS
               << "ms, waiting on " << numNonHeldCommands
               << " non-held commands (of " << _commandList.size() << " total, "
               << numOustandingExternalCommands << " external outstanding)"
               << endl);
    } else {
      // This is a valid select -- waiting for socket activity or timeout
      SDEBUG("Active select: activeSockets="
             << activeSockets << ", timeout=" << timeout / STIME_US_PER_MS
             << "ms, elapsed=" << elapsed / STIME_US_PER_MS << "ms, waiting on "
             << numNonHeldCommands << " non-held commands (of "
             << _commandList.size() << " total, "
             << numOustandingExternalCommands << " external outstanding)"
             << endl);
    }
    ...
    for (size_t c = 0; c < _nodeArray.size(); ++c)
      if (_nodeArray[c]) {
        ...
        while (_nodeArray[c]->update(_nextActivity))
          ;
        status += " | #" + SToStr(c) + " " +
                  (string)SQLCStateNames[_nodeArray[c]->getState()];

        // Clean up any completed commands
        SQLiteNode::Command *sqlCommand = 0;
        while (!_commandList.empty() &&
               (sqlCommand = _nodeArray[c]->getProcessedCommand()))
          SFOREACH(list<Command>, _commandList, commandIt)
        if (commandIt->command == sqlCommand) {
          ...
          _commandsFinished++;
          bool aborted =
              !SIEquals(sqlCommand->response.methodLine, "200 OK") &&
              !SIEquals(sqlCommand->response.methodLine, "303 Timeout");
          bool incremented =
              SStartsWith(sqlCommand->request.methodLine, "INCREMENT");
...
/////////////////////////////////////////////////////////////////////////////
void SQLiteTestBlinks(SQLiteTester &tester, const string &message,
                      int initiator, int blinker) {
  ...
  tester._abortsOk =
      true; // Blinking can create valid aborts so let's not fail if we see them
  ...
  while (STimeNow() < end) {
    ...
    if (commands > 0 && tester._nodeArray[initiator] &&
        !tester._nodeArray[initiator]->gracefulShutdown()) {
  ...
  if (blinker == initiator) {
    ...
    if (100 * dropped / tester._commandsStarted > PERCENT) {
      ...
      STESTLOG("Too many dropped commands: expected up to "
               << PERCENT << "% but had "
               << 100 * dropped / tester._commandsStarted << "% (" << dropped
    ...
    int minDB =
        maxDB -
        tester._incrementsAborted; // These might not have been commiteed
    if (!SWITHIN(minDB, tester.read(0), maxDB)) {
      // Out of range
      STESTLOG("Database value out of range: is "
               << tester.read(0) << ", should be within " << minDB << "-"
               << maxDB);
    }

    // If the master is blinking while the slave initiates, the slave will
    // usually re-submit, but will
    ...
    if (tester._commandsAborted > numBlinks) {
      // Note the failure
      STESTLOG("Too many aborted commands: expected up to "
               << numBlinks << " but had " << tester._commandsAborted);
      test.success = false;
    }
  }
...
void SQLiteTest(const SData &args) {
  ...
  if (args.isSet("-all") || args.isSet("-2")) {
    ...
    // ---------------------------------------------------------------------
    {
      ...
      tester.createCommands(
          0, NUM_COMMANDS);           // Queue a bunch of commands on the master
      tester._incrementsFinished = 0; // Reset the increment counter
      int before = tester.read(0);    // Record the start value
      STESTEQUALS(tester.read(0),
                  tester.read(1)); // Verify both nodes have the same value
      while (!tester._commandList.empty())
        tester.update(test); // Process the commands
      STESTEQUALS(tester.read(0),
                  before + tester._incrementsFinished); // Verify it ads up
    ...
    // ---------------------------------------------------------------------
    {
      ...
      tester.createCommands(
          1, NUM_COMMANDS);           // Queue a bunch of commands on the slave
      tester._incrementsFinished = 0; // Reset the increment counter
      int before = tester.read(0);    // Record the start value
      STESTEQUALS(tester.read(0),
                  tester.read(1)); // Verify both nodes have the same value
      while (!tester._commandList.empty())
        tester.update(test); // Process the commands
      STESTEQUALS(tester.read(0),
                  before + tester._incrementsFinished); // Verify it adds up
      STESTEQUALS(tester.read(0), tester.read(1)); // Verify they're the same
      STESTEQUALS(tester.getActivitySummary(), "");
    }
    ...
    // ---------------------------------------------------------------------
    {
      ...
      SData holdRequest(
          "INCREMENT"); // Create a command that is held by another
      ...
      SData releaseRequest(
          "INCREMENT"); // Create a command to release the first
    ...
    // ---------------------------------------------------------------------
    {
      STestTimer test("Test slave hold/release commands are cancelled on dirty "
                      "slave shutdown");
      SData holdRequest(
          "INCREMENT"); // Create a command that is held by another
      holdRequest["Connection"] = "wait";
      ...
      while (tester._nodeArray[0]->getQueuedCommandList().empty())
        ...
        tester.update(test);
      STESTEQUALS(tester._commandList.size(), 1); // Verify not processed
      tester.toggle(1, false); // Kill the slave in a non-graceful way, so it
                               // can't ESCALATE_CANCEL
      while (!tester._nodeArray[0]->getQueuedCommandList().empty())
        // Wait for master to cancel escalated command
        tester.update(test);
    ...
    // ---------------------------------------------------------------------
    {
      STestTimer test("Testing graceful two-node master shutdown with "
      ...
      tester.toggle(
          0, true); // Shut down the master gracefully while 1/2 commands remain
      while (tester._nodeArray[0])
        tester.update(test); // Wait for it to shut down
      // Master has shut down, slave should still have ~1/2 commands ready to
      // re-queue when master restarts
      ...
      while (!tester._commandList.empty())
        tester.update(
            test); // Wait for slave to re-escalate and process the rest
      STESTEQUALS(tester.getActivitySummary(), "");
    }

    // ---------------------------------------------------------------------
    {
      STestTimer test("Verify a HeldBy: command doesn't block a master from "
                      "graceful shutdown");
      SData holdRequest(
          "INCREMENT"); // Create a command that is held by another
      holdRequest["Connection"] = "wait";
    ...
    // ---------------------------------------------------------------------
    {
      STestTimer test("Verify a Connection:wait command doesn't block a slave "
                      "from graceful shutdown");
      SData holdRequest(
          "INCREMENT"); // Create a command that is held by another
      holdRequest["Connection"] = "wait";
      holdRequest["HeldBy"] = "Mr. Dude";
      tester.createCommand(1,
                           holdRequest); // Queue the held command on the slave
      while (tester._nodeArray[0]->getQueuedCommandList().empty())
        tester.update(test);  // Wait until master gets the command
    ...
    SQLiteTestBlinks(
        tester, "Testing two-node master commands with master blinks", 0, 0);
    SQLiteTestBlinks(
        tester, "Testing two-node master commands with slave blinks", 0, 1);
    SQLiteTestBlinks(
        tester, "Testing two-node slave commands with master blinks", 1, 0);
    SQLiteTestBlinks(tester,
                     "Testing two-node slave commands with slave blinks", 1, 1);
  }
  ...
  if (args.isSet("-all") || args.isSet("-3")) {
    ...
    SQLiteTestBlinks(
        tester, "Testing three-node master commands with master blinks", 0, 0);
    SQLiteTestBlinks(
        tester, "Testing three-node master commands with primary slave blinks",
        0, 1);
    SQLiteTestBlinks(
        tester,
        "Testing three-node master commands with secondary slave blinks", 0, 1);
    SQLiteTestBlinks(
        tester, "Testing three-node primary slave commands with master blinks",
        1, 0);
    SQLiteTestBlinks(
        tester,
        "Testing three-node primary slave commands with primary slave blinks",
        1, 1);
    SQLiteTestBlinks(
        tester,
        "Testing three-node primary slave commands with secondary slave blinks",
        1, 2);
    SQLiteTestBlinks(
        tester,
        "Testing three-node secondary slave commands with master blinks", 2, 0);
    SQLiteTestBlinks(
        tester,
        "Testing three-node secondary slave commands with primary slave blinks",
        2, 1);
    SQLiteTestBlinks(tester, "Testing three-node secondary slave commands with "
                             "secondary slave blinks",
                     2, 2);
    ...
    // Test total chaos
    {
      ...
      while (STimeNow() < end) {
        ...
        if (!numAlive || now - lastBlink > STIME_US_PER_S * 20) {
          ...
          tester.toggle(blinker,
                        SRand15() % 2); // Randomly choose dirty or clean
          lastBlink = now;
          ++numBlinks;
        }
        ...
        if (commands && tester._nodeArray[initiator] &&
            !tester._nodeArray[initiator]->gracefulShutdown()) {
          // Time for a new command
          tester.createCommands(initiator, commands);
          lastCommand = now;
      ...
      STESTASSERT(tester._commandsStarted >= tester._commandsFinished);
      int dropped = tester._commandsStarted - tester._commandsFinished;
      STESTLOG("dropped=" << dropped
                          << ", aborted=" << tester._commandsAborted);
    }
  }
  ...
  if (args.isSet("-all") || args.isSet("-5")) {
    ...
    // ---------------------------------------------------------------------
    {
      ...
      while (!tester.mastering(0) && !tester.slaving(2) && !tester.slaving(3) &&
             !tester.slaving(4))
        tester.update(test);
    }
