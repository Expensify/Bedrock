...
public: // External API
  ...
  // Constructor
  SQLiteNode(const string &filename, const string &name, const string &host,
             int priority, int cacheSize, int autoCheckpoint,
             uint64_t firstTimeout, const string &version,
             int quorumCheckpoint = 0, const string &synchronousCommands = "",
             bool readOnly = false, int maxJournalSize = 1000000);
  virtual ~SQLiteNode();
  ...
  // Only let a read only node have an external party set the state master
  // version. These need to be known
  // by the read only threads even though they are replication concepts.  They
  // let the slaves know if they
  // should accept commands, and if their version matches master so they can
  // peek (otherwise skip peek and
  // escalate to master).
...
#define SPRIORITY_MIN 0
  Command *openCommand(const SData &request, int priority, bool unique = false,
                       int64_t commandExecuteTime = 0);

  // Gets a completed command from the database
  Command *getProcessedCommand();

  // Searches through commands in all states to see if we can find one that
  // matches
  Command *findCommand(const string &requestHeaderName,
                       const string &requestHeaderValue);

  // Gets the next command queued for processsing
  // **FIXME: This is only used in BedrockServer, and the priority is a needless
  // optimizatoin -- remove priority
  ...
  // Force quorum among the replica after every N commits.  This prevents master
  // from running ahead
  // too far. "Too far" is an arbitrary threshold that trades potential loss of
  // consistency in the
...
protected:
  ...
  map<int, list<Command *>>
      _queuedCommandMap; // priority  -> list<Command*> map
...
private: // Internal API
  ...
  void _reconnectAll();
  list<Command *>
  _getOrderedCommandListFromMap(const map<string, Command *> commandMap);
