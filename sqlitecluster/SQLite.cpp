...
thread_local uint64_t SQLite::_commitLockWait;
...
uint64_t SQLite::getCommitLockWait()
{
    return _commitLockWait;
}

void SQLite::resetCommitLockWait()
{
    _commitLockWait = 0;
}
...
bool SQLite::prepare(uint64_t* transactionID, string* transactionhash, chrono::microseconds commitLockTimeout, atomic<bool>* abortPtr, const optional<string>& replicationGUID)
{
    ...
    if (!_mutexLocked) {
        ...
        _commitLockWait += chrono::duration_cast<chrono::microseconds>(elapsed).count();
...
void SQLite::getLastNonBlankCommit(uint64_t index, uint64_t& commitID, string& hash)
{
    ...
    if (commitID && hash.empty()) {
        SERROR("Journal commit " << commitID << " has a nonblank query with an empty hash.");
    }
}
