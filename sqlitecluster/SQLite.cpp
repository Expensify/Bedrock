...
bool SQLite::read(const string& query, SQResult& result, bool skipQueryLog) const {
    uint64_t before = STimeNow();
    ...
    if (foundQuery != _queryCache.end()) {
        ...
        queryResult = !SQuery(_db, "read only query", query, result, 2000 * STIME_US_PER_MS, false, skipQueryLog);
