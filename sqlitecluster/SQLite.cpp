...
SQLite::SQLite(const string& filename, int cacheSize, int autoCheckpoint, int maxJournalSize, int journalTable,
  ...
  :  _minJournalTables(minJournalTables)
{
    // Initialize
...
bool SQLite::prepare() {
    ...
    // This is a hack to write to multiple journals from a single thread. It exists only for testing purposes, and
    // should be removed when adding real multi-threaded writes.
    int table = (SRandom::rand64() % (_minJournalTables + 2)) - 1;
    _journalName = getJournalTableName(table);
