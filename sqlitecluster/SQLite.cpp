...
void SQLite::sqliteLogCallback(void *pArg, int iErrCode, const char *zMsg) {
  SSYSLOG(LOG_INFO, SWHEREAMI << "[info] "
                              << "{SQLITE} Code: " << iErrCode
                              << ", Message: " << zMsg);
}

// --------------------------------------------------------------------------
SQLite::SQLite(const string &filename, int cacheSize, int autoCheckpoint,
               bool readOnly, int maxJournalSize) {
  // Initialize
  SINFO("Opening " << (readOnly ? "Read Only" : "Writable")
                   << " sqlite connection");
  _filename = filename;
  ...
  SASSERT(
      sqlite3_threadsafe()); // Make sure sqlite is compiled in threadsafe mode.
  ...
  if (!readOnly) {
    ...
    SASSERT(
        !sqlite3_open_v2(filename.c_str(), &_db, DB_WRITE_OPEN_FLAGS, NULL));
    _setPragmas(_db, autoCheckpoint);
    if (SQVerifyTable(_db, "journal", "CREATE TABLE journal ( id INTEGER "
                                      "PRIMARY KEY, query TEXT, hash TEXT )")) {
  ...
  SQuery(_db, "increasing cache size",
         "PRAGMA cache_size = -" + SQ(cacheSize) +
             ";"); // -size means KB; +size means pages

  // Look up the current state of the database.
  SQResult result;
  SASSERT(
      SQuery(_db, "getting commit count",
             "SELECT maxID, (maxID-minID)+1 FROM (SELECT (SELECT MAX(id) FROM "
             "journal) as maxID, (SELECT MIN(id) FROM journal) as minID)",
             result));
  _commitCount = SToUInt64(result[0][0]);
  _journalSize = SToUInt64(result[0][1]);
  const string &query =
      "SELECT hash FROM journal WHERE id=" + SToStr(_commitCount);
  SASSERT(SQuery(_db, "getting DB hash", query.c_str(), result));
  if (!result.empty()) {
    _committedHash = result[0][0];
  }
  DBINFO("Database opened, commitCount="
         << _commitCount << ", journalSize=" << _journalSize << " of "
...
SQLite::~SQLite() {
  ...
  DBINFO("Closing database '" << _filename << "', commitCount=" << _commitCount
                              << "(" << _committedHash << ")");
  ...
  if (!_readOnly) {
    ...
    SQResult result;
    SASSERT(SQuery(_db, "verifying commit count", "SELECT MAX(id) FROM journal",
                   result));
    ...
    if (!_committedHash.empty()) {
      ...
      const string &query =
          (string) "SELECT hash FROM journal WHERE id=" + SToStr(_commitCount);
...
bool SQLite::beginTransaction() {
  ...
  _insideTransaction =
      SQuery(_db, "starting db transaction", "BEGIN TRANSACTION");
...
// --------------------------------------------------------------------------
bool SQLite::verifyTable(const string &tableName, const string &sql,
                         bool &created) {
  SASSERT(!SEndsWith(sql, ";")); // sqlite trims semicolon, so let's not supply
                                 // it else we get confused later
  // First, see if it's there
  SQResult result;
  SASSERT(
      read("SELECT sql FROM sqlite_master WHERE type='table' AND tbl_name=" +
               SQ(tableName) + ";",
           result));
  ...
  if (result.empty()) {
    ...
    if (SStrip(collapsedResult, " ", false) ==
        SStrip(collapsedSQL, " ", false)) {
      // Looking good
      SINFO("'" << tableName << "' already exists with correct schema.");
      return true;
    } else {
      // Not right -- need to upgrade?
      SHMMM("'" << tableName << "' has incorrect schema, need to upgrade? Is '"
                << collapsedResult << "'");
      return false;
    }
  }
...
// --------------------------------------------------------------------------
bool SQLite::addColumn(const string &tableName, const string &column,
                       const string &columnType) {
  // Add a column to the table if it does not exist.  Totally freak out on
  // error.
  const string &sql = SCollapse(
      read("SELECT sql FROM sqlite_master WHERE type='table' AND tbl_name='" +
           tableName + "';"));
  if (!SContains(sql, " " + column + " ")) {
    // Add column
    SINFO("Adding " << column << " " << columnType << " to " << tableName);
    SASSERT(write("ALTER TABLE " + tableName + " ADD COLUMN " + column + " " +
                  columnType + ";"));
    return true;
  }
  SWARN("Schema upgrade failed for table "
        << tableName << ", unrecognized sql '" << sql << "'");
  return false;
}
...
bool SQLite::write(const string &query) {
  ...
  SASSERTWARN(SToUpper(query).find("CURRENT_TIMESTAMP") ==
              string::npos); // Else will be replayed wrong
  // First, check our current state
  SQResult results;
  SASSERT(SQuery(_db, "looking up schema version", "PRAGMA schema_version;",
                 results));
...
bool SQLite::prepare() {
  ...
  uint64_t before = STimeNow();
  bool result =
      SQuery(_db, "updating journal", "INSERT INTO journal VALUES ( NULL, " +
                                          SQ(_uncommittedQuery) + ", " +
                                          SQ(_uncommittedHash) + " )");
  ...
  if (_journalSize + 1 > _maxJournalSize) {
    ...
    SASSERT(SQuery(_db, "Deleting oldest row",
                   "DELETE FROM journal WHERE id=" +
                       SToStr(_commitCount - _journalSize + 1)));
    _writeElapsed += STimeNow() - before;
  }
...
void SQLite::rollback() {
  ...
  if (_insideTransaction) {
    ...
    if (!_uncommittedQuery.empty() &&
        _rollbackElapsed > 1000 * STIME_US_PER_MS) {
      SALERT("[opszy] Freak out! Rolled back an actual query in "
             << (_rollbackElapsed / STIME_US_PER_MS) << "ms.");
    }
  } else {
...
// --------------------------------------------------------------------------
uint64_t SQLite::getLastTransactionTiming(uint64_t &begin, uint64_t &read,
                                          uint64_t &write, uint64_t &prepare,
                                          uint64_t &commit,
                                          uint64_t &rollback) {
...
// --------------------------------------------------------------------------
bool SQLite::getCommits(uint64_t fromIndex, uint64_t toIndex,
                        SQResult &result) {
  ...
  const string &sql = "SELECT hash, query FROM journal WHERE id >= " +
                      SToStr(fromIndex) + " AND id <= " + SToStr(toIndex) +
                      " ORDER BY id";
...
int64_t SQLite::getLastInsertRowID() {
  ...
  SASSERTWARN(SContains(_uncommittedQuery, "INSERT") ||
              SContains(_uncommittedQuery, "REPLACE"));
...
uint64_t SQLite::getCommitCount() {
  if (_readOnly) {
    SQResult result;
    SASSERT(SQuery(_db, "getting commit count", "SELECT MAX(id) FROM journal",
                   result));
...
void SQLite::_setPragmas(sqlite3 *db, int autoCheckpoint) {
  // Occasionally a read-only thread will complain that the database is locked.
  // This is only seems to
  // happen at startup when we open many sqlite3 connections at once.  SQLite
  // will use an exponential
  // backoff to keep trying to query until 1000ms has passed. At that point
  // SQLITE_BUSY is returned.
  SASSERT(SQuery(db, "Setting busy timeout to 1000ms",
                 "PRAGMA busy_timeout = 1000;"));

  // WAL is what allows simultanous read/writing.
  SASSERT(
      SQuery(db, "enabling write ahead logging", "PRAGMA journal_mode = WAL;"));
  ...
  // effective.
  SASSERT(SQuery(db, "new file format for DESC indexes",
                 "PRAGMA legacy_file_format = OFF"));

  // These other pragma only relate to read/write databases.
  if (!_readOnly) {
    SASSERT(SQuery(db, "disabling synchronous commits",
                   "PRAGMA synchronous = OFF;"));
    SASSERT(
        SQuery(db, "disabling change counting", "PRAGMA count_changes = OFF;"));
    SASSERT(autoCheckpoint >= 0);
    DBINFO("Enabling automatic checkpointing every " << SToStr(autoCheckpoint)
                                                     << " pages.");
    SASSERT(SQuery(db, "enabling auto-checkpointing",
                   "PRAGMA wal_autocheckpoint = " + SQ(autoCheckpoint) + ";"));
  }
}
