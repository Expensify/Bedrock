...
SQLite::SQLite( const string& filename, int cacheSize, int autoCheckpoint, bool readOnly, int maxJournalSize )
{
    ...
    SQuery( _db, "increasing cache size", "PRAGMA cache_size = -" + SQ(cacheSize) + ";" ); // -size means KB; +size means pages

    // Look up the current state of the database.
    SQResult result;
    SASSERT(SQuery(_db, "getting commit count", "SELECT maxID, (maxID-minID)+1 FROM (SELECT (SELECT MAX(id) FROM journal) as maxID, (SELECT MIN(id) FROM journal) as minID)", result));
    ...
    DBINFO("Database opened, commitCount=" << _commitCount << ", journalSize=" << _journalSize << " of " << _maxJournalSize << " max (" << _committedHash << ")");
}
...
SQLite::~SQLite( )
{
    ...
    if( !_uncommittedQuery.empty() ) rollback();
...
bool SQLite::addColumn( const string& tableName, const string& column, const string& columnType )
{
    ...
    const string& sql = SCollapse( read( "SELECT sql FROM sqlite_master WHERE type='table' AND tbl_name='" + tableName + "';" ) );
...
SASSERT( _insideTransaction );
    ...
    uint64_t before = STimeNow();
    bool result = SQuery(_db, "updating journal", "INSERT INTO journal VALUES ( NULL, " + SQ(_uncommittedQuery) + ", " + SQ(_uncommittedHash) + " )");
...
SASSERT( !_uncommittedHash.empty() ); // Must prepare first
    ...
    if (_journalSize+1 > _maxJournalSize) {
        ...
        truncating = true;
        uint64_t before = STimeNow();
        SASSERT(SQuery(_db, "Deleting oldest row", "DELETE FROM journal WHERE id=" + SToStr(_commitCount-_journalSize+1)));
        _writeElapsed += STimeNow()-before;
    }
...
void SQLite::rollback( )
{
    ...
    if (_insideTransaction) {
        ...
        if (!_uncommittedQuery.empty() && _rollbackElapsed > 1000*STIME_US_PER_MS ) {
            SALERT("[opszy] Freak out! Rolled back an actual query in " << (_rollbackElapsed/STIME_US_PER_MS) << "ms.");
        }
    } else {
...
// --------------------------------------------------------------------------
uint64_t SQLite::getLastTransactionTiming( uint64_t& begin, uint64_t& read, uint64_t& write, uint64_t& prepare, uint64_t& commit, uint64_t& rollback )
{
...
SASSERTWARN( SWITHIN( 1, id, _commitCount ) );
    ...
    SQResult result;
    if( !SQuery( _db, "getting commit", sql, result ) ) return false;
    SASSERTWARN( result.size() == 1 );
    if( result.size() != 1 ) return false;
    SASSERTWARN( result[0].size() == 2 );
    if( result[0].size() != 2 ) return false;
    query = result[0][0];
    hash  = result[0][1];
...
SASSERTWARN( SWITHIN( fromIndex, toIndex, _commitCount ) );
    ...
    const string& sql = "SELECT hash, query FROM journal WHERE id >= " + SToStr(fromIndex) + " AND id <= " + SToStr(toIndex) + " ORDER BY id" ;
