...
// *jsonCode* Values
// -----------------------
// For consistency, all API commands return response codes in the following
// categories:
//
// ### 2xx Class ###
...
//
// ### 3xx Class ###
// Any response between 300 and 399 means that the request was valid, but
// rejected
...
//
// ### 5xx Class ###
// Any response between 500 and 599 indicates the server experienced some
// internal
...
// * 534 Unexpected HTTP request/response - usually timeout or 500 level server
// error.
//

// --------------------------------------------------------------------------
BedrockNode::BedrockNode(const SData &args, BedrockServer *server_)
    : SQLiteNode(args["-db"], args["-nodeName"], args["-nodeHost"],
                 args.calc("-priority"), args.calc("-cacheSize"),
                 1024, // auto-checkpoint every 1024 pages
                 STIME_US_PER_M * 2 +
                     SRand64() % STIME_US_PER_S * 30, // Be patient first time
                 server_->getVersion(),
                 args.calc("-quorumCheckpoint"), args["-synchronousCommands"],
  ...
  try {
    ...
    SFOREACH(list<Plugin *>, *Plugin::g_registeredPluginList, pluginIt) {
      ...
      if (plugin->enabled() && plugin->peekCommand(this, db, command)) {
        // Peeked it!
        SINFO("Plugin '" << plugin->getName() << "' peeked command '"
                         << request.methodLine << "'");
        pluginPeeked = true;
        break;
      }
    ...
    if (!pluginPeeked) {
      ...
      SINFO("Command '" << request.methodLine
                        << "' is not peekable, queuing for processing.");
      return false; // Not done
    }

    // Success.  If a command has set "content", encode it in the response.
    SINFO("Responding '" << response.methodLine << "' to read-only '"
                         << request.methodLine << "'.");
    if (!content.empty()) {
      ...
      if (response.content != newContent) {
        if (!response.content.empty()) {
          SWARN("Replacing existing response content in "
                << request.methodLine);
        }
    ...
    const string &msg = "Error processing read-only command '" +
                        request.methodLine + "' (" + e + "), ignoring: " +
                        request.serialize();
...
void BedrockNode::_processCommand(SQLite &db, Command *command) {
  ...
  try {
    ...
    if (SIEquals(request.methodLine, "UpgradeDatabase")) {
      ...
      for_each(Plugin::g_registeredPluginList->begin(),
               Plugin::g_registeredPluginList->end(),
      ...
      SFOREACH(list<Plugin *>, *Plugin::g_registeredPluginList, pluginIt) {
        ...
        if (plugin->enabled() && plugin->processCommand(this, db, command)) {
          // Processed it!
          SINFO("Plugin '" << plugin->getName() << "' processed command '"
                           << request.methodLine << "'");
    ...
    SINFO("Responding '" << response.methodLine << "' to '"
                         << request.methodLine << "'.");
    ...
    // Error -- roll back the database and return the error
    db.rollback();
    const string &msg = "Error processing command '" + request.methodLine +
                        "' (" + e + "), ignoring: " + request.serialize();
    if (SContains(e, "_ALERT_"))
      SALERT(msg);
    else if (SContains(e, "_WARN_"))
      SWARN(msg);
...
void BedrockNode::_cleanCommand(Command *command) {
  if (command->httpsRequest) {
    if (command->httpsRequest->owner) {
      ...
      SERROR("No owner for this https request "
             << command->httpsRequest->fullResponse.methodLine);
    }
