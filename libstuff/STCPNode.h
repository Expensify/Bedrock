...
struct STCPNode : public STCPServer
{
    ...
    struct Peer : public SData
    {
        ...
        // Helper methods
        Peer( const string& name_, const string& host_, const STable& params_ ) { reset(); name=name_; host=host_; params=params_; failedConnections=0; }
        bool connected( )           { return( s && s->state==STCP_CONNECTED ); }
        void reset( )               { clear(); s=0; latency=0; }
    };
