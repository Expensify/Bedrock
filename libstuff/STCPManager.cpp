    ...
    if (socket->state != STCP_CLOSED) {
      ...
      if (socket->s < 0) {
        SWARN("Invalid FD number("
              << socket->s
              << "), we're probably about to corrupt stack memory. FD_SETSIZE="
              << FD_SETSIZE);
      }
  ...
  SFOREACH(list<Socket *>, socketList, socketIt) {
    ...
    case STCP_CONNECTING: {
      ...
      if (result) {
        ...
        SDEBUG("Connect to '" << socket->addr << "' failed with SO_ERROR #"
                              << result << ", closing.");
    ...
    case STCP_CONNECTED: {
      ...
      if (socket->ssl) {
        ...
        SDEBUG(
            "sslState=" << SSSLGetState(socket->ssl)
      ...
      if (!aliveAfterRecv || !aliveAfterSend) {
        // How did we die?
        SDEBUG("Connection to '" << socket->addr
                                 << "' died (recv=" << aliveAfterRecv
                                 << ", send=" << aliveAfterSend << ")");
        socket->state = STCP_CLOSED;
      }
      break;
    ...
    case STCP_SHUTTINGDOWN:
      ...
      if (socket->ssl) {
        ...
        if (!aliveAfterSend ||
            (!aliveAfterRecv && socket->sendBuffer.empty())) {
          ...
          if (socket->sendBuffer.empty()) {
            ...
            SWARN("Dirty shutdown of SSL socket '" << socket->addr << "' ("
                                                   << socket->sendBuffer.size()
                                                   << " bytes remain)");
          }
          socket->state = STCP_CLOSED;
        ...
        if (!socket->sendBuffer.empty()) {
          ...
          if (!S_sendconsume(socket->s, socket->sendBuffer)) {
            // Done trying to send
            SHMMM("Unable to finish sending to '"
                  << socket->addr << "' on shutdown, clearing.");
            ::shutdown(socket->s, SHUT_RDWR);
