...
void SHTTPSManager::closeTransaction( Transaction* transaction )
{
    ...
    if( transaction->s ) closeSocket( transaction->s );
    transaction->s = 0;
    SDELETE( transaction );
}
    ...
    while( nextIt != _activeTransactionList.end() )
    {
        ...
        else if( active->s->state > STCP_CONNECTED || elapsed > TIMEOUT )
        {
            ...
            if( active->response == 501 )
            {
                ...
                SHMMM( "SHTTPSManager: '" << active->fullRequest.methodLine << "' sent with no response. We don't know if they processed it!" );
            }
        }
        else
        {
        ...
        {
            // Switch lists
            SINFO( "Completed request '" << active->fullRequest.methodLine << "' to '" << active->fullRequest["Host"] << "' with response '" << active->response << "' in '" << elapsed/STIME_US_PER_MS << "'ms" );
...
SHTTPSManager::Transaction* SHTTPSManager::_httpsSend( const string& url, const SData& request )
{
    ...
    // response.
    string host, path;
    if( !SParseURI( url, host, path ) ) return _createErrorTransaction( );
    if( !SContains( host, ":" ) )
        host += ":443";
    Socket* s = openSocket( host, SStartsWith( url, "https://" ) ? _x509 : 0 );
    if( !s ) return _createErrorTransaction( );
