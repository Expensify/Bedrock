...
// --------------------------------------------------------------------------
SHTTPSManager::SHTTPSManager(const string &pem, const string &srvCrt,
                             const string &caCrt) {
  // Generate the x509 certificate.
...
void SHTTPSManager::postSelect(fd_map &fdm, uint64_t &nextActivity) {
  ...
  while (nextIt != _activeTransactionList.end()) {
    ...
    if (size) {
      ...
      active->finished = now;
      if (SContains(active->fullResponse.methodLine, " 200 ") ||
          active->fullResponse.content.size()) {
      ...
      SWARN("Connection " << (elapsed > TIMEOUT ? "timed out"
                                                : "died prematurely")
    ...
    if (active->response) {
      // Switch lists
      SINFO("Completed request '" << active->fullRequest.methodLine << "' to '"
                                  << active->fullRequest["Host"]
                                  << "' with response '" << active->response
                                  << "' in '" << elapsed / STIME_US_PER_MS
                                  << "'ms");
...
SHTTPSManager::Transaction *SHTTPSManager::_createErrorTransaction() {
  ...
  SWARN("We had to create an error transaction instead of attempting a real "
        "one.");
...
// --------------------------------------------------------------------------
SHTTPSManager::Transaction *SHTTPSManager::_httpsSend(const string &url,
                                                      const SData &request) {
