...
uint64_t STimeThisBeginningOfMonth() {
    // Get today's date in GMT, zero out the hour, make it the 1st, convert into a Unix
    // timestamp, and then into a libstuff timestamp.
    time_t loNow;
    time(&loNow);
    uint64_t hiNow = STimeNow();
...
uint64_t STimeThisBeginningOfMonthPlusOneYear() {
    // Get today's date in GMT, zero out the hour, make it the 1st, add 1 year, convert into a Unix
    // timestamp, and then into a libstuff timestamp.
    time_t loNow;
    time(&loNow);
    uint64_t hiNow = STimeNow();
