...
string SComposeTime(const string &format, uint64_t when) {
  // Convert from high-precision time (usec) to standard low-precision time
  // (sec), then format and return
...
timeval SToTimeval(uint64_t when) {
  ...
  return {(time_t)(when / STIME_US_PER_S),
          (suseconds_t)(when % STIME_US_PER_S)};
}
