...
SX509 *SX509Open(const string &pem, const string &srvCrt, const string &caCrt) {
  ...
  const char *srvCrtPtr =
      (srvCrt.empty() ? mbedtls_test_srv_crt : srvCrt.c_str());
  ...
  try {
    ...
    if (mbedtls_pk_parse_key(&x509->pk, (unsigned char *)pemPtr,
                             (int)strlen(pemPtr) + 1, NULL, 0)) {
      throw "parsing key";
    }
    if (mbedtls_x509_crt_parse(&x509->srvcert, (unsigned char *)srvCrtPtr,
                               (int)strlen(srvCrtPtr) + 1)) {
      throw "parsing server certificate";
    }
    if (mbedtls_x509_crt_parse(&x509->srvcert, (unsigned char *)caCrtPtr,
                               (int)strlen(caCrtPtr) + 1)) {
