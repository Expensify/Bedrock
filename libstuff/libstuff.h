...
#define SFOREACHMAP( _CT0_, _CT1_, _C_, _I_ )         for(map< _CT0_ , _CT1_ >::iterator _I_ = (_C_).begin( ); _I_ != (_C_).end( ); ++_I_)
#define SFOREACHMAPREVERSE( _CT0_, _CT1_, _C_, _I_ )  for(map< _CT0_ , _CT1_ >::reverse_iterator _I_ = (_C_).rbegin( ); _I_ != (_C_).rend( ); ++_I_)
...
#define SASSERT( _LHS_ )                  do { if(!(_LHS_) )          { SERROR("Assertion failed: (" << #_LHS_ << ") != true"); abort();                                             }} while(false)
#define SASSERTEQUALS( _LHS_, _RHS_ )     do { if((_LHS_) != (_RHS_) ){ SERROR("Assertion failed: (" << #_LHS_ << ") != (" << #_RHS_ << "): (" << _LHS_ << ") != (" << _RHS_ << ")");}} while(false)
#define SASSERTWARN( _LHS_ )              do { if(!(_LHS_) )          { SWARN ("Assertion failed: (" << #_LHS_ << ") != true");                                                      }} while(false)
#define SASSERTWARNEQUALS( _LHS_, _RHS_ ) do { if((_LHS_) != (_RHS_) ){ SWARN ("Assertion failed: (" << #_LHS_ << ") != (" << #_RHS_ << "): (" << _LHS_ << ") != (" << _RHS_ << ")");}} while(false)
#define SASSERTTHROW(condition, uuid)     do { if(!(condition))       { throw AssertionFailedException(#condition, uuid);                                                            }} while(false)
...
{
public:
    bool operator()(const string& s1, const string& s2) const { return lexicographical_compare(s1.begin(), s1.end(), s2.begin(), s2.end(), nocase_compare()); }
private:
...
struct SStopwatch
{
    ...
    // from construction.
    SStopwatch( )                { start(); alarmDuration=0; }
    SStopwatch( uint64_t alarm ) { startTime=0; alarmDuration=alarm; }
    ...
    bool ding( )  { if( !ringing() ) return false; start(); return true; }
};
...
inline void SLogLevel( int level ) { _g_SLogMask = LOG_UPTO( level ); setlogmask( _g_SLogMask ); }
...
#define SSYSLOG( _PRI_, _MSG_ ) do { SThreadLocalStorage* tls=SThreadGetLocalStorage(); if( _g_SLogMask & (1<<(_PRI_)) ) { ostringstream __out; __out << _MSG_ << endl; const string& __s = __out.str( ); if( _g_SLogToSTDOUT ) printf( "%s", __s.c_str( ) ); else for( int __i = 0; __i < (int)__s.size( ); __i += 1500 ) syslog( LOG_WARNING, "%s", __s.substr( __i, 1500 ).c_str( ) ); } } while( false )

#define SWHEREAMI               tls->logPrefix << "(" << basename( (char *)__FILE__) << ":" << __LINE__ << ") " << __FUNCTION__ << " [" << tls->name << "] "
...
#define SERROR( _MSG_ )         do { SSYSLOG( LOG_ERR, SWHEREAMI << "[eror] " << SLOGPREFIX << _MSG_ ); SLogStackTrace( ); fflush( stdout ); exit( 1 ); } while( false )
...
inline SThreadLocalStorage* SThreadGetLocalStorage() { return (SThreadLocalStorage*)pthread_getspecific(_g_SThread_TLSKey); }

// Thread-local log prefix
inline void SLogSetThreadPrefix(const string& logPrefix) { SThreadLocalStorage* tls=SThreadGetLocalStorage(); tls->logPrefix=logPrefix; }
...
struct SAutoMutex
{
    ...
    // Locks on creation
    SAutoMutex( void* mutex_ ) { SASSERT( mutex_ ); mutex=mutex_; SMutexLock( mutex ); }
    
    // Unlocks on destruction
    ~SAutoMutex( ) { SASSERT( mutex ); SMutexUnlock( mutex ); }
};
...
public:
    ...
    SSynchronized( const T& val ) { _mutex = SMutexOpen( ); _synchronizedValue = val; }
    ~SSynchronized( ) { SMutexClose( _mutex ); }

    // Getter and setter
    T    get( )            { SAUTOLOCK( _mutex ); return _synchronizedValue; }
    void set(const T& val) { SAUTOLOCK( _mutex ); _synchronizedValue = val; }
...
#define SDELETE( _PTR_ ) do { if( _PTR_ ) { delete _PTR_; _PTR_ = 0; } } while( false )
...
inline uint64_t SRand64( ) { return ((SRand15( ) << 60) | (SRand15( ) << 45) | (SRand15( ) << 30) | (SRand15( ) << 15) | (SRand15( ) << 0)); }
...
template <class T> inline string SToStr( const T& t ) { ostringstream ss; ss << t; return ss.str( ); }
...
template <class A, class B, class C> inline bool SContains( const map<A,B,C>& nameValueMap, const A& name ) { return( nameValueMap.find( name ) != nameValueMap.end( ) ); }
template <class A> inline bool SContains( const list<A>& valueList, const A& value ) { return ::find( valueList.begin(), valueList.end(), value ) != valueList.end(); }
template <class A> inline bool SContains( const set<A>& valueList,  const A& value ) { return ::find( valueList.begin(), valueList.end(), value ) != valueList.end(); }
...
inline bool SEndsWith(   const string& haystack, const string& needle ) { if( needle.size( ) > haystack.size( ) ) return false; else return( haystack.substr( haystack.size()-needle.size() ) == needle ); }
...
inline bool SREMatch( const string& regExp, const string& s, string& match) { return pcrecpp::RE(regExp).FullMatch(s,&match); }
...
inline string SToLower( const string& value ) { string out; out.resize( value.size( ) ); for( int c=0; c<(int)value.size( ); ++c ) out[c] = SToLower( value[c] ); return out; }
inline string SToUpper( const string& value ) { string out; out.resize( value.size( ) ); for( int c=0; c<(int)value.size( ); ++c ) out[c] = SToUpper( value[c] ); return out; }
...
inline string SEscape     ( const string& lhs, const string& unsafe, char escaper = '\\' ) { return SEscape( lhs.c_str(), unsafe, escaper ); }
...
inline string SBefore     ( const string& value, const string& needle )                        { size_t pos = value.find( needle );         if( pos == string::npos ) return ""; else return value.substr( 0, pos ); }
inline string SAfter      ( const string& value, const string& needle )                        { size_t pos = value.find( needle );         if( pos == string::npos ) return ""; else return value.substr( pos + needle.size( ) ); }
inline string SAfterLastOf( const string& value, const string& needle )                        { size_t pos = value.find_last_of( needle ); if( pos == string::npos ) return ""; else return value.substr( pos + 1 ); }
inline string SBetween    ( const string& value, const string& lhs, const string& rhs )        { return SBefore( SAfter( value, lhs ), rhs ); }
inline bool   SInsertAfter(       string& value, const string& needle, const string& thread  ) { size_t pos = value.find( needle );         if( pos == string::npos ) return false; value.insert( pos+needle.size( ), thread ); return true; }
inline string SAfterUpTo  ( const string& value, const string& after,  const string& upTo    ) { return( SBefore( SAfter( value, after ), upTo ) ); }
...
inline void SConsumeBack ( string& lhs, int num ) { if( (int)lhs.size( ) <= num ) { lhs.clear( ); } else { lhs = lhs.substr( 0, lhs.size( )-num ); } }
inline void SAppend( string& lhs, const char* rhs, int num ) { size_t oldSize=lhs.size(); lhs.resize( oldSize+num ); memcpy( &lhs[oldSize], rhs, num ); }
...
inline int    SParseHTTP( const string& buffer, string& methodLine, STable& nameValueMap, string& content ) { return SParseHTTP( buffer.c_str( ), (int)buffer.size( ), methodLine, nameValueMap, content ); }
...
inline bool   SParseURI( const string& uri, string& host, string& path ) { return SParseURI( uri.c_str( ), (int)uri.size( ), host, path ); }
extern bool   SParseURIPath( const char* buffer, int length, string& path, STable& nameValueMap );
inline bool   SParseURIPath( const string& uri, string& path, STable& nameValueMap ) { return SParseURIPath( uri.c_str( ), (int)uri.size( ), path, nameValueMap ); }
extern void   SComposeHTTP( string& buffer, const string& methodLine, const STable& nameValueMap, const string& content );
inline string SComposeHTTP( const string& methodLine, const STable& nameValueMap, const string& content ) { string buffer; SComposeHTTP( buffer, methodLine, nameValueMap, content ); return buffer; }
...
inline bool   SHostIsValid( const string& host ) { string domain; uint16_t port=0; return SParseHost( host, domain, port ); }
inline bool   SURIIsValid( const string& uri ) { string host, path; return SParseURI( uri, host, path ); }
inline string SGetDomain( const string& host ) { string domain; uint16_t ignore; if( SParseHost( host, domain, ignore ) ) return domain; else return host; }
...
inline bool          SParseIntegerList(const string& value, list<int64_t>& valueList, char separator = ',') { return SParseIntegerList(value.c_str(), valueList, separator); }
inline list<int64_t> SParseIntegerList(const string& value, char separator = ',')                           { list<int64_t> valueList; SParseIntegerList(value, valueList, separator); return valueList; }
...
inline bool           SParseList  ( const string& value, list<string>&   valueList,   char separator = ',' ) { return SParseList( value.c_str( ), valueList, separator ); }
inline bool           SParseVector( const string& value, vector<string>& valueVector, char separator = ',' ) { return SParseVector( value.c_str( ), valueVector, separator ); }
inline list<string>   SParseList  ( const string& value,                              char separator = ',' ) { list<string> valueList; SParseList( value, valueList, separator ); return valueList; }
inline vector<string> SParseVector( const string& value,                              char separator = ',' ) { vector<string> valueVector; SParseVector( value, valueVector, separator ); return valueVector; }
...
template <typename T>
string SComposeList(const T& valueList, const string& separator = ", ")
{
    if (valueList.empty()) {
        return "";
    }
...
inline void SAppendToList( string& value, const string& addValue ) { list<string> valueList; SParseList( value, valueList ); valueList.push_back( addValue ); value = SComposeList( valueList ); }
...
inline string       SComposeJSONArray( const vector<string>& valueList ) { if( valueList.empty( ) )    return "[]"; string working = "["; SFOREACHCONST( vector<string>, valueList, valueIt ) working += SToJSON(*valueIt) + ",";                                      working.resize( working.size( )-1 ); working += "]"; return working; }
inline string       SComposeJSONArray( const list<string>& valueList )   { if( valueList.empty( ) )    return "[]"; string working = "["; SFOREACHCONST( list<string>, valueList, valueIt )   working += SToJSON(*valueIt) + ",";                                      working.resize( working.size( )-1 ); working += "]"; return working; }
...
inline string       SGetJSONArrayFront( const string& jsonArray ) { list<string> l = SParseJSONArray( jsonArray ); return l.empty( ) ? "" : l.front( ); };
...
inline string SToStr( const sockaddr_in& addr ) { return SToStr( inet_ntoa( addr.sin_addr ) ) + ":" + SToStr( ntohs( addr.sin_port ) ); }
...
inline string  S_recv( int s ) { string buf; S_recvappend( s, buf ); return buf; }
extern bool    S_sendconsume( int s, string& sendBuffer );
inline bool    S_send( int s, string sendBuffer ) { S_sendconsume( s, sendBuffer ); return sendBuffer.empty(); }
...
inline string   SFileLoad( const string& path ) { string buffer; SFileLoad( path, buffer ); return buffer; }
...
string SQList(const Container& valueList)
{
...
extern bool SQuery( sqlite3* db, const char* e, const string& sql, SQResult& result, int64_t warnThreshold = 1000*STIME_US_PER_MS );
inline bool SQuery( sqlite3* db, const char* e, const string& sql, int64_t warnThreshold = 1000*STIME_US_PER_MS ) { SQResult ignore; return SQuery( db, e, sql, ignore, warnThreshold ); }
...
#define STESTLOG( _MSG_ ) do { cout << _MSG_ << endl; cout.flush(); SSYSLOG( LOG_DEBUG,   SWHEREAMI << "[test] " << SLOGPREFIX << _MSG_ ); } while(false)
#define STESTASSERT( _COND_ )     do { if( !(_COND_) ) { STESTLOG( (test.success?"\n":"") << "\tAssertion failed: " << #_COND_ ); test.success=false; } } while( false )
#define STESTEQUALS( _LHS_, _RHS_ )     do { if( (_LHS_) != (_RHS_) ) { STESTLOG( (test.success?"\n":"") << "\tAssertion failed: (" << #_LHS_ << ") != (" << #_RHS_ << "): (" << _LHS_ << ") != (" << _RHS_ << ")" ); test.success=false; } } while( false )
struct STestGroup
{
    bool success;
...
struct STestTimer
{
    ...
    // Starts a new test
    STestTimer( const string& message, STestGroup* testGroup = 0 ) : begin( STimeNow() ), success( true ), testGroup( testGroup )
    {
    ...
    ~STestTimer( )
    {
        ...
        if (testGroup) { testGroup->success &= success; }
