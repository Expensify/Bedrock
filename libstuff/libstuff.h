...
// _C_  : Container
// _I_  : Iterator
#define SFOREACH(_CT_, _C_, _I_)                                               \
  for (_CT_::iterator _I_ = (_C_).begin(); _I_ != (_C_).end(); ++_I_)
#define SFOREACHREVERSE(_CT_, _C_, _I_)                                        \
  for (_CT_::reverse_iterator _I_ = (_C_).rbegin(); _I_ != (_C_).rend(); ++_I_)
#define SFOREACHCONST(_CT_, _C_, _I_)                                          \
  for (_CT_::const_iterator _I_ = (_C_).begin(); _I_ != (_C_).end(); ++_I_)
#define SFOREACHMAP(_CT0_, _CT1_, _C_, _I_)                                    \
  for (map<_CT0_, _CT1_>::iterator _I_ = (_C_).begin(); _I_ != (_C_).end();    \
       ++_I_)
#define SFOREACHMAPREVERSE(_CT0_, _CT1_, _C_, _I_)                             \
  for (map<_CT0_, _CT1_>::reverse_iterator _I_ = (_C_).rbegin();               \
       _I_ != (_C_).rend(); ++_I_)
#define SFOREACHMAPCONST(_CT0_, _CT1_, _C_, _I_)                               \
  for (map<_CT0_, _CT1_>::const_iterator _I_ = (_C_).begin();                  \
       _I_ != (_C_).end(); ++_I_)
#define SFOREACHTABLE(_C_, _I_)                                                \
  for (STable::const_iterator _I_ = (_C_).begin(); _I_ != (_C_).end(); ++_I_)
...
#define SASSERTEQUALS(_LHS_, _RHS_)                                            \
  do {                                                                         \
    if ((_LHS_) != (_RHS_)) {                                                  \
      SERROR("Assertion failed: (" << #_LHS_ << ") != (" << #_RHS_ << "): ("   \
                                   << _LHS_ << ") != (" << _RHS_ << ")");      \
    }                                                                          \
  } while (false)
...
#define SASSERTWARNEQUALS(_LHS_, _RHS_)                                        \
  do {                                                                         \
    if ((_LHS_) != (_RHS_)) {                                                  \
      SWARN("Assertion failed: (" << #_LHS_ << ") != (" << #_RHS_ << "): ("    \
                                  << _LHS_ << ") != (" << _RHS_ << ")");       \
    }                                                                          \
  } while (false)
...
struct SData {
  ...
  // Create an SData object; if no Content-Length then take everything as the
  // content
...
extern uint64_t
STimeThisMorning(); // Timestamp for this morning at midnight GMT
...
// **NOTE: rsyslog max line size is 2048 bytes.  We split on 1500 byte
// bounderies in order to fit the
...
#define SWHEREAMI                                                              \
  tls->logPrefix << "(" << basename((char *)__FILE__) << ":" << __LINE__       \
                 << ") " << __FUNCTION__ << " [" << tls->name << "] "

#define SLOGPREFIX ""
#define SLOG(_MSG_) SSYSLOG(LOG_DEBUG, SWHEREAMI << SLOGPREFIX << _MSG_)
#define SDEBUG(_MSG_)                                                          \
  SSYSLOG(LOG_DEBUG, SWHEREAMI << "[dbug] " << SLOGPREFIX << _MSG_)
#define SINFO(_MSG_)                                                           \
  SSYSLOG(LOG_INFO, SWHEREAMI << "[info] " << SLOGPREFIX << _MSG_)
#define SHMMM(_MSG_)                                                           \
  SSYSLOG(LOG_WARNING, SWHEREAMI << "[hmmm] " << SLOGPREFIX << _MSG_)
#define SWARN(_MSG_)                                                           \
  SSYSLOG(LOG_WARNING, SWHEREAMI << "[warn] " << SLOGPREFIX << _MSG_)
#define SALERT(_MSG_)                                                          \
  SSYSLOG(LOG_WARNING, SWHEREAMI << "[alrt] " << SLOGPREFIX << _MSG_)
#define SERROR(_MSG_)                                                          \
  do {                                                                         \
...
#define STRACE()                                                               \
  SLOG("[trac] " << __FILE__ << "(" << __LINE__ << ") :" << __FUNCTION__)
...
void *SThreadOpen(void (*proc)(void *procData), void *procData,
                  const string &threadName = "", size_t stackSize = 0);
...
#define SAUTOPREFIX(_PREFIX_)                                                  \
  SAutoThreadPrefix __SAUTOPREFIX##__LINE__(_PREFIX_)
...
#define SWITHIN(_MIN_, _VAL_, _MAX_)                                           \
  (((_MIN_) <= (_VAL_)) && ((_VAL_) <= (_MAX_)))
...
inline uint64_t SRand64() {
  return ((SRand15() << 60) | (SRand15() << 45) | (SRand15() << 30) |
          (SRand15() << 15) | (SRand15() << 0));
}
...
inline bool SContains(const map<A, B, C> &nameValueMap, const A &name) {
  return (nameValueMap.find(name) != nameValueMap.end());
}
template <class A>
inline bool SContains(const list<A> &valueList, const A &value) {
  return ::find(valueList.begin(), valueList.end(), value) != valueList.end();
}
template <class A>
inline bool SContains(const set<A> &valueList, const A &value) {
  return ::find(valueList.begin(), valueList.end(), value) != valueList.end();
}
...
inline string SEscape(const string &lhs, const string &unsafe,
                      char escaper = '\\') {
...
inline string SBetween(const string &value, const string &lhs,
                       const string &rhs) {
  return SBefore(SAfter(value, lhs), rhs);
}
inline bool SInsertAfter(string &value, const string &needle,
                         const string &thread) {
  size_t pos = value.find(needle);
  if (pos == string::npos)
    return false;
...
inline string SAfterUpTo(const string &value, const string &after,
                         const string &upTo) {
  return (SBefore(SAfter(value, after), upTo));
}
extern string SReplace(const string &value, const string &find,
                       const string &replace);
extern string SReplaceAllBut(const string &value, const string &safeChars,
                             char replaceChar);
extern string SReplaceAll(const string &value, const string &unsafeChars,
                          char replaceChar);
extern int SStateNameToInt(const char *states[], const string &stateName,
...
extern int SParseHTTP(const char *buffer, size_t length, string &methodLine,
                      STable &nameValueMap, string &content);
inline int SParseHTTP(const string &buffer, string &methodLine,
                      STable &nameValueMap, string &content) {
  return SParseHTTP(buffer.c_str(), (int)buffer.size(), methodLine,
                    nameValueMap, content);
}
extern bool SParseRequestMethodLine(const string &methodLine, string &method,
                                    string &uri);
extern bool SParseResponseMethodLine(const string &methodLine, string &protocol,
                                     int &code, string &reason);
extern bool SParseURI(const char *buffer, int length, string &host,
                      string &path);
inline bool SParseURI(const string &uri, string &host, string &path) {
  return SParseURI(uri.c_str(), (int)uri.size(), host, path);
}
extern bool SParseURIPath(const char *buffer, int length, string &path,
                          STable &nameValueMap);
inline bool SParseURIPath(const string &uri, string &path,
                          STable &nameValueMap) {
  return SParseURIPath(uri.c_str(), (int)uri.size(), path, nameValueMap);
}
extern void SComposeHTTP(string &buffer, const string &methodLine,
                         const STable &nameValueMap, const string &content);
inline string SComposeHTTP(const string &methodLine, const STable &nameValueMap,
...
// --------------------------------------------------------------------------
// List management
extern bool SParseIntegerList(const char *value, list<int64_t> &valueList,
                              char separator = ',');
inline bool SParseIntegerList(const string &value, list<int64_t> &valueList,
                              char separator = ',') {
  return SParseIntegerList(value.c_str(), valueList, separator);
}
inline list<int64_t> SParseIntegerList(const string &value,
                                       char separator = ',') {
...
extern bool SParseList(const char *value, list<string> &valueList,
                       char separator = ',');
extern bool SParseVector(const char *value, vector<string> &valueVector,
                         char separator = ',');
inline bool SParseList(const string &value, list<string> &valueList,
                       char separator = ',') {
  return SParseList(value.c_str(), valueList, separator);
}
inline bool SParseVector(const string &value, vector<string> &valueVector,
                         char separator = ',') {
...
string SComposeList(const T &valueList, const string &separator = ", ") {
  ...
  string working;
  for_each(valueList.begin(), valueList.end(),
           [&](decltype(typename T::value_type()) value) {
...
inline string SComposeJSONArray(const vector<string> &valueList) {
  if (valueList.empty())
    return "[]";
  string working = "[";
  SFOREACHCONST(vector<string>, valueList, valueIt)
  working += SToJSON(*valueIt) + ",";
...
inline string SComposeJSONArray(const list<string> &valueList) {
  if (valueList.empty())
    return "[]";
  string working = "[";
  SFOREACHCONST(list<string>, valueList, valueIt)
  working += SToJSON(*valueIt) + ",";
  working.resize(working.size() - 1);
  working += "]";
  return working;
}
extern string SComposeJSONObject(const STable &nameValueMap,
                                 const bool forceString = false);
...
// This will add the events specified in `evts` to the events we'll listen for
// for this socket,
// or, if this socket isn't in our set, it'll add it.
extern void SFDset(fd_map &fdm, int socket, short evts);

// Returns true if *ANY* of the bits in evts are set as returned value for this
// socket.
// Returns false otherwise, or if this socket isn't in this fd_set, or if evts
// is 0.
extern bool SFDAnySet(fd_map &fdm, int socket, short evts);

// Socket helpers
extern int S_socket(const string &host, bool isTCP, bool isPort,
                    bool isBlocking);
extern int S_accept(int port, sockaddr_in &fromAddr, bool isBlocking);
extern ssize_t S_recvfrom(int s, char *recvBuffer, int recvBufferSize,
...
extern string SAESEncrypt(const string &buffer, const string &iv,
                          const string &key);
extern string SAESDecrypt(const string &buffer, const string &iv,
                          const string &key);
...
inline string SMaskPAN(const string &pan) {
  ...
  return safePAN.substr(0, 6) + string(safePAN.size() - 10, 'X') +
         safePAN.substr(safePAN.size() - 4);
}
...
template <typename Container> string SQList(const Container &valueList) {
  ...
  for (typename Container::const_iterator valueIt = valueList.begin();
       valueIt != valueList.end(); ++valueIt) {
...
extern bool SQuery(sqlite3 *db, const char *e, const string &sql,
                   SQResult &result,
                   int64_t warnThreshold = 1000 * STIME_US_PER_MS);
inline bool SQuery(sqlite3 *db, const char *e, const string &sql,
                   int64_t warnThreshold = 1000 * STIME_US_PER_MS) {
  SQResult ignore;
...
#define SQUERY(_db_, _query_, _result_)                                        \
  SQuery(_db_, __FILE__ __SLINE__, (string)_query_, _result_)
#define SQUERYIGNORE(_db_, _query_)                                            \
  SQuery(_db_, __FILE__ __SLINE__, (string)_query_)
#define SASSERTQUERY(_db_, _query_, _result_)                                  \
  SASSERT(SQUERY(_db_, _query_, _result_))
#define SASSERTQUERYIGNORE(_db_, _query_) SASSERT(SQUERYIGNORE(_db_, _query_))
extern bool SQVerifyTable(sqlite3 *db, const string &tableName,
                          const string &sql);
