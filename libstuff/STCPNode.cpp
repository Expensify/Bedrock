...
STCPNode::~STCPNode( )
{
    ...
    SFOREACH( list<Socket*>, acceptedSocketList, socketIt ) closeSocket( *socketIt );
    acceptedSocketList.clear( );
    SFOREACH( list<Peer*>, peerList, peerIt )
    {
        // Shut down the peer
        Peer* peer = *peerIt;
        if( peer->s ) closeSocket( peer->s );
        SDELETE( peer );
    }
    peerList.clear( );
}
...
void STCPNode::postSelect( fd_map& fdm, uint64_t& nextActivity )
{
    ...
    Socket* socket = 0;
    while( (socket = acceptSocket()) ) acceptedSocketList.push_back( socket );
    ...
    while( nextSocketIt != acceptedSocketList.end() )
    {
        ...
        try
        {
            ...
            if( socket->state != STCP_CONNECTED ) throw "premature disconnect";
            ...
            if( (messageSize = message.deserialize( socket->recvBuffer )) )
            {
                ...
                else throw "expecting NODE_LOGIN";
            }
        }
        catch( const char* e )
        {
            ...
            if( socket->recvBuffer.empty() && socket->sendBuffer.empty() )
                ...
                SWARN( "Incoming connection failed from '" << socket->addr << "' (" << e << "), recv='" << socket->recvBuffer << "', send='" << socket->sendBuffer << "'" );
            closeSocket( socket );
            acceptedSocketList.erase( socketIt );
        }
    }
        ...
        if( peer->s )
        {
            ...
            switch( peer->s->state )
            {
                case STCP_CONNECTED:
                {
                    ...
                    {
                        // Error -- reconnect
                        PWARN( "Error processing message '" << message.methodLine << "' (" << e << "), reconnecting:" << message.serialize() );
                ...
                case STCP_CLOSED :
                {
                    ...
                    if( peer->s->connectFailure )
                        PINFO( "Peer connection failed after " << (STimeNow()-peer->s->openTime)/STIME_US_PER_MS << "ms, reconnecting in " << delay/STIME_US_PER_MS << "ms" );
                    else
                        PHMMM( "Lost peer connection after " << (STimeNow()-peer->s->openTime)/STIME_US_PER_MS << "ms, reconnecting in " << delay/STIME_US_PER_MS << "ms" );
                    _onDisconnect( peer );
                    if( peer->s->connectFailure ) peer->failedConnections++;
                    closeSocket( peer->s );
                    peer->reset( );
