...
// --------------------------------------------------------------------------
STCPNode::STCPNode(const string &name_, const string &host,
                   const uint64_t recvTimeout_)
    : STCPServer(host) {
  // Initialize
  name = name_;
...
// --------------------------------------------------------------------------
void STCPNode::addPeer(const string &peerName, const string &host,
                       const STable &params) {
  ...
  peer->nextReconnect =
      STimeNow() +
      SRand64() %
          (STIME_US_PER_S * 2); // Wait up to 2s before trying the first time
  ...
  while (nextSocketIt != acceptedSocketList.end()) {
    ...
    try {
      ...
      if ((messageSize = message.deserialize(socket->recvBuffer))) {
        ...
        if (SIEquals(message.methodLine, "NODE_LOGIN")) {
          ...
          if (!foundIt) {
            ...
            SWARN("Unauthenticated node '"
                  << message["Name"] << "' attempted to connected, rejecting.");
      ...
      if (socket->recvBuffer.empty() && socket->sendBuffer.empty())
        SDEBUG("Incoming connection failed from '" << socket->addr << "' (" << e
                                                   << "), empty buffers");
      else
        SWARN("Incoming connection failed from '"
              << socket->addr << "' (" << e << "), recv='" << socket->recvBuffer
  ...
  SFOREACH(list<Peer *>, peerList, peerIt) {
    ...
    if (peer->s) {
      ...
      case STCP_CONNECTED: {
        ...
        int messageSize = 0;
        try {
          if (peer->s->lastRecvTime &&
              peer->s->lastRecvTime + recvTimeout < STimeNow()) {
            // Reset and reconnect.
          ...
          if (STimeNow() - peer->s->lastSendTime >
              recvTimeout - 5 * STIME_US_PER_S) {
          ...
          while ((messageSize = message.deserialize(peer->s->recvBuffer))) {
            ...
            if (SIEquals(message.methodLine, "PING")) {
              ...
              SINFO("Received PING from peer '" << peer->name
                                                << "'. Sending PONG.");
              SData pong("PONG");
              ...
              SINFO("Received PONG from peer '"
                    << peer->name << "' (" << peer->latency / STIME_US_PER_MS
                    << "ms latency)");
            } else {
          ...
          // Error -- reconnect
          PWARN("Error processing message '"
                << message.methodLine << "' (" << e
      ...
      case STCP_CLOSED: {
        ...
        if (peer->s->connectFailure)
          PINFO("Peer connection failed after "
                << (STimeNow() - peer->s->openTime) / STIME_US_PER_MS
                << "ms, reconnecting in " << delay / STIME_US_PER_MS << "ms");
        else
          PHMMM("Lost peer connection after "
                << (STimeNow() - peer->s->openTime) / STIME_US_PER_MS
                << "ms, reconnecting in " << delay / STIME_US_PER_MS << "ms");
        _onDisconnect(peer);
        if (peer->s->connectFailure)
      ...
      if (STimeNow() > peer->nextReconnect) {
        ...
        if (peer->s) {
          ...
          SWARN("Failed to open socket '" << peer->host
                                          << "', trying again in 60s");
          peer->failedConnections++;
