...
SSSLState *SSSLOpen(int s, SX509 *x509) {
  ...
  mbedtls_ssl_config_defaults(&state->conf, MBEDTLS_SSL_IS_CLIENT,
  ...
  mbedtls_ssl_set_bio(&state->ssl, &state->s, mbedtls_net_send,
                      mbedtls_net_recv, 0);

  if (x509) {
    // Add the certificate
    mbedtls_ssl_conf_ca_chain(&state->conf, x509->srvcert.next, 0);
    SASSERT(mbedtls_ssl_conf_own_cert(&state->conf, &x509->srvcert,
                                      &x509->pk) == 0);
...
int SSSLSend(SSSLState *sslState, const char *buffer, int length) {
  ...
  const int numSent =
      mbedtls_ssl_write(&sslState->ssl, (unsigned char *)buffer, length);
  if (numSent > 0) {
    return numSent;
  }
...
int SSSLRecv(SSSLState *sslState, char *buffer, int length) {
  ...
  SASSERT(sslState && buffer);
  const int numRecv =
      mbedtls_ssl_read(&sslState->ssl, (unsigned char *)buffer, length);
  if (numRecv > 0) {
    return numRecv;
  }
...
bool SSSLSendAll(SSSLState *ssl, const string &buffer) {
  ...
  while (totalSent < (int)buffer.size()) {
    int numSent =
        SSSLSend(ssl, &buffer[totalSent], (int)buffer.size() - totalSent);
    if (numSent == -1) {
      return false;
    }
    totalSent += numSent;
