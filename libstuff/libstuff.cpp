...
uint64_t SFromHex( const string& value )
{
    ...
    for( int c=0; c<(int)value.size( ); ++c )
    {
        ...
        binValue <<= 4;
             if( '0' <= value[c] && value[c] <= '9' ) binValue += value[c] - '0';
        else if( 'a' <= value[c] && value[c] <= 'f' ) binValue += value[c] - 'a' + 10;
        else if( 'A' <= value[c] && value[c] <= 'F' ) binValue += value[c] - 'A' + 10;
        else
        {
...
string SStrFromHex( const string& buffer )
{
    ...
    for( size_t c=0; c<buffer.size(); c+=2 )
    {
        ...
        else if( a <  'A' ) a = 0; // Invalid
        else if( a <= 'F' ) a = a-'A' + 10;
        else if( a <  'a' ) a = 0; // Invalid
        else if( a <= 'f' ) a = a-'a' + 10;
        else                a = 0; // Invalid
        a <<= 4;
        ...
        else if( b <  'A' ) b = 0; // Invalid
        else if( b <= 'F' ) b = b-'A' + 10;
        else if( b <  'a' ) b = 0; // Invalid
        else if( b <= 'f' ) b = b-'a' + 10;
        else                b = 0; // Invalid
...
string SClampSize( const string& in, int digits, char fill )
{
    ...
    if( (int)in.size() >= digits ) return in.substr( 0, digits );
...
string STrim( const string& lhs )
{
    ...
    size_t front = 0;
    while( front < lhs.size( ) && isspace( lhs[front] ) ) ++front;
    size_t back = lhs.size( )-1;
    while( back != string::npos && isspace( lhs[back] ) ) --back;
    if( front <= back ) return lhs.substr( front, back-front+1 );
    else                return "";
}
...
string SReplaceAllBut( const string& value, const string& safeChars, char replaceChar )
{
    ...
    string out;
    for( size_t c=0; c<value.size(); ++c )
        if( safeChars.find( value[c] ) != string::npos ) out += value[c];
        else                                             out += replaceChar;
    return out;
}
...
string SReplaceAll( const string& value, const string& unsafeChars, char replaceChar )
{
    ...
    string out;
    for( size_t c=0; c<value.size(); ++c )
        if( unsafeChars.find( value[c] ) == string::npos ) out += value[c];
        else                                               out += replaceChar;
    return out;
}
...
bool SParseList( const char* ptr, list<string>& valueList, char separator )
{
    ...
    while( *ptr )
    {
        ...
        {
            // Only add if the component is non-empty
            if( !component.empty( ) ) valueList.push_back( component );
            component.clear( );
        }
...
bool SParseVector( const char* ptr, vector<string>& valueVector, char separator )
{
    ...
    while( *ptr )
    {
        ...
        {
            // Only add if the component is non-empty
            if( !component.empty( ) ) valueVector.push_back( component );
            component.clear( );
        }
...
SData SParseCommandLine( int argc, char* argv[] )
{
    ...
    for( int c=1; c<argc; ++c )
    {
        ...
        if( name.empty( ) )
        {
            ...
            if( isName ) name = argv[c];
            else         SAppendToList( results.methodLine, argv[c] );
        }
        else
        {
...
const char* _SParseHTTP_GetUpToNext( const char* start, const char* lineEnd, char separator, string& out )
{
    ...
    while( (end < lineEnd) && (*end != separator) ) ++end;
...
int SParseHTTP( const char* buffer, size_t length, string& methodLine, STable& nameValueMap, string& content )
{
    ...
    while( lineStart < inputEnd )
    {
        ...
        if( lineEnd == lineStart )
        {
            ...
            if( !methodLine.empty( ) )
            {
                ...
                else if( !SIEquals( nameValueMap["Transfer-Encoding"], "chunked" ) )
                {
                    ...
                    int contentLength = ( SContains<string,string>( nameValueMap, "Content-Length" ) ? atoi( nameValueMap["Content-Length"].c_str( ) ) : 0 );
                    if( !contentLength ) return headerLength;
        ...
        else
        {
            ...
            if( isHeaderOrFooter )
            {
                ...
                if( isspace( *lineStart ) )
                {
                    ...
                    if( !name.empty( ) ) SAppend( nameValueMap[ name ], lineStart, (int)(lineEnd-lineStart) );
                    else                 SAppend( methodLine, lineStart, (int)(lineEnd-lineStart) );
                }
                else
                {
        ...
        else if( inputEnd-lineStart >= 2 && lineStart[0] == '\n' && lineStart[1] == '\r' ) lineStart += 2;
        else if( lineStart[0] == '\n' ) ++lineStart;
        else if( lineStart[0] == '\r' ) ++lineStart;
        else SWARN( "How did we get here?" );
    }
...
int _SDecodeURIChar( const char* buffer, int length, string& out )
{
    ...
    char outChar = 0;
         if( SWITHIN( '0', buffer[1], '9' ) ) outChar |= (buffer[1] - '0' +  0) << 4;
    else if( SWITHIN( 'a', buffer[1], 'f' ) ) outChar |= (buffer[1] - 'a' + 10) << 4;
    else if( SWITHIN( 'A', buffer[1], 'F' ) ) outChar |= (buffer[1] - 'A' + 10) << 4;
    else
    {
        ...
        return 2;        
         ...
         if( SWITHIN( '0', buffer[2], '9' ) ) outChar |= (buffer[2] - '0' +  0);
    else if( SWITHIN( 'a', buffer[2], 'f' ) ) outChar |= (buffer[2] - 'a' + 10);
    else if( SWITHIN( 'A', buffer[2], 'F' ) ) outChar |= (buffer[2] - 'A' + 10);
    else
    {
        // Invalid -- not sure what's going on.  Cancel decode.
...
bool SParseHost( const string& host, string& domain, uint16_t& port )
{
    ...
    if( domain.empty( ) || portStr.empty( ) ) return false; // Invalid host
    
    // Make sure the second part is a valid 16-bit host
    int portInt = atoi( portStr.c_str( ) );
    if( portInt < 0 || portInt > 65535 ) return false; // Invalid port
...
string SEncodeURIComponent( const string& value )
{
    ...
    for( int c=0; c<(int)value.size( ); ++c )
    {
        ...
        if( SWITHIN( 'a', ch, 'z' ) || SWITHIN( 'A', ch, 'Z' ) || SWITHIN( '0', ch, '9' ) ) working += ch;
        else switch( ch )
        {
            case ' ' :
...
{
    // Is it an integer?
    if( SToStr( SToInt64( value.c_str( ) ) ) == value ) return value;

    // Is it boolean?
    if( SIEquals( value, "true"  ) ) return "true";
    if( SIEquals( value, "false" ) ) return "false";

    // Is it null?
    if( SIEquals( value, "null" ) ) return "null";
...
// --------------------------------------------------------------------------
#define _JSONWS( ) do { while( ptr < end && isspace(*ptr) ) ++ptr; if( ptr >= end ) return ptr; } while(0)
#define _JSONTEST( _CH_ ) do { if( ptr >= end || *ptr != _CH_ ) { if( ptr>=end ) SDEBUG("Expecting: '" << _CH_ << "', found 'eol'" ); else SDEBUG( "Expecting: '" << _CH_ << "', found '" << ptr << "'" ); return NULL; } ++ptr; } while(0)
#define _JSONASSERTPTR( ) do { if( ptr == NULL ) return NULL; } while(0)
//#define _JSONLOG( ) do { cout << __LINE__ << ": " << ptr << endl; } while(0)
#define _JSONLOG( ) do { } while(0)
...
_JSONLOG( );
    ...
    _JSONTEST( '[' );
    _JSONWS( );
    if( *ptr == ']' ) return ptr+1; // Empty array
    while( true )
    {
        // Find the value
        _JSONWS( );
        ...
        if( populateOut ) out.push_back( value );
_JSONLOG();

        // See if we're done
        _JSONWS( );
        if( *ptr == ']' ) return ptr+1; // Done
        _JSONTEST( ',' );
    }
}
...
_JSONLOG( );
    ...
    _JSONTEST( '{' );
    _JSONWS( );
    if( *ptr == '}' ) return ptr+1; // Empty object
    while( true )
    {
        // Find the name
        _JSONWS( );
...
_JSONLOG();
        ...
        // See if we're done
        _JSONWS( );
        if( *ptr == '}' ) return ptr+1; // Finished this object
        _JSONTEST( ',' );
    }
}
...
_JSONLOG( );
    ...
    switch( *ptr )
    {
        ...
        default :
        {
            ...
            {
                // Found boolean true
                if( populateValue ) value = "true";
                ptr += 4; // strlen(true)
            }
            else if( !strncmp( ptr, "false", 5 ) )
            {
                // Found boolean false
                if( populateValue ) value = "false";
                ptr += 5; // strlen(false)
            }
            else if( !strncmp( ptr, "null", 4 ) )
            {
                // Found null
                if( populateValue ) value = "null";
                ptr += 4; // strlen(null)
            }
...
string SGZip( const string& content )
{
    ...
    int status = deflateInit2( &stream, Z_BEST_COMPRESSION, Z_DEFLATED, MAX_WBITS | GZIP_ENCODING, MAX_MEM_LEVEL, Z_DEFAULT_STRATEGY );
...
int S_socket( const string& host, bool isTCP, bool isPort, bool isBlocking )
{
    ...
    {    
        ...
        string domain;
        uint16_t port = 0;
        if( !SParseHost( host, domain, port ) ) throw "invalid host";        
        ...
        if( !ip || ip == INADDR_NONE )
        {
            ...
            if( !hostent || hostent->h_length != 4 || !hostent->h_addr_list ) throw "can't resolve host";
            in_addr* addr = (in_addr*)hostent->h_addr_list[0];
            ip = addr->s_addr;
        }
            
        // Open a socket
        if( isTCP ) s = (int)socket( AF_INET, SOCK_STREAM, IPPROTO_TCP );
        else        s = (int)socket( AF_INET, SOCK_DGRAM,  IPPROTO_UDP );
        if( !s || s == -1 ) throw "couldn't open";
        ...
        if( !isBlocking )
        {
            ...
            if( (flags < 0) || fcntl( s, F_SETFL, flags | O_NONBLOCK ) ) throw "couldn't set non-blocking";
        }
        ...
        if( isPort )
        {
            ...
            u_long enable = 1;
            if( setsockopt( s, SOL_SOCKET, SO_REUSEADDR, (char*)&enable, sizeof(enable) ) ) throw "couldn't set REUSEADDR";
            ...
            if( isTCP && listen( s, SOMAXCONN ) ) throw "couldn't listen";
        }
        else
        {
            // If TCP, connect
            ...
            if( connect( s, (sockaddr*)&addr, sizeof(addr) ) == -1 )
                switch( S_errno )
                {
                    ...
                    default:  throw "couldn't connect";
                }
        }
    ...
    {
        // Failed to open
        SWARN( "Failed to open " << ( isTCP ? "TCP" : "UDP" ) << ( isPort ? " port" : " socket" ) << " '" << host << "': " << message << "(errno=" << S_errno << " '" << strerror(S_errno) << "')" );
    ...
    else if( numRecv < 0 )
    {
        ...
        switch( S_errno )
        {
            ...
            case S_ECONNRESET: 
                ...
                SWARN( "recvfrom(" << fromAddr << ") failed with response '" << strerror(S_errno) << "' (#" << S_errno << "), closing." );
                return -1; // Request close
            ...
            default:
                ...
                SWARN( "recvfrom(" << fromAddr << ") failed with response '" << strerror(S_errno) << "' (#" << S_errno << "), ignoring." );
                return 0;
        }
    }
    else
...
int S_accept( int port, sockaddr_in& fromAddr, bool isBlocking )
{
    ...
    else
    {
        ...
        switch( S_errno )
        {
            ...
            case S_EOPNOTSUPP:
                // Interesting; reset the port and hope it clears.
                SWARN( "accept() failed with response '" << strerror(S_errno) << "' (#" << S_errno << ") from '" << fromAddr << "', closing." );
                return -1; // Request close of the socket
            
            case S_ECONNRESET:
            default:
                // Interesting, but non-fatal
                SWARN( "accept() failed with response '" << strerror(S_errno) << "' (#" << S_errno << ") from '" << fromAddr << "', ignoring." );
...
bool S_recvappend( int s, string& recvBuffer )
{
    ...
    while( (numRecv = recvfrom( s, buffer, sizeof(buffer), 0, (sockaddr*)&fromAddr, &fromAddrLen)) > 0 )
    {
        ...
        if( blocking ) return true; // We're still alive
    }
        
    // See how we finished
    if( numRecv == 0 ) return false; // Graceful shutdown; socket closed
    else
    {
        // Some kind of error -- what happened?
        switch( S_errno )
        {
            ...
            default:
                ...
                SWARN( "recv(" << fromAddr << ") failed with response '" << strerror(S_errno) << "' (#" << S_errno << "), closing." );
...
SASSERT( s );
    ...
    if( sendBuffer.empty( ) ) return true; // Assume no error, still alive
    
    // Send as much as we can
    ssize_t numSent = send(s, sendBuffer.c_str(), (int)sendBuffer.size(), MSG_NOSIGNAL);
    if( numSent > 0 ) SConsumeFront( sendBuffer, numSent );
    
    // Exit of no error
    if( numSent >= 0 ) return true; // No error; still alive
...
int S_poll(fd_map& fdm, uint64_t timeout)
{
    ...
    for_each(fdm.begin(), fdm.end(), [&](pair<int, pollfd> pfd){
    ...
    for_each(pollvec.begin(), pollvec.end(), [&](pollfd pfd){
...
bool SFileLoad( const string& path, string& buffer )
{
    ...
    if( !fp ) return false; // Couldn't open
...
bool SFileSave( const string& path, const string& buffer )
{
    ...
    fclose( fp );
    if( numWritten == buffer.size( ) ) return true; // Success
...
bool SFileCopy( const string& fromPath, const string& toPath )
{
    ...
    try
    {
        ...
        if( !from ) throw "read open error";
        if( !to   ) throw "write open error";
...
bool SQuery( sqlite3* db, const char* e, const string& sql, SQResult& result, int64_t warnThreshold )
{
    ...
    for(int tries = 0; tries < MAX_TRIES; tries++) {
        ...
        SWARN("sqlite3_exec returned SQLITE_BUSY on try #" << (tries + 1) << " of " << MAX_TRIES << ". "
    ...
    // All done!
    if( error == SQLITE_OK ) return true;
...
int STestLibStuff( )
{
...

    {
        ...
        STESTEQUALS( SToHex( SHMACSHA1( "key", "The quick brown fox jumps over the lazy dog" ) ), "DE7C9B85B8B78AA6BC8A7A36F70A90701C9DB4D9" );
    }
    ...
    STestTimer test( "Testing JSON encode/decode", testGroup );
        ...
        innerObject0["badJSON"] = "{\"name\":\"bb\",\"type\":\"text\"}],\"approverTable\":false,\"ccList\":[],\"approver\":\"blah@company.com\"}";
        innerObject0["ofxTest"] = "{\"ofx\":\"<OFX>\\n<SIGNONMSGSRSV1>\\n<SONRS>\\n<STATUS><CODE>15501\\n<SEVERITY>ERROR\\n<MESSAGE>The profile currently in use.\\n<\\/STATUS>\\n<DTSERVER>20110304033419.851[0:GMT]\\n<LANGUAGE>ENG\\n<FI>\\n<ORG>BB&amp;T\\n<FID>BB&amp;T\\n<\\/FI>\\n<\\/SONRS>\\n<\\/SIGNONMSGSRSV1>\\n<SIGNUPMSGSRSV1>\\n<ACCTINFOTRNRS>\\n<TRNUID>315B1FB9-913C-991F-1BD7-6537CAA54C26\\n<STATUS>\\n<CODE>15500\\n<SEVERITY>ERROR\\n<\\/STATUS>\\n<CLTCOOKIE>4\\n<\\/ACCTINFOTRNRS>\\n<\\/SIGNUPMSGSRSV1>\\n<\\/OFX>\"}";
        ...
        STable ignore = SParseJSONObject( SStrFromHex( "" ) );
    }
...

    {
        ...
        // php -r 'echo "\"".gzdecode(hex2bin("1F8B08000000000002032BC9C82C5600A2448592D4E21200EAE71E0D0E000000"))."\"\n";'
        // "this is a test"
    }

    {
        ...
        STESTASSERT(SConstantTimeEquals("1F8B08000000000002032BC9C82C5600A2448592D4E21200EAE71E0D0E000000", "1F8B08000000000002032BC9C82C5600A2448592D4E21200EAE71E0D0E000000"));
