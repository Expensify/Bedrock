...
// **NOTE: _g_SThread_TLSKey can stay 0 when initialized, else we'd just look at
// it
...
string SToHex(const string &value) {
  ...
  for (size_t c = 0; c < value.size(); ++c) {
    ...
    working +=
        ((digit >> 4) < 10 ? '0' + (digit >> 4) : 'A' + (digit >> 4) - 10);
    working +=
        ((digit & 0xF) < 10 ? '0' + (digit & 0xF) : 'A' + (digit & 0xF) - 10);
  }
  return working;
}
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Converts a number to "base36", meaning numbers and letters (case
// insensitive).
...
// --------------------------------------------------------------------------
string SReplace(const string &value, const string &find,
...
// --------------------------------------------------------------------------
string SReplaceAllBut(const string &value, const string &safeChars,
                      char replaceChar) {
...
// --------------------------------------------------------------------------
string SReplaceAll(const string &value, const string &unsafeChars,
                   char replaceChar) {
  // Loop across the string and replace any invalid character
  string out;
  for (size_t c = 0; c < value.size(); ++c)
...
// --------------------------------------------------------------------------
int SStateNameToInt(const char *states[], const string &stateName,
...
// --------------------------------------------------------------------------
bool SParseIntegerList(const char *ptr, list<int64_t> &valueList,
                       char separator) {
  // Clear the input
  valueList.clear();
  ...
  string component;
  while (*ptr) {
    // Is this the start of a new string?  If so, ignore to trim leading
    // whitespace.
...
bool SParseVector(const char *ptr, vector<string> &valueVector,
                  char separator) {
  // STL way:
  //
  // valueVector.clear();
...
SData SParseCommandLine(int argc, char *argv[]) {
  // Just walk across and find the pairs, then put the remainder on a list in
  // the method
  SData results;
  string name;
...
/////////////////////////////////////////////////////////////////////////////
// --------------------------------------------------------------------------
const char *_SParseHTTP_GetUpToNext(const char *start, const char *lineEnd,
                                    char separator, string &out) {
...
void _SParseHTTP_GetUpToEnd(const char *start, const char *end, string &out) {
  // Get everything up to the end of the line, triming leading and trailing
  // whitespace
  while (*start == ' ')
    ++start;
  while (*(end - 1) == ' ')
...
// --------------------------------------------------------------------------
int SParseHTTP(const char *buffer, size_t length, string &methodLine,
               STable &nameValueMap, string &content) {
  // Clear the output
  methodLine.clear();
  nameValueMap.clear();
  content.clear();
  ...
  while (lineStart < inputEnd) {
    ...
    if (lineEnd == lineStart) {
      // Blank line -- if we have at least the method, then we're done.
      // Otherwise, ignore.
      if (!methodLine.empty()) {
        // If we are done processing a chunked body.
        if (isChunked) {
          // Figure out the end of the message by consuming up to 2 EOL
          // characters,
          // then return the total length.
          SASSERTWARN(lastChunkFound);
          const char *parseEnd = lineEnd;
          int numEOLs = 2;
          while (parseEnd < inputEnd &&
                 (*parseEnd == '\r' || *parseEnd == '\n') && numEOLs--)
            ++parseEnd;
        ...
        // If not processing a chunked body, then finish up.
          ...
          // If there is no content-length, just return the length of the
          // headers
          int contentLength =
              (SContains<string, string>(nameValueMap, "Content-Length")
                   ? atoi(nameValueMap["Content-Length"].c_str())
                   : 0);
          if (!contentLength)
            return headerLength;

          // There is a content length -- if we don't have enough, then cancel
          // the parse.
      ...
      // Is it a new chunk?
        ...
        // Get the chunk length and ignore the optional stuff after the optional
        // semicolon.
        string chunkHeader;
        _SParseHTTP_GetUpToEnd(lineStart, lineEnd, chunkHeader);
        const string &hexChunkLength = SContains(chunkHeader, ";")
                                           ? SBefore(chunkHeader, ";")
                                           : chunkHeader;
        ...
        // Else it is a footer which should be treated just like a header.  Set
      ...
      if (isHeaderOrFooter) {
        ...
        if (isspace(*lineStart)) {
          // Starts with whitespace -- if we have a name, add it to the end of
          // the last
          ...
          const char *nameEnd =
              _SParseHTTP_GetUpToNext(lineStart, lineEnd, ':', name);
          if (!name.empty()) {
    ...
    if (inputEnd - lineStart >= 2 && lineStart[0] == '\r' &&
        lineStart[1] == '\n')
      lineStart += 2;
...
// --------------------------------------------------------------------------
bool SParseRequestMethodLine(const string &methodLine, string &method,
                             string &uri) {
  // Clear the input
  method.clear();
  uri.clear();
...
// --------------------------------------------------------------------------
bool SParseResponseMethodLine(const string &methodLine, string &protocol,
  ...
  const char *codeEnd =
      _SParseHTTP_GetUpToNext(protocolEnd + 1, end, ' ', codeStr);
...
bool SParseURI(const char *buffer, int length, string &host, string &path) {
  ...
  if (strncmp(buffer, "http://", strlen("http://")) &&
      strncmp(buffer, "https://", strlen("https://")))
...
// --------------------------------------------------------------------------
bool SParseURIPath(const char *buffer, int length, string &path,
                   STable &nameValueMap) {
  // Clear the output
  path.clear();
  nameValueMap.clear();
...
// --------------------------------------------------------------------------
void SComposeHTTP(string &buffer, const string &methodLine,
                  const STable &nameValueMap, const string &content) {
  bool tryGzip = false;
  ...
  SFOREACHTABLE(nameValueMap, mapIt) {
    if (SIEquals("Set-Cookie", mapIt->first)) {
      ...
      SParseList(mapIt->second, cookieList,
                 S_COOKIE_SEPARATOR); // A bit of a hack, yuck
      SFOREACH(list<string>, cookieList, cookieIt) {
        buffer += "Set-Cookie: " + *cookieIt + "\r\n";
      }
    } else if (SIEquals("Content-Length", mapIt->first)) {
      // Ignore Content-Length; will be generated fresh later
    } else if (SIEquals("Content-Encoding", mapIt->first) &&
               SIEquals("gzip", mapIt->second)) {
...
string SEncodeURIComponent(const string &value) {
  ...
  // it "escapes all characters except the following: alphabetic, decimal
  // digits, - _ . ! ~ * ' ( )"
...
// --------------------------------------------------------------------------
extern const char *_SParseJSONValue(const char *ptr, const char *end,
                                    string &value, bool populateValue);
...
string SComposeJSONObject(const STable &nameValueMap, const bool forceString) {
  ...
  SFOREACHTABLE(nameValueMap, mapIt)
  working +=
      "\"" + mapIt->first + "\":" + SToJSON(mapIt->second, forceString) + ",";
...
const char *_SParseJSONString(const char *ptr, const char *end, string &out,
                              bool populateOut) {
  SASSERT(ptr && end);
  SASSERT(*ptr);
  _JSONLOG();
...
// --------------------------------------------------------------------------
const char *_SParseJSONArray(const char *ptr, const char *end,
                             list<string> &out, bool populateOut) {
  SASSERT(ptr && end);
  SASSERT(*ptr);
  _JSONLOG();
...
// --------------------------------------------------------------------------
const char *_SParseJSONObject(const char *ptr, const char *end, STable &out,
                              bool populateOut) {
  SASSERT(ptr && end);
  SASSERT(*ptr);
  _JSONLOG();
...
// --------------------------------------------------------------------------
const char *_SParseJSONValue(const char *ptr, const char *end, string &value,
                             bool populateValue) {
  _JSONLOG();
...
STable SParseJSONObject(const string &object) {
  ...
  if (parseEnd < end) {
    ...
    if (parseEnd) {
      SWARN("Incomplete parse at:" << parseEnd << "(" << (int)(end - parseEnd)
                                   << ", ch:" << (int)(*parseEnd) << ")");
    } else {
...
string SGZip(const string &content) {
  ...
  int status = deflateInit2(&stream, Z_BEST_COMPRESSION, Z_DEFLATED,
                            MAX_WBITS | GZIP_ENCODING, MAX_MEM_LEVEL,
                            Z_DEFAULT_STRATEGY);
...
int S_socket(const string &host, bool isTCP, bool isPort, bool isBlocking) {
  ...
  try {
    ...
    if (!ip || ip == INADDR_NONE) {
      ...
      // NOTE: gethostbyname blocks so set the DNS timeout to 1s in
      // /etc/resolv.conf
      uint64_t start = STimeNow();
      hostent *hostent = gethostbyname(domain.c_str());
      uint64_t elapsed = STimeNow() - start;
      if (elapsed > 100 * STIME_US_PER_MS)
        SWARN("Slow DNS lookup. " << elapsed / STIME_US_PER_MS << "ms for '"
                                  << domain << "'.");
    ...
    if (isPort) {
      ...
      u_long enable = 1;
      if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (char *)&enable,
                     sizeof(enable)))
    ...
    // Failed to open
    SWARN("Failed to open " << (isTCP ? "TCP" : "UDP")
                            << (isPort ? " port" : " socket") << " '" << host
                            << "': " << message << "(errno=" << S_errno << " '"
                            << strerror(S_errno) << "')");
    if (s > 0)
      closesocket(s);
    return -1;
...
// --------------------------------------------------------------------------
ssize_t S_recvfrom(int s, char *recvBuffer, int recvBufferSize,
  ...
  SZERO(fromAddr);
  ssize_t numRecv = recvfrom(s, recvBuffer, recvBufferSize - 1, 0,
  ...
  if (numRecv == 0) {
    ...
    SWARN(
        "recvfrom(" << fromAddr
                    << ") failed with graceful shutdown on UDP port, closing.");
    ...
    case S_ECONNRESET:
      ...
      SWARN("recvfrom(" << fromAddr << ") failed with response '"
...
int S_accept(int port, sockaddr_in &fromAddr, bool isBlocking) {
  ...
  if (s != -1) {
    ...
    case S_EOPNOTSUPP:
      // Interesting; reset the port and hope it clears.
      SWARN("accept() failed with response '"
            << strerror(S_errno) << "' (#" << S_errno << ") from '" << fromAddr
            << "', closing.");
...
bool S_recvappend(int s, string &recvBuffer) {
  ...
  sockaddr_in fromAddr;
  socklen_t fromAddrLen = sizeof(fromAddr);
  while ((numRecv = recvfrom(s, buffer, sizeof(buffer), 0,
  ...
  if (numRecv == 0)
    ...
    default:
      ...
      SWARN("recv(" << fromAddr << ") failed with response '"
                    << strerror(S_errno) << "' (#" << S_errno << "), closing.");
...
bool S_sendconsume(int s, string &sendBuffer) {
  ...
  ssize_t numSent =
      send(s, sendBuffer.c_str(), (int)sendBuffer.size(), MSG_NOSIGNAL);
  if (numSent > 0)
  ...
  default:
    // Interesting -- reset the socket and hope it clears
    SWARN("send(" << SGetPeerName(s) << ") failed with response '"
                  << strerror(S_errno) << "' (#" << S_errno << ", closing.");
    return false; // Socket died
...
int S_poll(fd_map &fdm, uint64_t timeout) {
  // Why doesn't this function lock around our fd_map, you might ask? Because in
  // the existing bedrock architecture,
  // each worker thread allocates its own fd_map, and thus different threads
  // wont compete for the same resource
  // here. The only place they share resources is around a bedrock MessageQueue,
  // which does its own locking. If we
  // ever want to allow multiple threads to manipulate a shared fd_map directly,
  // then we need locking in the related
  // functions.

  // Build a vector we can use to pass data to poll().
  vector<pollfd> pollvec;
  for_each(fdm.begin(), fdm.end(),
           [&](pair<int, pollfd> pfd) { pollvec.push_back(pfd.second); });

  // Timeout is specified in microseconds, but poll uses milliseconds, so we
  // divide by 1000.
  ...
  for_each(pollvec.begin(), pollvec.end(),
           [&](pollfd pfd) { fdm[pfd.fd].revents = pfd.revents; });

  if (returnValue == -1) {
    SWARN("Poll failed with response '" << strerror(S_errno) << "' (#"
                                        << S_errno << "), ignoring");
  }
  return returnValue;
}
...
// --------------------------------------------------------------------------
string SAESEncrypt(const string &buffer, const string &ivStr,
                   const string &key) {
  ...
  if (buffer.size() % SAES_BLOCK_SIZE != 0) {
    paddedBuffer.append(
        SAES_BLOCK_SIZE - ((int)buffer.size() % SAES_BLOCK_SIZE), (char)0);
  }
  ...
  mbedtls_aes_crypt_cbc(&ctx, MBEDTLS_AES_ENCRYPT, (int)paddedBuffer.size(), iv,
                        (unsigned char *)paddedBuffer.c_str(),
                        (unsigned char *)encryptedBuffer.c_str());
  return encryptedBuffer;
}

// --------------------------------------------------------------------------
string SAESDecrypt(const string &buffer, const string &ivStr,
                   const string &key) {
  SASSERT(key.size() == SAES_KEY_SIZE);
  ...
  mbedtls_aes_crypt_cbc(&ctx, MBEDTLS_AES_DECRYPT, (int)buffer.size(), iv,
...
bool SFileCopy(const string &fromPath, const string &toPath) {
  ...
  try {
    ...
    // Problem
    SWARN("Failed copying file '" << fromPath << "' to '" << toPath << "' ("
                                  << e << ")");
  }
  if (from)
    fclose(from);
...
string SHashSHA1(const string &buffer) {
  ...
  string result;
  result.resize(20);
  mbedtls_sha1((unsigned char *)buffer.c_str(), (int)buffer.size(),
...
string SEncodeBase64(const string &buffer) {
  ...
  size_t olen = 0;
  mbedtls_base64_encode(0, 0, &olen, (unsigned char *)buffer.c_str(),
                        buffer.size());

  // Next, do the encode
  string out;
  out.resize(olen - 1); // -1 because trailing 0 is implied
  mbedtls_base64_encode((unsigned char *)&out[0], olen, &olen,
                        (unsigned char *)buffer.c_str(), buffer.size());
  return out;
}
...
string SDecodeBase64(const string &buffer) {
  // First, get the required buffer size
  size_t olen = 0;
  mbedtls_base64_decode(0, 0, &olen, (unsigned char *)buffer.c_str(),
                        buffer.size());

  // Next, do the decode
  string out;
  out.resize(olen);
  mbedtls_base64_decode((unsigned char *)&out[0], olen, &olen,
                        (unsigned char *)buffer.c_str(), buffer.size());
  return out;
}
...
// --------------------------------------------------------------------------
void *SThreadOpen(void (*proc)(void *procData), void *procData,
                  const string &name, size_t stackSize) {
...
extern void SQueryLogOpen(const string &logFilename) {
  ...
  if (_g_sQueryLogFP) {
    // Already open
    SHMMM("Attempting to open query log '"
          << logFilename << "' but a log is already open, ignoring.");
  } else {
...
void SQueryLogClose() {
  ...
  if (!_g_sQueryLogFP) {
    ...
    // Clear the global variable and wait a second, in case it's being called
    // right now
...
#define MAX_TRIES 3
  ...
  for (int tries = 0; tries < MAX_TRIES; tries++) {
    ...
    SWARN("sqlite3_exec returned SQLITE_BUSY on try #"
          ...
          << (((tries + 1) < MAX_TRIES) ? "Sleeping 1 second and re-trying."
                                        : "No more retries."));
  ...
  if ((int64_t)elapsed > warnThreshold)
    SWARN("Slow query (" << elapsed / STIME_US_PER_MS << "ms) " << sql.length()
                         << ": " << sql.substr(0, 150));
  ...
  if (_g_sQueryLogFP) {
    ...
    const string &csvRow = "\"" + dbFilename + "\", " + "\"" +
                           SEscape(STrim(sql), "\"", '"') + "\", " +
                           SToStr(elapsed) + "\n";
    SASSERT(fwrite(csvRow.c_str(), 1, csvRow.size(), _g_sQueryLogFP) ==
            csvRow.size());
  }
  ...
  if (error == SQLITE_OK)
    return true;
  SWARN("'" << e << "', query failed with error #" << error << " ("
...
bool SQVerifyTable(sqlite3 *db, const string &tableName, const string &sql) {
  ...
  SQResult result;
  SASSERTQUERY(db,
               "SELECT * FROM sqlite_master WHERE tbl_name=" + SQ(tableName),
               result);
  if (result.empty()) {
...
int STestLibStuff() {
...

  {
    ...
    const string &lowerMethod =
        SToLower("GET /osrvimages/line_square.gif HTTP/1.1");
...

  {
    ...
    STESTEQUALS(SToHex(SHMACSHA1("", "")),
                "FBDB1D1B18AA6C08324B7D64B71FB76370690E1D");
    STESTEQUALS(
        SToHex(SHMACSHA1("key", "The quick brown fox jumps over the lazy dog")),
...

  {
    ...
    STESTEQUALS(SToJSON("{\"imAFloat\":-0.23456789}"),
                "{\"imAFloat\":-0.23456789}");
    STESTEQUALS(SToJSON("{\"imAFloat\":-123456789.23456789}"),
                "{\"imAFloat\":-123456789.23456789}");
    ...
    innerObject0["wackyTest"] =
        "`1234567890-=~!@#$%^&*()_+[]\\{}|.;':\",./<>\\n?";
    innerObject0["badJSON"] = "{\"name\":\"bb\",\"type\":\"text\"}],"
                              "\"approverTable\":false,\"ccList\":[],"
                              "\"approver\":\"blah@company.com\"}";
    innerObject0["ofxTest"] =
        "{\"ofx\":\"<OFX>\\n<SIGNONMSGSRSV1>\\n<SONRS>\\n<STATUS><CODE>"
        "15501\\n<SEVERITY>"
    ...
    list<string> innerObjectListVerify =
        SParseJSONArray(SComposeJSONArray(innerObjectList));
    ...
    STESTEQUALS(innerObject0["ofxTest"],
                SComposeJSONObject(SParseJSONObject(innerObject0["ofxTest"])));

    // Test parsing a crazy thing
    STable ignore = SParseJSONObject(
        SStrFromHex("7D6628F7AE67FBACE9DAF79312C48BA0B41AADD5BA1704E929B96B6F87"
                    "708C0898868D55C0AAAE117CF20F1317D151"
                    "348706C9EDFE8A0CDD13BFB476367DEA2761A102B26443C7D3A464DB49"
                    "A37F1F816B8BEC4C55DBD9DAF0B70652D32A"
                    "CBD224F9487E25398E740E99B24089A6343B6FD6C1BC6A89AF90F3DC69"
                    "016A42066AAF430B1B584D236B8AD285828D"
                    "59BB8375E2E955E246390DE9AA69D05DEF1FBC25318C9CCFE90159EC7E"
                    "AA71637C07BD"));
  }
...

  {
    ...
    recvBuffer = "some method line\r\n"
                 ...
                 "\r\n"
                 "ignored body";
    processed = SParseHTTP(recvBuffer.c_str(), recvBuffer.length(), methodLine,
                           headers, content);
    STESTEQUALS((int64_t)processed,
                (int)recvBuffer.size() - (int)strlen("ignored body"));
